...
export default function UploadAdapterPage() {
  ...
  const handleUpdateCapability = (index: number, field: keyof AdapterCapability, value: any) => {
    ...
    updated[index] = { 
      ...updated[index], 
      [field]: value || (field === 'name' ? '新能力' : field === 'description' ? '请描述此能力' : value)
  ...
  const onSubmit = async (data: AdapterFormData) => {
    try {
      ...
      toast({
        title: '错误',
        description: error.message || '创建失败，请重试',
        variant: 'destructive',
      });
    }
  };
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (currentIndex < tabs.length - 1) {
                    setActiveTab(tabs[currentIndex + 1] || 'media');
                  }
                }}
              >
                下一步
