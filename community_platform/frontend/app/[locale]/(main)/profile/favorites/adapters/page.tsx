...
import { FavoriteAdapterCard } from '@/features/social/components';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
/**
 * FavoriteAdaptersPage Component
 */
export default function FavoriteAdaptersPage() {
  ...
  React.useEffect(() => {
    if (!isAuthenticated) {
      router.push('/login?redirect=/profile/favorites/adapters');
    }
  ...
  const categories = useMemo(() => {
    ...
    favorites.forEach((favorite) => {
      if (favorite.targetAdapter?.category) {
        cats.add(favorite.targetAdapter.category);
      }
    });
  ...
  const filteredFavorites = useMemo(() => {
    ...
    if (categoryFilter !== 'all') {
      result = result.filter(
        (favorite) => favorite.targetAdapter?.category === categoryFilter
      );
    }
  ...
  return (
    <div className="container max-w-6xl mx-auto py-8 px-4">
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold">收藏的适配器</h1>
      ...
      {isLoading ? (
        ...
        <EmptyState
          icon={<Package className="w-full h-full" />}
          title={searchQuery || categoryFilter !== 'all' ? '没有找到相关适配器' : '暂无收藏'}
          description={
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {removeTarget && removeTarget.ids.length > 1 ? (
                <>
                  确定要从收藏中移除这 <span className="font-semibold">{removeTarget.ids.length}</span> 个适配器吗？
                  此操作无法撤销。
                </>
              ) : (
                <>
