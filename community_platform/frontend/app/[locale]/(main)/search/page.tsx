...
export default function SearchPage() {
  ...
  const typeStats = searchResult
    ? SearchResultHelper.getTypeStats((searchResult as SearchResult).items)
    : null;
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between">
            <div className="text-sm text-muted-foreground">
              找到 <span className="font-medium text-foreground">{(searchResult as SearchResult).total}</span>{' '}
              个结果
              {(searchResult as SearchResult).took && (
                <span> ({((searchResult as SearchResult).took / 1000).toFixed(2)} 秒)</span>
              )}
            </div>
            {(searchResult as SearchResult).suggestions && (searchResult as SearchResult).suggestions!.length > 0 && (
              <div className="text-sm">
                <span className="text-muted-foreground">建议搜索: </span>
                {(searchResult as SearchResult).suggestions!.map((suggestion: string, index: number) => (
                  <Button
                    key={index}
                    variant="link"
      ...
      <Tabs value={searchType} onValueChange={(value) => handleTypeChange(value as SearchType)}>
        ...
        <TabsContent value={searchType} className="mt-0">
          {error ? (
            ...
            <>
              <SearchResults
                items={(searchResult as SearchResult | undefined)?.items || []}
                isLoading={isLoading}
              />

              {/* 分页 */}
              {searchResult && (searchResult as SearchResult).totalPages > 1 && (
                <div className="mt-8">
                  <Pagination>
                    <PaginationContent>
                      <PaginationItem>
                      ...
                      {Array.from({ length: Math.min((searchResult as SearchResult).totalPages, 5) }, (_, i) => {
                      ...
                      {(searchResult as SearchResult).totalPages > 5 && (
                        <PaginationItem>
                          <PaginationEllipsis />
                        </PaginationItem>
                      )}
                      ...
                      <PaginationItem>
                        <PaginationNext
                          onClick={() =>
                            page < (searchResult as SearchResult).totalPages && handlePageChange(page + 1)
                          }
                          className={
                            page >= (searchResult as SearchResult).totalPages
