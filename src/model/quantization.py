...
class AdvanceQuantizationManager(ModelManager):
    ...
    def __init__(self, 
        ...
        if self.device == "cpu":
            ...
            if self.get_quantization_method() in ["bnb","gptq","awq"]:
                self.logger.warning(f"CPU上不支持{self.get_quantization_method()}量化方法,将使用动态量化或原始模型")
    ...
    def get_quantization_method(self)->str:
        ...
        if target_method != "auto":
            if target_method in available_backends and available_backends[target_method] and \
               self.methods_config.get(target_method, {}).get("enabled", False):
        ...
        for method in method_priority:
            # 检查方法是否在配置中启用且实际可用
            if self.methods_config.get(method, {}).get("enabled", False) and available_backends.get(method, False):
    ...
    def _load_gptq_model(self,model_path:str)->Any:
        ...
        try:
            ...
            use_triton = gptq_config.get("use_triton",True) and torch.cuda.is_available()
            
            self.logger.info(f"使用GPTQ量化,位数: {bits},分组大小: {group_size},使用triton加速: {use_triton}")
    ...
    def _load_awq_model(self,model_path:str)->Any:
        ...
        try:
            ...
            self.logger.info(f"使用AWQ量化,位数: {awq_config.get('bits',4)},分组大小: {awq_config.get('group_size',128)},使用zero_point: {awq_config.get('zero_point',True)}")
    ...
    def _load_bnb_model(self,model_path:str)->Any:
        ...
        try:
            ...
            self.logger.info(f"使用BNB量化,位数: {bits},计算类型: {compute_dtype_str},使用double_quant: {use_double_quant},量化类型: {quant_type}")
    ...
    def get_memory_usage(self,model:Any)->float:
        ...
        if torch.cuda.is_available():
            memory_info["cuda_allocated"] = torch.cuda.memory_allocated() / (1024 * 1024 * 1024)
            memory_info["cuda_reserved"] = torch.cuda.memory_reserved() / (1024 * 1024 * 1024)
