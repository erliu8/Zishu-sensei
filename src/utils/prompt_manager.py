...
class PromptManager:
        ...
        for file_path in character_path.glob("*.json"):
            try:
                with open(file_path,"r",encoding="utf-8") as f:
                    ...
                    for template_id,template_str in template_data.items():
                        self.character_templates[character_id][template_id] = PromptTemplate(template_str,f"{character_id}.{template_id}")
                    
                    self.logger.info(f"成功加载角色{character_id}提示模板: {file_path.name}")
            except Exception as e:
                self.logger.error(f"加载角色{character_id}提示模板失败{file_path}: {e}")
                
    def get_template(self,template_id:str,character_id:Optional[str]=None)->Optional[PromptTemplate]:
        """
        获取提示模板
    ...
    def format_prompt(self,template_id:str,template_str:str,character_id:Optional[str]=None,**kwargs)->str:
        """
        格式化提示模板
    ...
    def save_template(self,template_id:str,character_id:Optional[str]=None)->bool:
        """
        保存提示模板
        ...
        try:
            if character_id:
                if character_id not in self.character_templates or template_id not in self.character_templates[character_id]:
...
class MultimodelPromptManager(PromptManager):
                    ...
                    for file_path in model_dir.glob("*.json"):
                        try:
                            with open(file_path,"r",encoding="utf-8") as f:
                                ...
                                for template_id,template_str in template_data.items():
                                    self.model_templates[model_id][template_id] = PromptTemplate(template_str,f"{model_id}.{template_id}")
                                    
                                    self.logger.info(f"成功加载模型{model_id}提示模板: {file_path.name}")
                        except Exception as e:
                            self.logger.error(f"加载模型{model_id}提示模板失败: {e}")
        else:
            self.logger.warning(f"模型提示模板目录不存在: {models_path}")
            
    def get_model_template(self,model_id:str,template_id:str)->Optional[PromptTemplate]:
        """获取模型特定提示模板
    ...
    def format_for_teacher_ensemble(self,template_id:str,teacher_ids:List[str],**kwargs)->Dict[str,str]:
        """
        格式化多教师蒸馏提示模板
        ...
        for teacher_id in teacher_ids:
            result[teacher_id] = self.format_for_model(template_id,teacher_id,**kwargs)
        return result
    
    def get_distillation_prompts(self,phase:str,task:str,**kwargs)->Dict[str,Dict[str,str]]:
        """获取蒸馏提示模板
