...
def check_sdpa_support()->Dict[str,Any]:
    ...
    try:
        ...
        if has_sdpa:
            ...
            logger.info(f"设备: {device_props.name}, 计算能力: {device_props.major}.{device_props.minor}")
            ...
            #检查哪些后端可用
            result["mem_efficient_available"] = torch.backends.cuda.mem_efficient_sdp_enabled()
...
def configure_sdpa_for_model(model:torch.nn.Module,config:Dict[str,Any])->torch.nn.Module:
    """配置模型使用SDPA高效注意力机制
    ...
    try:
        ...
        for module_name,module in model.named_modules():
            if hasattr(module,"_attn_implementation") or hasattr(module,"attn_implementation"):
