...
class User(Base):
    ...
    groups = relationship(
        "Group", secondary=user_groups_association, back_populates="users"
    )
    permissions = relationship(
        "Permission", secondary=user_permissions_association, back_populates="users"
    )

    # 自引用关系（用于管理层级）
    invited_users = relationship(
        "User", backref=backref("invited_by", remote_side=[id])
    )
...
class UserSecurity(Base):
    ...
    def set_password(self, password: str):
        ...
        self.password_history.append(
            {"hash": self.password_hash, "created_at": datetime.utcnow().isoformat()}
        )
...
class UserToken(Base):
    ...
    )  # access, refresh, reset_password, etc.
...
def create_user_with_profile(
    session: Session, username: str, email: str, password: str, **profile_data
) -> User:
    """创建用户及其档案"""
    # 创建用户
    user = User(username=username, email=email, status=UserStatus.PENDING)
