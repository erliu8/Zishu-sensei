...
adapter_tags_association = Table(
    ...
    Base.metadata,
    Column(
        "adapter_id", UUID(as_uuid=True), ForeignKey("adapters.id"), primary_key=True
    ),
    Column("tag_id", UUID(as_uuid=True), ForeignKey("tags.id"), primary_key=True),
    Column("created_at", DateTime(timezone=True), default=func.now()),
    schema="zishu",
...
class Community(Base):
    ...
    community_type = Column(
        SQLEnum(CommunityType), nullable=False, default=CommunityType.PUBLIC
    )
    status = Column(
        SQLEnum(CommunityStatus), nullable=False, default=CommunityStatus.ACTIVE
    )
    ...
    tags = relationship(
        "Tag", secondary=community_tags_association, back_populates="communities"
    )
    ...
    def add_member(self, user_id: uuid.UUID, role: MemberRole = MemberRole.MEMBER):
        ...
        member = CommunityMember(community_id=self.id, user_id=user_id, role=role)
    ...
    def update_post_count(self):
        """更新帖子数量"""
        self.post_count = len(
            [p for p in self.posts if p.status == PostStatus.PUBLISHED]
        )
...
class Post(Base):
    ...
    def add_reaction(self, user_id: uuid.UUID, reaction_type: ReactionType):
        ...
        existing = next(
            (
                ...
                if r.user_id == user_id and r.target_type == "post"
            ),
            None,
        )
        if existing:
    ...
    def update_comment_count(self):
        ...
        self.comment_count = len(
            [c for c in self.comments if c.status == CommentStatus.PUBLISHED]
        )
        self.last_activity_at = func.now()
...
class Comment(Base):
    ...
    content = Column(Text, nullable=False)
    status = Column(
        SQLEnum(CommentStatus), nullable=False, default=CommentStatus.PUBLISHED
    )
    ...
    def add_reaction(self, user_id: uuid.UUID, reaction_type: ReactionType):
        ...
        existing = next(
            (
                r
                for r in self.reactions
                if r.user_id == user_id and r.target_type == "comment"
            ),
            None,
        )
        if existing:
    ...
    def update_reply_count(self):
        """更新回复数量"""
        self.reply_count = len(
            [r for r in self.replies if r.status == CommentStatus.PUBLISHED]
        )
...
class Adapter(Base):
    ...
    reviews = relationship(
        "AdapterReview", back_populates="adapter", cascade="all, delete-orphan"
    )
    tags = relationship(
        "Tag", secondary=adapter_tags_association, back_populates="adapters"
    )
    ...
    def add_review(
        self, user_id: uuid.UUID, rating: int, title: str = None, content: str = None
    ):
        """添加评价"""
...
class Tag(Base):
    ...
    communities = relationship(
        "Community", secondary=community_tags_association, back_populates="tags"
    )
    posts = relationship("Post", secondary=post_tags_association, back_populates="tags")
    adapters = relationship(
        "Adapter", secondary=adapter_tags_association, back_populates="tags"
    )
...
def create_community_with_owner(
    session: Session, name: str, slug: str, owner_id: uuid.UUID, **kwargs
) -> Community:
    """创建社区并设置所有者"""
    community = Community(name=name, slug=slug, owner_id=owner_id, **kwargs)
    ...
    member = CommunityMember(
        community_id=community.id, user_id=owner_id, role=MemberRole.OWNER
    )
    session.add(member)
...
def get_user_communities(session: Session, user_id: uuid.UUID) -> List[Community]:
    ...
    return (
        ...
        .filter(CommunityMember.user_id == user_id, Community.is_deleted == False)
        .all()
    )


def get_trending_posts(
    session: Session, community_id: uuid.UUID = None, limit: int = 10
) -> List[Post]:
    """获取热门帖子"""
    query = session.query(Post).filter(
        Post.status == PostStatus.PUBLISHED, Post.is_deleted == False
    )
    ...
    return (
        query.order_by(
            (Post.like_count + Post.comment_count * 2).desc(), Post.created_at.desc()
        )
        .limit(limit)
        .all()
    )
...
def get_popular_adapters(
    session: Session, category: AdapterCategory = None, limit: int = 10
) -> List[Adapter]:
    """获取热门适配器"""
    query = session.query(Adapter).filter(
        Adapter.status == AdapterStatus.PUBLISHED, Adapter.is_deleted == False
    )
    ...
    return (
        query.order_by(Adapter.download_count.desc(), Adapter.rating.desc())
        .limit(limit)
        .all()
    )
...
def search_communities(
    ...
    return (
        ...
        .order_by(Community.member_count.desc(), Community.is_featured.desc())
        .limit(limit)
        .all()
    )
...
def cleanup_soft_deleted_data(session: Session, days_old: int = 30) -> Dict[str, int]:
    ...
    results["communities"] = (
        session.query(Community)
        .filter(Community.is_deleted == True, Community.deleted_at < cutoff_date)
        .delete()
    )
    ...
    results["posts"] = (
        session.query(Post)
        .filter(Post.is_deleted == True, Post.deleted_at < cutoff_date)
        .delete()
    )
    ...
    results["comments"] = (
        ...
        .filter(Comment.is_deleted == True, Comment.deleted_at < cutoff_date)
        .delete()
    )
    ...
    results["adapters"] = (
        ...
        .filter(Adapter.is_deleted == True, Adapter.deleted_at < cutoff_date)
        .delete()
    )
...
def get_community_stats(session: Session, community_id: uuid.UUID) -> Dict[str, Any]:
    ...
    stats = {
        ...
        "total_comments": session.query(Comment)
        .join(Post)
        .filter(
            Post.community_id == community_id, Comment.status == CommentStatus.PUBLISHED
        )
        .count(),
    }
    ...
    stats.update(
        {
            "posts_today": session.query(Post)
            .filter(
            ...
            .count(),
            "posts_this_week": session.query(Post)
            .filter(
                Post.community_id == community_id,
            ...
            .count(),
            "posts_this_month": session.query(Post)
            .filter(
                Post.community_id == community_id,
...
def get_adapter_stats(session: Session, adapter_id: uuid.UUID) -> Dict[str, Any]:
    ...
    for i in range(1, 6):
        count = (
            session.query(AdapterReview)
            .filter(AdapterReview.adapter_id == adapter_id, AdapterReview.rating == i)
            .count()
        )
...
__all__ = [
    ...
    "Tag",
    # 关联表
    "community_tags_association",
    "post_tags_association",
    "adapter_tags_association",
    # 工具函数
    "create_community_with_owner",
    "get_user_communities",
    "get_trending_posts",
    "get_popular_adapters",
    "search_communities",
