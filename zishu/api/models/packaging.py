...
from sqlalchemy import Column, String, Text, Integer, Float, Boolean, DateTime, JSON, ForeignKey, Enum, Index
...
from ..schemas.packaging import PackageStatus, BuildStatus, TestType, PackageType, DistributionChannel, PackageFormat
...
class PackageModel(Base):
    ...
    # 状态信息
    status = Column(Enum(PackageStatus), default=PackageStatus.DRAFT, nullable=False, index=True, comment="包状态")
    ...
    # 反向关系
    build_tasks = relationship("BuildTaskModel", back_populates="package", cascade="all, delete-orphan")
    test_results = relationship("TestResultModel", back_populates="package", cascade="all, delete-orphan")
    publish_results = relationship("PublishResultModel", back_populates="package", cascade="all, delete-orphan")
    package_versions = relationship("PackageVersionModel", back_populates="package", cascade="all, delete-orphan")
    package_files = relationship("PackageFileModel", back_populates="package", cascade="all, delete-orphan")
    signatures = relationship("PackageSignatureModel", back_populates="package", cascade="all, delete-orphan")
...
class PackageVersionModel(Base):
    ...
    version_type = Column(String(20), default="release", comment="版本类型")  # release, prerelease, beta, alpha
...
class PackageFileModel(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    package_id = Column(UUID(as_uuid=True), ForeignKey('packages.id'), nullable=False, index=True)
    version_id = Column(UUID(as_uuid=True), ForeignKey('package_versions.id'), index=True)
    ...
    # 文件类型
    file_category = Column(String(50), comment="文件分类")  # source, config, doc, test, asset
...
class BuildTaskModel(Base):
    ...
    # 状态信息
    status = Column(Enum(BuildStatus), default=BuildStatus.PENDING, nullable=False, index=True, comment="构建状态")
...
class TestResultModel(Base):
    ...
    # 测试结果
    status = Column(String(20), nullable=False, comment="测试状态")  # passed, failed, error, skipped
...
class PublishResultModel(Base):
    ...
    # 发布状态
    status = Column(String(20), nullable=False, comment="发布状态")  # success, failed, partial
...
class PackageSignatureModel(Base):
    ...
    # 签名信息
    signature_type = Column(String(50), nullable=False, comment="签名类型")  # gpg, x509, ed25519
...
class PackageDependencyModel(Base):
    ...
    # 依赖类型
    dependency_type = Column(String(50), default="runtime", comment="依赖类型")  # runtime, dev, build, test
...
class PackageDownloadModel(Base):
    ...
    # 下载信息
    download_date = Column(DateTime(timezone=True), server_default=func.now(), nullable=False, index=True)
