    ...
    async def initialize(self):
        ...
        try:
            ...
            self._auth_middleware = AuthMiddleware(
                security_manager=self._security_manager, config=middleware_config
            )
...
async def create_user_session(
    user_id: str, ip_address: str, user_agent: str, remember_me: bool = False
) -> Dict[str, Any]:
    """创建用户会话"""
    auth_system = await get_auth_system()
...
async def validate_password(
    password: str, user_info: Optional[Dict[str, str]] = None
) -> Dict[str, Any]:
    """验证密码强度"""
    try:
        auth_system = await get_auth_system()
...
async def verify_password(plain_password: str, hashed_password: str) -> bool:
    ...
    try:
        auth_system = await get_auth_system()
        return auth_system.password_manager.verify_password(
    ...
    async def main():
        ...
        async with auth_system_context(config) as auth_system:
            ...
            token_data = await validate_user_token(
                session_data["access_token"], "127.0.0.1"
            )
