...
class AuthConfig:
    ...
    # 中间件配置
    middleware_excluded_paths: List[str] = field(
    ...
    audit_sensitive_paths: List[str] = field(
        default_factory=lambda: ["/auth", "/admin"]
    )
    audit_exclude_paths: List[str] = field(
        default_factory=lambda: ["/health", "/ping", "/docs"]
    )
    ...
    def from_env(cls) -> "AuthConfig":
        ...
        config.refresh_token_expire_days = int(
            os.getenv("REFRESH_TOKEN_EXPIRE_DAYS", config.refresh_token_expire_days)
        )

        # 密码策略
        config.password_min_length = int(
            os.getenv("PASSWORD_MIN_LENGTH", config.password_min_length)
        )
        ...
        # 会话配置
        config.session_timeout_minutes = int(
            os.getenv("SESSION_TIMEOUT_MINUTES", config.session_timeout_minutes)
        )
        config.session_max_concurrent = int(
            os.getenv("SESSION_MAX_CONCURRENT", config.session_max_concurrent)
        )

        # 安全配置
        config.security_encryption_key = os.getenv(
            "SECURITY_ENCRYPTION_KEY", config.security_encryption_key
        )
        config.security_max_login_attempts = int(
            os.getenv("MAX_LOGIN_ATTEMPTS", config.security_max_login_attempts)
        )
        config.security_lockout_duration = int(
            os.getenv("LOCKOUT_DURATION", config.security_lockout_duration)
        )
        config.security_enable_2fa = os.getenv("ENABLE_2FA", "true").lower() == "true"

        # 邮件配置
        config.email_smtp_server = os.getenv(
            "EMAIL_SMTP_SERVER", config.email_smtp_server
        )
        config.email_smtp_port = int(
            os.getenv("EMAIL_SMTP_PORT", config.email_smtp_port)
        )
        ...
        config.email_from_address = os.getenv(
