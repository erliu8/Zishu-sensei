...
from ..schemas.packaging import (
    BuildStatus, TestType, PackageStatus, DistributionChannel,
    BuildTaskResponse, TestResultResponse, PublishResultResponse
)
...
def publish_package_task(
    ...
    **kwargs
    ...
    try:
        ...
        try:
            result = loop.run_until_complete(
                _execute_publish_task(package_id, version, channels, publish_config, user_id)
            )
    ...
    try:
        ...
        if pipeline_config.get('build_enabled', True):
            ...
            build_result = build_package_task.apply_async(
                args=[pipeline_config['build_task_id'], user_id],
                queue='build'
            ).get(timeout=3600)
...
def cleanup_task(self, cleanup_config: Dict, **kwargs):
    ...
    try:
        ...
        if cleanup_config.get('clean_build_files', True):
            try:
                result = _cleanup_build_files(cleanup_config.get('build_retention_days', 7))
        ...
        if cleanup_config.get('clean_task_records', True):
            try:
                result = _cleanup_task_records(cleanup_config.get('task_retention_days', 30))
    ...
    try:
        ...
        try:
            ...
            if workspace_dir.exists() and workspace_dir.is_dir():
                ...
                health_status['services']['filesystem'] = 'unhealthy: workspace not accessible'
                health_status['overall_status'] = 'unhealthy'
        except Exception as e:
...
celery_app.conf.beat_schedule = {
    ...
    'daily-cleanup': {
        'task': 'cleanup_task',
        'schedule': 86400.0,  # 每天
        'args': [{'clean_build_files': True, 'clean_task_records': True, 'clean_cache': True}],
