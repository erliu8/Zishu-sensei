...
class FileService:
            ...
            if self.redis_client:
                ...
                if cached_data:
                    ...
                    if user_id and not await self._check_file_access(
                        file_info, user_id, FileOperation.READ
                    ):
    ...
    async def update_file(
        self, file_id: str, update_data: FileUpdateRequest, user_id: str
    ) -> FileInfo:
        """
        更新文件信息
        ...
        try:
            ...
            if not await self._check_file_access(
                file_info, user_id, FileOperation.UPDATE
            ):
            ...
            if (
                ...
                and update_data.description != file_info.description
            ):
            ...
            if (
                update_data.folder_id is not None
                ...
                if update_data.folder_id:
                    ...
                    if not await self._check_folder_access(
                        target_folder, user_id, FileOperation.CREATE
                    ):
            ...
            if (
                update_data.access_level
                and update_data.access_level != file_info.security.access_level
            ):
                # 更新安全信息
                ...
                changes.append(
                    f"访问级别: {file_info.security.access_level} -> {update_data.access_level}"
                )
    ...
    async def delete_file_permanently(self, file_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            # 权限检查
            if not await self._check_file_access(
                file_info, user_id, FileOperation.DELETE
            ):
            ...
            if self.db_session:
                # 删除相关的权限记录
                await self.db_session.execute(
                    delete(FilePermissionModel).where(
                ...
                # 删除文件记录
                await self.db_session.execute(delete(File).where(File.id == file_id))
    ...
    async def search_files(
        self, search_request: FileSearchRequest, user_id: str
    ) -> FileListResponse:
        """
        搜索文件
    ...
    async def list_folders(
        self, parent_id: Optional[str] = None, owner_id: Optional[str] = None
    ) -> List[FolderInfo]:
        """获取文件夹列表"""
        try:
            if not self.db_session:
    ...
    async def revoke_permission(
        self, file_id: str, user_id: str, permission: Optional[FileOperation] = None
    ) -> bool:
        """撤销文件权限"""
        try:
            if not self.db_session:
    ...
    async def get_user_permissions(
        ...
        try:
            ...
            result = await self.db_session.execute(
                select(FilePermissionModel.permission).where(
                    and_(
                        FilePermissionModel.file_id == file_id,
                        FilePermissionModel.user_id == user_id,
                        or_(
    ...
    async def get_file_activities(
        ...
        try:
            ...
            # 构建查询条件
            query = select(FileActivityModel).where(
                FileActivityModel.file_id == file_id
            )
            count_query = select(func.count(FileActivityModel.id)).where(
                FileActivityModel.file_id == file_id
            )
            ...
            offset = (page - 1) * page_size
            query = (
                query.order_by(desc(FileActivityModel.timestamp))
                .offset(offset)
                .limit(page_size)
            )
    ...
    async def get_statistics(
        self, user_id: Optional[str] = None, date_range: int = 30
    ) -> Dict[str, Any]:
        """获取文件统计信息"""
        try:
            if not self.db_session:
            ...
            activity_result = await self.db_session.execute(
                select(func.count(FileActivityModel.id)).where(
                    and_(
            ...
            for i in range(7):
                ...
                day_result = await self.db_session.execute(
                    ...
                    .select_from(base_query.subquery())
                    .where(
                        and_(File.created_at >= day_start, File.created_at < day_end)
                    )
                )
    ...
    async def create_share(
        self, file_ids: List[str], share_request: FileShareRequest, user_id: str
    ) -> Dict[str, Any]:
        """
        创建文件分享
        ...
        try:
            ...
            for file_id in file_ids:
                ...
                if not await self._check_file_access(
                    file_info, user_id, FileOperation.SHARE
                ):
            ...
            share_data = {
                ...
                "password": self._hash_password(share_request.password)
            ...
            if self.redis_client:
                ...
                await self.redis_client.setex(
                    share_key,
                    int((share_request.expires_at - datetime.now()).total_seconds())
                ...
                await self.log_activity(
                    FileActivity(
                        ...
                        details={
                            "share_id": share_id,
                            "share_type": share_request.share_type,
                            "expires_at": share_request.expires_at.isoformat()
                            if share_request.expires_at
                            else None,
                        },
                        success=True,
    ...
    async def batch_operation(
        self, operation: FileOperation, file_ids: List[str], user_id: str, **kwargs
        ...
        for file_id in file_ids:
            try:
                if operation == FileOperation.DELETE:
                    ...
                    updated_file = await self.update_file(
                        file_id, update_request, user_id
                    )
                    ...
                    success = await self.update_file_status(
                        file_id, FileStatus.ARCHIVED
                    )
                    result = FileOperationResponse(
                        success=True,
                        operation=operation,
    ...
    async def move_file(
        self, file_id: str, target_folder_id: Optional[str], user_id: str
    ) -> bool:
        """移动文件到指定文件夹"""
        try:
            update_request = FileUpdateRequest(folder_id=target_folder_id)
    ...
    async def copy_file(
        ...
        return await self._copy_file(
            file_id, user_id, {"target_folder_id": target_folder_id}
        )
    ...
    async def get_folder_tree(
        ...
        try:
            ...
            async def build_tree(
                parent_id: Optional[str], current_depth: int
            ) -> List[Dict[str, Any]]:
    ...
    async def delete_folder(
        ...
        try:
            ...
            # 权限检查
            if user_id and not await self._check_folder_access(
                folder_info, user_id, FileOperation.DELETE
            ):
                raise PermissionDeniedError("没有删除权限")
    ...
    async def update_folder(
        self, folder_id: str, update_data: Dict[str, Any], user_id: str
    ) -> FolderInfo:
        """更新文件夹信息"""
        try:
            folder_info = await self.get_folder(folder_id)
            ...
            # 权限检查
            if not await self._check_folder_access(
                folder_info, user_id, FileOperation.UPDATE
            ):
                raise PermissionDeniedError("没有更新权限")
    ...
    async def get_activity_summary(self, file_id: str) -> Dict[str, int]:
        ...
        try:
            ...
            result = await self.db_session.execute(
                select(FileActivityModel.operation, func.count(FileActivityModel.id))
    ...
    async def _validate_file_creation(self, file_info: FileInfo):
        ...
        # 验证文件大小
        if not FileValidator.validate_file_size(
            file_info.metadata.size, file_info.metadata.file_type
        ):
            ...
            if self.db_session:
                result = await self.db_session.execute(
                    select(func.sum(File.size)).where(
                        and_(
                            File.owner_id == user_id, File.status != FileStatus.DELETED
                        )
                    )
                )
                ...
                if current_usage + file_size > quota_limit:
                    raise ValidationError(
                        f"存储空间不足，当前使用: {FileHelper.format_file_size(current_usage)}, 配额: {FileHelper.format_file_size(quota_limit)}"
                    )
    ...
    async def _check_file_access(
        self, file_info: FileInfo, user_id: str, operation: FileOperation
    ) -> bool:
        """检查文件访问权限"""
        try:
            # 文件所有者拥有所有权限
            ...
            # 检查公开文件的读取权限
            if (
                file_info.security.access_level == FileAccessLevel.PUBLIC
                and operation in [FileOperation.READ, FileOperation.DOWNLOAD]
            ):
                return True
    ...
    async def _check_folder_access(
        self, folder_info: FolderInfo, user_id: str, operation: FileOperation
    ) -> bool:
        """检查文件夹访问权限"""
        # 文件夹所有者拥有所有权限
        ...
        # 检查公开文件夹的读取权限
        if (
            folder_info.access_level == FileAccessLevel.PUBLIC
            and operation == FileOperation.READ
        ):
            return True
    ...
    async def _update_folder_stats(
        self, folder_id: str, size_delta: int = 0, file_delta: int = 0
    ):
        """更新文件夹统计信息"""
        try:
            if self.db_session:
    ...
    async def _copy_file(
        self, file_id: str, user_id: str, kwargs: Dict[str, Any]
    ) -> FileInfo:
        """复制文件内部方法"""
        try:
            # 获取原文件信息
            ...
            # 检查复制权限
            if not await self._check_file_access(
                original_file, user_id, FileOperation.COPY
            ):
                raise PermissionDeniedError("没有复制权限")

            # 创建新的文件信息
            new_file_id = str(uuid.uuid4())
            target_folder_id = kwargs.get(
                "target_folder_id", original_file.parent_folder_id
            )
    ...
    async def _delete_folder_recursive(
        self, folder_id: str, permanent: bool, user_id: Optional[str]
    ):
        """递归删除文件夹内容"""
        try:
            # 获取文件夹内容
            contents = await self.get_folder_contents(
                folder_id, include_hidden=True, user_id=user_id
            )
            ...
            for subfolder in contents.folders:
                try:
                    await self._delete_folder_recursive(
                        subfolder.id, permanent, user_id
                    )
