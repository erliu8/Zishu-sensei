...
from ..schemas.packaging import (
    PackageFormat, BuildStatus, TestType, BuildConfiguration,
    TestConfiguration, FileInfo, PackageType
)
...
class BuildService:
        ...
        try:
            ...
            try:
                ...
                if parallel and len(test_types) > 1:
                    # 并行运行测试
                    tasks = [
                        self._run_single_test(workspace, package, test_type, coverage_required)
                    ...
                    # 串行运行测试
                    for test_type in test_types:
                        result = await self._run_single_test(workspace, package, test_type, coverage_required)
    ...
    async def _build_python_package(self, workspace: Path, build_task) -> List[FileInfo]:
        """构建Python包"""
        artifacts = []
    ...
    async def _build_nodejs_package(self, workspace: Path, build_task) -> List[FileInfo]:
        """构建Node.js包"""
        artifacts = []
    ...
    async def _build_docker_package(self, workspace: Path, build_task) -> List[FileInfo]:
        """构建Docker包"""
        artifacts = []
        ...
        try:
            ...
            # 导出镜像
            image_file = workspace / f"{build_task.package_name}-{build_task.version}.tar"
        ...
        try:
            ...
            if report_file.exists():
                ...
                return TestResult(
                    ...
                    error_message=result.stderr.decode() if result.stderr and result.returncode != 0 else None
                )
    ...
    async def _install_dependencies(self, workspace: Path, build_task) -> None:
        ...
        if package_type == PackageType.ADAPTER:
            ...
            # Node.js依赖
            if any(dep['name'].startswith('@') or dep['name'] in ['react', 'vue', 'angular'] for dep in dependencies):
    ...
    # 其他辅助方法的占位符实现
    async def _prepare_docker_environment(self, workspace: Path, build_config: Dict) -> None:
        """准备Docker环境"""
        pass
    
    async def _install_python_dependencies(self, workspace: Path, dependencies: List[Dict]) -> None:
        """安装Python依赖"""
        pass
    
    async def _install_nodejs_dependencies(self, workspace: Path, dependencies: List[Dict]) -> None:
        """安装Node.js依赖"""
        pass
    ...
    async def _generate_package_formats(self, artifacts: List[FileInfo], target_formats: List[PackageFormat], workspace: Path) -> List[FileInfo]:
        """生成包格式"""
        return artifacts
    ...
    async def _run_single_test(self, workspace: Path, package, test_type: TestType, coverage_required: bool) -> TestResult:
        """运行单个测试"""
        pass
    ...
    def _merge_test_results(self, test_results: List[TestResult]) -> TestResult:
        """合并测试结果"""
        # 简单实现，返回第一个结果
        return test_results[0] if test_results else TestResult(
            test_type=TestType.UNIT,
            status="skipped",
            passed=0,
            failed=0,
    ...
    async def _build_gradle_package(self, workspace: Path, build_task) -> List[FileInfo]:
        """构建Gradle包"""
        return []
    ...
    async def _build_custom_package(self, workspace: Path, build_task) -> List[FileInfo]:
        """自定义构建"""
        return []
