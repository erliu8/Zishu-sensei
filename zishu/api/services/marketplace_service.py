...
from ..schemas.marketplace import (
    ...
    # 数据模型
    MoneyAmount, ProductBase, PricingInfo, VendorBase, Product, Vendor,
    Order, Payment, Review, Transaction, Campaign,
...
try:
    from ..models.marketplace import (
        Product as ProductModel, Vendor as VendorModel, Order as OrderModel,
        OrderItem as OrderItemModel, Payment as PaymentModel, Review as ReviewModel,
        Transaction as TransactionModel, Campaign as CampaignModel
    )
    HAS_DATABASE = True
except ImportError:
    # 提供占位符类
    class ProductModel: pass
    class VendorModel: pass
    class OrderModel: pass
    class OrderItemModel: pass
    class PaymentModel: pass
    class ReviewModel: pass
    class TransactionModel: pass
    class CampaignModel: pass
    HAS_DATABASE = False
...
class MarketplaceService:
    ...
    async def get_product_detail(
        ...
        """获取商品详情"""
        # 尝试从缓存获取
        cache_key = self._get_cache_key("product", product_id, str(user_id) if user_id else "anonymous")
        cached = await self._get_from_cache(cache_key)
        if cached:
            return ProductDetailResponse(**cached)
        
        # 从数据库获取
        product = await self._get_product_by_id(session, product_id, include_vendor=True)
        if not product:
            raise ProductNotFoundError(f"商品不存在: {product_id}")
    ...
    async def list_products(
        ...
        result = await session.execute(
            query.options(selectinload(ProductModel.vendor))
        )
    ...
    async def get_vendor_detail(
        ...
        """获取商家详情"""
        # 尝试从缓存获取
        cache_key = self._get_cache_key("vendor", vendor_id, str(user_id) if user_id else "anonymous")
        cached = await self._get_from_cache(cache_key)
        if cached:
        ...
        # 获取最近评价（基于商品评价）
        recent_reviews = await self._get_vendor_recent_reviews(session, vendor_id, limit=5)
            ...
            if await self._verify_payment_callback(payment, gateway_data):
                ...
                if order:
                    ...
                    for item in order.items:
                        product = await self._get_product_by_id(session, str(item.product_id))
                        if product:
    ...
    async def create_review(
        ...
        try:
            ...
            if request.order_id:
                order = await self._get_order_by_id(session, request.order_id)
                if order and order.buyer_id == user_id and order.status == OrderStatus.COMPLETED:
                    # 检查订单中是否包含此商品
                    for item in order.items:
            ...
            review_data['status'] = ReviewStatus.PUBLISHED if self.config.review_auto_publish else ReviewStatus.PENDING
    ...
    async def create_campaign(
        ...
        try:
            ...
            if 'discount_amount' in campaign_data and campaign_data['discount_amount']:
                campaign_data['discount_amount'] = campaign_data['discount_amount'].amount
                campaign_data['discount_currency'] = campaign_data['discount_amount'].currency
    ...
    async def get_marketplace_stats(
        ...
        stats = MarketplaceStats(
            ...
            revenue_this_week=MoneyAmount(amount=Decimal('0'), currency=CurrencyType.CNY),
            revenue_this_month=MoneyAmount(amount=Decimal('0'), currency=CurrencyType.CNY),
            category_stats={},
            vendor_type_stats={},
            top_products=[],
            top_vendors=[],
    ...
    async def get_vendor_stats(
        ...
        pending_orders_result = await session.execute(
            select(func.count(OrderModel.id)).where(
                OrderModel.vendor_id == uuid.UUID(vendor_id),
                OrderModel.status.in_([OrderStatus.PENDING, OrderStatus.CONFIRMED, OrderStatus.PAID])
            )
        )
        ...
        stats = VendorStats(
            ...
            total_revenue=MoneyAmount(amount=vendor.total_sales_amount, currency=vendor.total_sales_currency),
            this_month_revenue=MoneyAmount(amount=this_month_revenue, currency=CurrencyType.CNY),
    ...
    async def _get_product_by_id(
        ...
        """根据ID获取商品"""
        query = select(ProductModel).where(
            ProductModel.id == uuid.UUID(product_id),
            ProductModel.is_deleted == False
        )
    ...
    async def _process_refund(
        ...
        try:
            ...
            # 创建退款记录（这里简化处理）
            refund_transaction_id = f"REF{int(time.time())}{uuid.uuid4().hex[:8].upper()}"
