...
from ..schemas.auth import (
    LoginRequest, RegisterRequest, PasswordResetRequest, PasswordChangeRequest
)
...
class AuthService:
    ...
    async def reset_password_confirm(
        ...
        try:
            ...
            await self._update_user_password(
                user["id"], 
                password_hash,
                user.get("password_hash")  # 添加到密码历史
            )
    ...
    async def setup_two_factor(
        ...
        try:
            ...
            totp_uri = pyotp.totp.TOTP(secret_key).provisioning_uri(
                name=user["email"],
                issuer_name=self.config.security_2fa_issuer
            )
    ...
    async def _create_security_context(
        ...
        """创建安全上下文"""
        permissions = [
            PermissionType(perm) for perm in user.get("permissions", [PermissionType.USER.value])
        ]
    ...
    def _is_account_locked(self, username: str) -> bool:
        ...
        unlock_time = lock_time + timedelta(seconds=self.config.security_lockout_duration)
    ...
    async def _handle_failed_login(self, username: str, ip_address: str):
        ...
        # 检查是否需要锁定账户
        if len(self._failed_attempts[username]) >= self.config.security_max_login_attempts:
    ...
    async def _log_auth_event(
        ...
        try:
            ...
            # 这里应该保存到数据库或发送到审计系统
            logger.info(f"认证事件: {event_type.value} - {username} - {ip_address} - {metadata}")
    ...
    async def _update_user_password(
        ...
        user_id: str, 
        password_hash: str, 
        old_password_hash: Optional[str] = None
    ):
        """更新用户密码"""
        # TODO: 实现数据库更新
        # 需要更新密码哈希并维护密码历史
