...
from ..schemas.community import (
    ...
    CommentStatus, ReactionType, NotificationType, ModerationAction,
    
    # 数据模型
    CommunityInfo, CommunitySettings, CommunityStats, CommunityMember,
    PostInfo, PostContent, PostStats, CommentInfo, ReactionInfo,
    NotificationInfo, ModerationRecord, CommunityActivity,
    
    # 请求/响应模型
    CommunityCreateRequest, CommunityUpdateRequest, CommunitySearchRequest,
    PostCreateRequest, PostUpdateRequest, PostSearchRequest,
    CommentCreateRequest, CommentUpdateRequest, ReactionRequest,
    MembershipRequest, ModerationRequest, NotificationRequest,
    
    CommunityListResponse, CommunityDetailResponse, PostListResponse,
    PostDetailResponse, CommentListResponse, MemberListResponse,
    NotificationListResponse, StatisticsResponse
)

# 数据库模型导入
try:
    from ..models.community import (
        Community, CommunityMember as CommunityMemberModel, 
        Post, Comment, Reaction, CommunityNotification,
        ModerationLog, CommunityActivity as CommunityActivityModel
    )
    HAS_DATABASE = True
except ImportError:
    # 提供占位符类
    class Community: pass
    class CommunityMemberModel: pass
    class Post: pass
    class Comment: pass
    class Reaction: pass
    class CommunityNotification: pass
    class ModerationLog: pass
    class CommunityActivityModel: pass
    HAS_DATABASE = False
...
class CommunityService:
        ...
        try:
            ...
            if existing_community:
                return CommunityOperationResult(
                    success=False,
                    message="社区名称已存在",
                    error_code="COMMUNITY_NAME_EXISTS"
                )
    ...
    async def get_community(
        ...
        try:
            ...
            user_role = None
            if user_id:
                user_role = await self._get_user_role_in_community(community_id, user_id)
    ...
    async def update_community(
        ...
        try:
            # 检查权限
            if not await self._check_community_permission(community_id, user_id, "manage"):
                return CommunityOperationResult(
                    success=False,
                    message="没有权限修改社区信息",
            ...
            # 执行更新
            query = update(Community).where(Community.id == community_id).values(**update_data)
    ...
    async def delete_community(
        ...
        try:
            ...
            if not await self._check_community_permission(community_id, user_id, "delete"):
                return CommunityOperationResult(
                    success=False,
                    message="没有权限删除社区",
                    error_code="PERMISSION_DENIED"
            ...
            # 软删除：更新状态为已删除
            query = update(Community).where(Community.id == community_id).values(
    ...
    async def search_communities(
        ...
        try:
            ...
            if request.sort_by == "created_at":
                query = query.order_by(desc(Community.created_at) if request.sort_desc else asc(Community.created_at))
            elif request.sort_by == "updated_at":
                query = query.order_by(desc(Community.updated_at) if request.sort_desc else asc(Community.updated_at))
            elif request.sort_by == "name":
                query = query.order_by(desc(Community.name) if request.sort_desc else asc(Community.name))
            else:
    ...
    async def leave_community(
        ...
        try:
            ...
            await self._log_community_activity(
                ...
                user_id=user_id,
                action="member_left",
                metadata={"role": member.role.value if hasattr(member, 'role') else "member"}
            )
    ...
    async def update_post(
        ...
        try:
            ...
            # 检查权限（作者或管理员可以编辑）
            can_edit = (post.author_id == user_id or 
                       await self._check_community_permission(post.community_id, user_id, "moderate"))
    ...
    async def delete_post(
        ...
        try:
            ...
            # 检查权限
            can_delete = (post.author_id == user_id or 
                         await self._check_community_permission(post.community_id, user_id, "moderate"))
            ...
            # 软删除
            update_query = update(Post).where(Post.id == post_id).values(
    ...
    async def search_posts(
        ...
        try:
            ...
            if request.community_id:
                # 检查社区访问权限
                if not await self._check_community_access(request.community_id, user_id):
                    return PostListResponse(
                        posts=[],
                        total=0,
                        page=request.page,
            ...
            # 排序
            if request.sort_by == "created_at":
                query = query.order_by(desc(Post.created_at) if request.sort_desc else asc(Post.created_at))
            elif request.sort_by == "updated_at":
                query = query.order_by(desc(Post.updated_at) if request.sort_desc else asc(Post.updated_at))
            elif request.sort_by == "title":
                query = query.order_by(desc(Post.title) if request.sort_desc else asc(Post.title))
            else:
                # 默认按创建时间排序，置顶帖子优先
    ...
    async def create_comment(
        ...
        try:
            ...
            # 检查帖子是否被锁定
            if post.is_locked:
                can_comment = await self._check_community_permission(post.community_id, author_id, "moderate")
                if not can_comment:
                    return CommunityOperationResult(
                        success=False,
                        message="帖子已被锁定，无法评论",
                        error_code="POST_LOCKED"
                    )
            ...
            # 自动内容审核
            if self.auto_moderation_enabled:
                await self._auto_moderate_content(comment_id, "comment", request.content)
    ...
    async def get_comments(
        ...
        try:
            ...
            for comment in comments:
                ...
                user_reaction = None
                if user_id:
                    user_reaction = await self._get_user_comment_reaction(comment.id, user_id)
    ...
    async def add_reaction(
        ...
        try:
            ...
            # 检查目标内容是否存在
            target_exists = await self._check_target_exists(request.target_id, request.target_type)
            ...
            if existing_reaction:
                if existing_reaction.type == request.type:
                    ...
                    # 更新反应类型
                    update_query = update(Reaction).where(
                        and_(
                            Reaction.user_id == user_id,
                            Reaction.target_id == request.target_id,
                            Reaction.target_type == request.target_type
            ...
            # 获取社区ID用于记录活动
            community_id = await self._get_community_id_by_target(request.target_id, request.target_type)
            if community_id:
                await self._log_community_activity(
                    community_id=community_id,
                    user_id=user_id,
            ...
            return CommunityOperationResult(
                success=False,
                message=f"反应操作失败: {str(e)}",
                error_code="REACTION_FAILED"
            )
    ...
    async def _check_community_access(self, community_id: str, user_id: Optional[str]) -> bool:
        """检查社区访问权限"""
        # 这里可以实现复杂的权限逻辑
    ...
    async def _check_community_permission(self, community_id: str, user_id: str, action: str) -> bool:
        """检查社区操作权限"""
        member = await self._get_community_member(community_id, user_id)
        if not member:
            return False
    ...
    async def _get_community_member(self, community_id: str, user_id: str) -> Optional[CommunityMemberModel]:
        """获取社区成员信息"""
        if not HAS_DATABASE or not self.db_session:
            return None
    ...
    async def _get_user_role_in_community(self, community_id: str, user_id: str) -> Optional[MemberRole]:
        """获取用户在社区中的角色"""
        member = await self._get_community_member(community_id, user_id)
    ...
    async def _get_community_stats(self, community_id: str) -> CommunityStats:
        ...
        try:
            ...
            comment_query = select(func.count()).where(
                Post.community_id == community_id
            ).join(Comment, Comment.post_id == Post.id).where(
                Comment.status == CommentStatus.PUBLISHED
            )
    ...
    async def _get_post_stats(self, post_id: str) -> PostStats:
        ...
        try:
            ...
            reaction_counts = {str(reaction_type): count for reaction_type, count in reaction_result.fetchall()}
    ...
    async def _get_user_reaction(self, target_id: str, user_id: str) -> Optional[ReactionType]:
        """获取用户对帖子的反应"""
        if not HAS_DATABASE or not self.db_session:
            return None
    ...
    async def _get_user_comment_reaction(self, comment_id: str, user_id: str) -> Optional[ReactionType]:
        """获取用户对评论的反应"""
        if not HAS_DATABASE or not self.db_session:
            return None
    ...
    async def _get_user_target_reaction(self, user_id: str, target_id: str, target_type: str) -> Optional[Reaction]:
        """获取用户对目标的反应"""
        if not HAS_DATABASE or not self.db_session:
            return None
    ...
    async def _get_community_id_by_target(self, target_id: str, target_type: str) -> Optional[str]:
        """根据目标获取社区ID"""
        if not HAS_DATABASE or not self.db_session:
            return None
        ...
        try:
            if target_type == "post":
                ...
                query = select(Post.community_id).join(Comment, Comment.post_id == Post.id).where(Comment.id == target_id)
                result = await self.db_session.execute(query)
                return result.scalar_one_or_none()
    ...
    async def get_community_analytics(
        ...
        try:
            ...
            # 时间范围内的活动统计
            activity_stats = await self._get_community_activity_stats(community_id, start_date, end_date)
            
            # 用户参与度统计
            engagement_stats = await self._get_community_engagement_stats(community_id, start_date, end_date)
            
            # 内容统计
            content_stats = await self._get_community_content_stats(community_id, start_date, end_date)
    ...
    async def _get_community_activity_stats(
        ...
        try:
            ...
            action_stats_query = select(
                CommunityActivityModel.action,
                func.count(CommunityActivityModel.id)
            ).where(
                and_(
                    CommunityActivityModel.community_id == community_id,
                    CommunityActivityModel.created_at >= start_date,
    ...
    async def _get_community_engagement_stats(
        ...
        try:
            ...
            # 新成员统计
            new_members_query = select(
                func.count(CommunityMemberModel.id)
            ).where(
                and_(
                    CommunityMemberModel.community_id == community_id,
            ...
            ).group_by(CommunityActivityModel.user_id).order_by(
                func.count(CommunityActivityModel.id).desc()
            ).limit(10)
    ...
    async def _get_community_content_stats(
        ...
        try:
            ...
            posts_by_type = {str(post_type): count for count, post_type in posts_result.fetchall()}
            
            # 评论统计
            comments_query = select(
                func.count(Comment.id)
            ).join(Post, Comment.post_id == Post.id).where(
                and_(
                    Post.community_id == community_id,
            ...
            reactions_query = select(
                Reaction.type,
                func.count(Reaction.id)
            ).join(Post, Reaction.target_id == Post.id).where(
                and_(
                    Post.community_id == community_id,
                    Reaction.target_type == "post",
            ...
            reactions_by_type = {str(reaction_type): count for reaction_type, count in reactions_result.fetchall()}
    ...
    async def moderate_content(
        ...
        try:
            # 检查审核权限
            community_id = await self._get_community_id_by_target(request.content_id, request.content_type)
            if not community_id:
            ...
            if not await self._check_community_permission(community_id, moderator_id, "moderate"):
                return ContentModerationResult(
                    content_id=request.content_id,
                    content_type=request.content_type,
                    action=ModerationAction.NO_ACTION,
    ...
    async def _hide_content(self, content_id: str, content_type: str):
        ...
        if content_type == "post":
            query = update(Post).where(Post.id == content_id).values(
                status=PostStatus.HIDDEN,
                updated_at=datetime.now(timezone.utc)
            )
        elif content_type == "comment":
            query = update(Comment).where(Comment.id == content_id).values(
    ...
    async def get_user_notifications(
        ...
        try:
            ...
            if community_id:
                unread_query = unread_query.where(CommunityNotification.community_id == community_id)
    ...
    async def mark_notification_read(
        ...
        try:
            ...
            # 更新通知状态
            query = update(CommunityNotification).where(
                and_(
    ...
    async def _auto_moderate_content(self, content_id: str, content_type: str, content: str):
        """自动内容审核"""
        # 这里可以实现自动内容审核逻辑
        # 例如：敏感词检测、垃圾内容检测等
        try:
            ...
            for word in sensitive_words:
                if word in content:
                    ...
                    # 创建审核记录
                    community_id = await self._get_community_id_by_target(content_id, content_type)
                    if community_id:
                        moderation_log = ModerationLog(
                            id=str(uuid.uuid4()),
                            community_id=community_id,
