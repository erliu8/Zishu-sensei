...
# 项目内部导入
from ..models.packaging import PackageModel, BuildTaskModel, TestResultModel, PublishResultModel
from ..schemas.packaging import (
    PackageCreateRequest, PackageUpdateRequest, PackageDetailResponse,
    PackageListItem, BuildRequest, BuildTaskResponse, TestRequest,
    TestResultResponse, PublishRequest, PublishResultResponse,
    SignRequest, PackageStatus, BuildStatus, PackageType,
    VersionInfo, FileInfo, DependencyInfo, SignatureInfo,
    PaginatedResponse, SearchFilters, PackageStatistics
)
from ..core.database import get_db_session
from ..core.config import settings
from ..core.security import get_current_user
from ..core.logging import get_logger
from ..utils.file_utils import calculate_file_hash, validate_file_type, compress_directory
...
class PackageConfig:
    ...
    def __post_init__(self):
        if self.allowed_formats is None:
            self.allowed_formats = {'.py', '.js', '.ts', '.json', '.yaml', '.yml', '.md', '.txt', '.cfg', '.ini'}
...
class PackagingService:
    ...
    async def create_package(
        ...
        try:
            ...
            package = PackageModel(
                ...
                homepage=str(request.metadata.homepage) if request.metadata.homepage else None,
                repository=str(request.metadata.repository) if request.metadata.repository else None,
                documentation=str(request.metadata.documentation) if request.metadata.documentation else None,
        ...
        try:
            ...
            if request.metadata:
                ...
                if request.metadata.name != package.name:
                    await self._validate_package_name_unique(request.metadata.name, db, exclude_id=package_id)
            ...
            if request.dependencies is not None:
                update_data['dependencies'] = [dep.dict() for dep in request.dependencies]
    ...
    async def list_packages(
        ...
        try:
            ...
            if user_id:
                conditions.append(
                    or_(
                        PackageModel.status.in_([PackageStatus.PUBLISHED, PackageStatus.DEPRECATED]),
            ...
            # 分页查询
            offset = (page - 1) * size
            query = query.offset(offset).limit(size).order_by(PackageModel.updated_at.desc())
    ...
    async def build_package(
        ...
        try:
            ...
            # 异步启动构建任务
            asyncio.create_task(
                self._execute_build_task(build_task.id, db)
            )
    ...
    async def test_package(
        ...
        try:
            ...
            test_record = TestResultModel(
                ...
                reports=[FileInfo(**report).dict() for report in test_result.get('reports', [])],
                created_by=user_id
            )
    ...
    async def get_package_statistics(
        ...
        try:
            ...
            total_downloads_result = await db.execute(
                select(func.sum(PackageModel.download_count))
                .select_from(base_query.subquery())
            )
            ...
            build_stats_query = select(
                ...
                func.sum(case((BuildTaskModel.status == BuildStatus.SUCCESS, 1), else_=0)).label('successful_builds'),
                func.sum(case((BuildTaskModel.status == BuildStatus.FAILED, 1), else_=0)).label('failed_builds'),
            ...
            test_stats_query = select(
                ...
                func.avg(
                    case(
                        ((TestResultModel.passed + TestResultModel.failed + TestResultModel.skipped) > 0,
                         TestResultModel.passed * 100.0 / 
                         (TestResultModel.passed + TestResultModel.failed + TestResultModel.skipped)),
                        else_=0
                    )
    ...
    async def _get_package_by_id(self, package_id: str, db: AsyncSession) -> PackageModel:
        """根据ID获取包"""
    ...
    async def _check_package_permission(self, package: PackageModel, user_id: str) -> None:
        """检查包权限"""
    ...
    async def _package_to_detail_response(self, package: PackageModel) -> PackageDetailResponse:
        """包模型转详情响应"""
        # 获取版本列表
        ...
        return PackageDetailResponse(
            ...
            test_config=TestConfiguration(**package.test_config) if package.test_config else None,
    ...
    async def _trigger_auto_build(self, package_id: str, user_id: str, db: AsyncSession) -> None:
        """触发自动构建"""
        try:
        ...
        try:
            ...
            await db.execute(
                ...
                .values(
                    ...
                    progress=100.0 if build_result['status'] == BuildStatus.SUCCESS else build_result.get('progress', 0.0),
                    logs=build_result.get('logs', []),
                    artifacts=[FileInfo(**artifact).dict() for artifact in build_result.get('artifacts', [])],
    ...
    async def _validate_publish_conditions(self, package: PackageModel, version: str) -> None:
        """验证发布条件"""
        # TODO: 实现发布条件验证
        pass
    ...
    async def _build_task_to_response(self, build_task: BuildTaskModel) -> BuildTaskResponse:
        """构建任务转响应"""
        # TODO: 实现转换逻辑
        pass
    
    async def _test_result_to_response(self, test_result: TestResultModel) -> TestResultResponse:
        """测试结果转响应"""
        # TODO: 实现转换逻辑
        pass
    
    async def _publish_result_to_response(self, publish_result: PublishResultModel) -> PublishResultResponse:
        """发布结果转响应"""
        # TODO: 实现转换逻辑
        pass
