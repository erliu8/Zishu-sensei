...
class FileSecurityInfo(BaseModel):
    """文件安全信息模型"""
    access_level: FileAccessLevel = Field(default=FileAccessLevel.PRIVATE, description="访问级别")
...
class FileVersion(BaseModel):
    ...
    change_type: Literal["major", "minor", "patch"] = Field(default="minor", description="变更类型")
...
class FileInfo(FileBase):
    ...
    security: FileSecurityInfo = Field(default_factory=FileSecurityInfo, description="安全信息")
...
class FileUploadRequest(BaseModel):
    """文件上传请求模型"""
    filename: str = Field(..., min_length=1, max_length=255, description="文件名")
    size: int = Field(..., ge=1, le=5*1024*1024*1024, description="文件大小(字节,最大5GB)")
    mime_type: Optional[str] = Field(None, description="MIME类型")
    
    # 存储选项
    folder_id: Optional[str] = Field(None, description="目标文件夹ID")
    storage_provider: Optional[StorageProvider] = Field(StorageProvider.LOCAL, description="存储提供商")
...
class FileShareRequest(BaseModel):
    ...
    share_type: Literal["public", "private", "password"] = Field(..., description="分享类型")
...
class FileInfoResponse(BaseModel):
    ...
    # 权限信息
    user_permissions: List[FileOperation] = Field(default_factory=list, description="用户权限")
...
class FileActivityResponse(BaseModel):
    ...
    recent_activities: List[FileActivity] = Field(default_factory=list, description="最近活动")
...
class FileStatisticsResponse(BaseModel):
    ...
    # 用户统计
    top_uploaders: List[Dict[str, Any]] = Field(default_factory=list, description="上传用户排行")
    top_downloaders: List[Dict[str, Any]] = Field(default_factory=list, description="下载用户排行")
    
    # 存储统计
    storage_usage: Dict[str, int] = Field(default_factory=dict, description="存储使用情况")
    storage_by_provider: Dict[str, int] = Field(default_factory=dict, description="按存储商统计")
...
class FileValidator:
    ...
    ALLOWED_EXTENSIONS = {
        ...
        FileType.CODE: ['.py', '.js', '.html', '.css', '.java', '.cpp', '.php', '.go', '.rs']
    }
    
    # 危险文件扩展名
    DANGEROUS_EXTENSIONS = [
        '.exe', '.bat', '.cmd', '.scr', '.pif', '.com', '.vbs', '.js', '.jar',
        '.ps1', '.sh', '.app', '.deb', '.rpm', '.dmg', '.pkg'
    ]
...
class FileHelper:
    ...
    @staticmethod
    def generate_thumbnail(file_path: str, output_path: str, size: tuple = (200, 200)) -> bool:
        """生成缩略图"""
...
class FileStorageConfig(BaseModel):
    """文件存储配置模型"""
    # 默认存储提供商
    default_provider: StorageProvider = Field(default=StorageProvider.LOCAL, description="默认存储提供商")
    
    # 存储提供商配置
    providers: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="存储提供商配置")
    ...
    backup_providers: List[StorageProvider] = Field(default_factory=list, description="备份存储提供商")
