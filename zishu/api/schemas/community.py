...
class CommunityBase(BaseModel):
    ...
    community_type: CommunityType = Field(default=CommunityType.PUBLIC, description="社区类型")
...
class Follow(BaseModel):
    ...
    target_type: Literal["user", "community", "adapter"] = Field(..., description="关注目标类型")
...
class Report(BaseModel):
    ...
    target_type: Literal["post", "comment", "user", "community"] = Field(..., description="举报目标类型")
    ...
    status: Literal["pending", "resolved", "rejected"] = Field(default="pending", description="处理状态")
...
class PostUpdateRequest(BaseModel):
    ...
    content: Optional[str] = Field(None, min_length=1, max_length=50000, description="帖子内容")
...
class CommentUpdateRequest(BaseModel):
    """更新评论请求"""
    content: Optional[str] = Field(None, min_length=1, max_length=10000, description="评论内容")
    attachments: Optional[List[str]] = Field(None, description="附件列表")
    metadata: Optional[Dict[str, Any]] = Field(None, description="元数据")
...
class AdapterUpdateRequest(BaseModel):
    ...
    description: Optional[str] = Field(None, min_length=10, max_length=1000, description="适配器描述")
...
class CommentDetailResponse(Comment):
    """评论详情响应"""
    author: UserProfileDetail = Field(..., description="作者信息")
    replies: List["CommentDetailResponse"] = Field(default_factory=list, description="回复列表")
    reactions: Dict[str, int] = Field(default_factory=dict, description="反应统计")
    user_reaction: Optional[ReactionType] = Field(None, description="用户反应")
...
class AdapterDetailResponse(Adapter):
    ...
    recent_reviews: List[AdapterReview] = Field(default_factory=list, description="最近评论")
...
class CommunityStats(BaseModel):
    ...
    top_contributors: List[Dict[str, Any]] = Field(default_factory=list, description="贡献者排行")
...
class AdapterStats(BaseModel):
    ...
    rating_distribution: Dict[str, int] = Field(default_factory=dict, description="评分分布")
    version_distribution: Dict[str, int] = Field(default_factory=dict, description="版本分布")
...
def validate_community_permissions(user_role: MemberRole, required_role: MemberRole) -> bool:
    """验证社区权限"""
    role_hierarchy = {
        MemberRole.GUEST: 0,
        MemberRole.MEMBER: 1,
...
__all__ = [
    ...
    # 基础模型
    "CommunityBase", "PostBase", "CommentBase", "AdapterBase",
    
    # 完整模型
    "Community", "Post", "Comment", "CommunityMember", "Reaction", "Follow",
    "Report", "Adapter", "AdapterReview", "Notification",
    
    # 请求模型
    "CommunityCreateRequest", "CommunityUpdateRequest", "PostCreateRequest",
    "PostUpdateRequest", "CommentCreateRequest", "CommentUpdateRequest",
    "CommunityJoinRequest", "CommunityInviteRequest", "ReactionCreateRequest",
    "FollowCreateRequest", "ReportCreateRequest", "AdapterCreateRequest",
    "AdapterUpdateRequest", "AdapterReviewCreateRequest",
    
    # 响应模型
    "CommunityListItem", "PostListItem", "CommentListItem", "AdapterListItem",
    "CommunityDetailResponse", "PostDetailResponse", "CommentDetailResponse",
    "AdapterDetailResponse", "PaginatedResponse", "SearchFilters",
    
    # 统计模型
    "CommunityStats", "AdapterStats",
