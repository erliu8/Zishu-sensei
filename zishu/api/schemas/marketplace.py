...
class Vendor(VendorBase):
    ...
    total_sales: MoneyAmount = Field(
        default_factory=lambda: MoneyAmount(amount=Decimal("0")), description="总销售额"
    )
    ...
    # 财务信息
    balance: MoneyAmount = Field(
        default_factory=lambda: MoneyAmount(amount=Decimal("0")), description="账户余额"
    )
...
class Order(BaseModel):
    ...
    discount_amount: MoneyAmount = Field(
        default_factory=lambda: MoneyAmount(amount=Decimal("0")), description="折扣金额"
    )
    tax_amount: MoneyAmount = Field(
        default_factory=lambda: MoneyAmount(amount=Decimal("0")), description="税费"
    )
    ...
    payment_status: PaymentStatus = Field(
        default=PaymentStatus.PENDING, description="支付状态"
    )
...
class Campaign(BaseModel):
    ...
    discount_percentage: Optional[float] = Field(
        None, ge=0.0, le=100.0, description="折扣百分比"
    )
...
class ProductUpdateRequest(BaseModel):
    ...
    description: Optional[str] = Field(
        None, min_length=10, max_length=5000, description="商品描述"
    )
    short_description: Optional[str] = Field(None, max_length=500, description="商品简介")
    category: Optional[str] = Field(
        None, min_length=1, max_length=100, description="商品分类"
    )
    subcategory: Optional[str] = Field(None, max_length=100, description="商品子分类")
...
class VendorUpdateRequest(BaseModel):
    ...
    display_name: Optional[str] = Field(
        None, min_length=2, max_length=200, description="显示名称"
    )
    description: Optional[str] = Field(None, max_length=2000, description="商家描述")
...
class ProductDetailResponse(Product):
    ...
    recent_reviews: List["ReviewListItem"] = Field(
        default_factory=list, description="最近评价"
    )
    related_products: List["ProductListItem"] = Field(
        default_factory=list, description="相关商品"
    )
...
class OrderDetailResponse(Order):
    ...
    product_details: List[Dict[str, Any]] = Field(
        default_factory=list, description="商品详情"
    )
    payment_history: List["PaymentListItem"] = Field(
        default_factory=list, description="支付记录"
    )
...
class MarketplaceStats(BaseModel):
    ...
    vendor_type_stats: Dict[str, int] = Field(
        default_factory=dict, description="商家类型统计"
    )
    ...
    top_categories: List[Dict[str, Any]] = Field(
        default_factory=list, description="热门分类"
    )
...
def calculate_discount_amount(
    base_price: MoneyAmount, discount_percentage: float
) -> MoneyAmount:
    """计算折扣金额"""
...
def is_campaign_active(campaign: Campaign) -> bool:
    ...
    return (
        campaign.is_active
        and campaign.start_time <= now <= campaign.end_time
        and (
...
def get_product_final_price(
    product: Product, campaigns: List[Campaign] = None
) -> MoneyAmount:
    """获取商品最终价格（考虑活动折扣）"""
    ...
    for campaign in campaigns:
        ...
        if (
            product.id in campaign.applicable_products
            or product.category in campaign.applicable_categories
            or product.vendor_id in campaign.applicable_vendors
        ):
            if campaign.discount_percentage:
                discount = base_price.amount * Decimal(
                    str(campaign.discount_percentage / 100)
                )
