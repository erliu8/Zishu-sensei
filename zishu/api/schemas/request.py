...
class ChatCompletionRequest(BaseModel):
    ...
    messages: List[Union[Dict[str, Any], Message]] = Field(
        ..., min_length=1, description="对话消息列表"
    )
    model: Optional[str] = Field(default="zishu-base", description="模型ID")

    # 生成参数
    max_tokens: Optional[int] = Field(
        default=2048, ge=1, le=8192, description="最大token数量"
    )
    temperature: Optional[float] = Field(
        default=0.7, ge=0.0, le=2.0, description="温度参数"
    )
    top_p: Optional[float] = Field(
        default=0.9, ge=0.0, le=1.0, description="top-p核采样参数"
    )
    ...
    repetition_penalty: Optional[float] = Field(
        default=1.1, ge=0.1, le=2.0, description="重复惩罚参数"
    )
...
class StreamChatRequest(BaseModel):
    ...
    chunk_size: Optional[int] = Field(
        default=1, ge=1, le=10, description="每次返回的token数量"
    )
    delay_ms: Optional[int] = Field(
        default=10, ge=0, le=100, description="流式返回延迟时间(毫秒)"
    )
...
class EmotionChatRequest(BaseModel):
    ...
    current_emotion: Optional[EmotionType] = Field(
        default="neutral", description="当前情绪状态"
    )
    target_emotion: Optional[EmotionType] = Field(None, description="目标情绪状态")  # 可从记忆中获取
    emotion_intensity: Optional[float] = Field(
        default=0.5, ge=0.0, le=1.0, description="情绪强度"
    )
...
class ModelLoadRequest(BaseModel):
    ...
    torch_dtype: Optional[str] = Field(
        default="auto", description="torch数据类型"
...
class CharacterConfigRequest(BaseModel):
    ...
    emotion_transitions: Optional[Dict[str, List[str]]] = Field(
        None, description="情绪转换规则"
    )
...
class CharacterEmotionRequest(BaseModel):
    ...
    duration: Optional[int] = Field(
        None, ge=1, description="情绪持续时间(秒)"
    )  # TODO: 需要根据情绪强度计算，可用更精确的时间
    transition_time: Optional[float] = Field(
        default=1.0, ge=0.1, le=10.0, description="情绪过渡时间(秒)"
    )
...
class AnimationGenerationRequest(BaseModel):
    ...
    duration: Optional[float] = Field(
        default=3.0, ge=0.5, le=30.0, description="动画持续时间(秒)"
    )
...
class BatchChatRequest(BaseModel):
    requests: List[ChatCompletionRequest] = Field(
        ..., min_length=1, max_length=10, description="批量请求列表"
    )
