...
from .auth import (
    ...
    LoginMethod, SessionStatus, BaseUserModel, UserProfile as AuthUserProfile,
...
class UserProfileDetail(BaseModel):
    ...
    # 隐私设置
    profile_visibility: ProfileVisibility = Field(default=ProfileVisibility.PUBLIC, description="档案可见性")
    ...
    # AI偏好设置
    preferred_character: Optional[str] = Field(None, max_length=100, description="偏好的AI角色")
...
class UserInfo(UserBase):
    """完整用户信息模型"""
    profile: UserProfileDetail = Field(default_factory=UserProfileDetail, description="用户档案")
    security: Optional[UserSecurityInfo] = Field(None, description="安全信息")
    statistics: UserStatistics = Field(default_factory=UserStatistics, description="统计信息")
...
class UserNotificationUpdateRequest(BaseModel):
    ...
    # 具体通知类型
    notification_preferences: Optional[Dict[str, bool]] = Field(None, description="具体通知偏好")
...
class UserBatchOperationRequest(BaseModel):
    ...
    operation: Literal[
        ...
        "export", "send_notification", "update_role", 
...
class SystemUserStatsResponse(BaseModel):
    ...
    activity_trend: List[Dict[str, Any]] = Field(default_factory=list, description="活跃趋势")
    
    timestamp: datetime = Field(default_factory=datetime.now, description="统计时间")

# ======================== 工具函数 ========================

def validate_user_permissions(user_role: UserRole, required_permissions: List[str]) -> bool:
    """验证用户权限"""
    # 从认证模块导入权限配置
...
def calculate_user_score(user: UserInfo) -> Dict[str, float]:
    ...
    # 档案完整度评分
    profile_fields = [
        user.profile.nickname, user.profile.bio, user.profile.avatar_url,
        user.profile.location, user.profile.birthday
    ]
    ...
    if user.statistics.total_messages > 0:
        scores["engagement"] = min(1.0, user.statistics.total_messages / 100)  # 100条消息为满分
...
def create_user_activity_log(user_id: str, action: str, details: Dict[str, Any]) -> Dict[str, Any]:
    """创建用户活动日志"""
    return {
        "id": str(uuid.uuid4()),
        "user_id": user_id,
...
USERNAME_ADJECTIVES = [
    "cool", "smart", "happy", "lucky", "super", "great", "awesome", "best"
]
