...
class UserProfile(BaseModel):
    ...
    profile_visibility: Literal["public", "friends", "private"] = Field(
        default="public", description="档案可见性"
    )
...
class UserSecurity(BaseModel):
    ...
    password_updated_at: datetime = Field(
        default_factory=datetime.now, description="密码更新时间"
    )
    ...
    # 安全问题
    security_questions: List[Dict[str, str]] = Field(
        default_factory=list, description="安全问题"
    )
...
class Permission(BaseModel):
    ...
    permission_id: str = Field(
        default_factory=lambda: str(uuid.uuid4()), description="权限ID"
    )
...
class LoginRequest(BaseModel):
    ...
    def validate_login_credentials(self):
        ...
        if self.login_method == LoginMethod.PASSWORD and not self.password:
            ...
            self.login_method in [LoginMethod.SMS_CODE, LoginMethod.EMAIL_CODE]
            and not self.verification_code
        ):
            raise ValueError("验证码登录需要提供验证码")
        elif self.login_method == LoginMethod.OAUTH and not all(
            [self.provider, self.provider_token]
        ):
...
class VerificationRequest(BaseModel):
    ...
    purpose: Literal["registration", "login", "reset_password", "change_info"] = Field(
        ..., description="验证目的"
    )
...
class UserUpdateRequest(BaseModel):
    ...
    # 隐私设置
    profile_visibility: Optional[Literal["public", "friends", "private"]] = Field(
        None, description="档案可见性"
    )
...
DEFAULT_ROLE_PERMISSIONS = {
    UserRole.USER: RolePermissions(
        role=UserRole.USER,
        permissions=["chat:basic", "profile:read", "profile:update", "session:manage"],
