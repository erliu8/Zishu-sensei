...
class AdapterCompatibility(BaseModel):
    ...
    framework_versions: Dict[str, str] = Field(
        default_factory=dict, description="框架版本要求"
    )
...
class AdapterMetadata(BaseModel):
    ...
    capabilities: List[AdapterCapability] = Field(
        default_factory=list, description="适配器能力"
    )
    compatibility: AdapterCompatibility = Field(
        default_factory=AdapterCompatibility, description="兼容性信息"
    )
    resource_requirements: AdapterResourceRequirements = Field(
        default_factory=AdapterResourceRequirements, description="资源需求"
    )
...
class AdapterConfig(BaseModel):
    ...
    # 软适配器特定配置
    soft_adapter_config: Optional[SoftAdapterConfig] = Field(
        None, description="软适配器配置(Prompt + RAG)"
    )

    # 硬适配器特定配置
    hard_adapter_config: Optional[HardAdapterConfig] = Field(
        None, description="硬适配器配置(原生代码)"
    )

    # 智能适配器特定配置
    intelligent_adapter_config: Optional[IntelligentAdapterConfig] = Field(
        None, description="智能适配器配置(自适应组合)"
    )

    # 通用特定适配器配置
    adapter_specific_config: Dict[str, Any] = Field(
        default_factory=dict, description="适配器特定配置"
    )
...
class PTuningV2Config(BaseModel):
    ...
    num_transformer_submodules: Optional[PositiveInt] = Field(
        None, description="Transformer子模块数量"
    )
...
class AdapterOperationResponse(BaseModel):
    ...
    gpu_memory_usage_mb: Optional[PositiveFloat] = Field(
        None, description="GPU内存使用(MB)"
    )
...
class AdapterMonitoringData(BaseModel):
    ...
    gpu_usage_percent: Optional[float] = Field(
        None, description="GPU使用率", ge=0.0, le=100.0
    )
...
def create_adapter_metadata(
    ...
    return AdapterMetadata(
        id=adapter_id, name=name, version=version, adapter_type=adapter_type, **kwargs
    )


def create_default_adapter_config(
    adapter_type: AdapterType, **overrides
) -> AdapterConfig:
    """创建默认适配器配置"""
...
def validate_adapter_compatibility(
    ...
    if framework_versions and adapter_metadata.compatibility.framework_versions:
        for (
            framework,
            required_version,
        ) in adapter_metadata.compatibility.framework_versions.items():
            if framework in framework_versions:
                ...
                if not _compare_versions(current_version, required_version):
                    warnings.append(
                        f"框架 {framework} 版本可能不兼容: 当前 {current_version}, 需要 {required_version}"
                    )
...
def _compare_versions(current: str, required: str) -> bool:
    ...
    try:
        current_parts = [
            int(x) for x in current.replace(">=", "").replace(">", "").split(".")
        ]
        required_parts = [
            int(x) for x in required.replace(">=", "").replace(">", "").split(".")
        ]
...
def merge_adapter_configs(
    base_config: AdapterConfig, override_config: Dict[str, Any]
) -> AdapterConfig:
    """合并适配器配置"""
    # 创建基础配置的副本
...
def create_adapter_operation(
    adapter_id: str, operation_type: str, **kwargs
) -> AdapterOperation:
    """创建适配器操作记录"""
    import uuid
...
def validate_adapter_metadata(metadata: AdapterMetadata) -> AdapterValidationResult:
    ...
    # 资源需求验证
    if (
        metadata.resource_requirements.min_memory_mb
        and metadata.resource_requirements.recommended_memory_mb
    ):
        if (
            metadata.resource_requirements.min_memory_mb
            > metadata.resource_requirements.recommended_memory_mb
        ):
...
def validate_adapter_config(
    config: AdapterConfig, adapter_type: AdapterType
) -> AdapterValidationResult:
    """验证适配器配置"""
    errors = []
    warnings = []
...
def _validate_prefix_tuning_config(
    config: Dict[str, Any], errors: List[str], warnings: List[str]
):
    """验证Prefix Tuning配置"""
...
def _validate_prompt_tuning_config(
    config: Dict[str, Any], errors: List[str], warnings: List[str]
):
    """验证Prompt Tuning配置"""
    if "num_virtual_tokens" in config:
...
def create_error_response(
    error: Exception, operation_id: str = None, adapter_id: str = None
) -> Dict[str, Any]:
    """创建错误响应"""
    import traceback
    import uuid
