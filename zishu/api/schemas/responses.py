...
class BaseResponse(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(), description="响应时间戳"
    )
    request_id: str = Field(
        default_factory=lambda: str(uuid.uuid4()), description="请求ID"
    )
...
class Choice(BaseModel):
    ...
    confidence: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="生成置信度"
    )  # TODO: 需要根据生成质量计算
    safety_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="安全得分"
    )  # TODO: 需要根据生成内容计算
...
class AdapterInfo(BaseModel):
    ...
    # 状态信息
    status: Literal["loaded", "unloaded", "loading", "error"] = Field(
        ..., description="适配器状态"
    )
...
class BatchResponse(BaseModel):
    ...
    # 详细结果
    results: List[Union[ChatCompletionResponse, "ErrorResponse"]] = Field(
        ..., description="批量结果"
    )
...
class ConversationSummary(BaseModel):
    ...
    id: str = Field(
        default_factory=lambda: str(__import__("uuid").uuid4()), description="摘要ID"
    )
    ...
    created: datetime = Field(
        default_factory=__import__("datetime").datetime.now, description="创建时间"
    )
...
class ChatHistory(BaseModel):
    ...
    session_id: str = Field(
        default_factory=lambda: str(__import__("uuid").uuid4()), description="会话ID"
    )
    conversation_id: str = Field(..., description="对话ID")
