...
class CharacterConfig(BaseModel):
    ...
    available_emotions: List[EmotionValue] = Field(
        default_factory=lambda: [e.value for e in EmotionType], description="可用情绪列表"
    )
    default_emotion: EmotionValue = Field(default="neutral", description="默认情绪")
    emotion_transitions: Dict[str, List[str]] = Field(
        default_factory=dict, description="情绪转换规则"
    )
    ...
    language_preferences: List[str] = Field(
        default_factory=lambda: ["zh-CN"], description="语言偏好"
    )
    ...
    personalization_level: float = Field(
        default=0.5, ge=0.0, le=1.0, description="个性化程度"
    )

    # 安全和限制
    content_filter_level: float = Field(
        default=0.7, ge=0.0, le=1.0, description="内容过滤级别"
    )
    max_response_length: int = Field(
        default=2000, ge=100, le=10000, description="最大回复长度"
    )
    ...
    template: CharacterTemplate = Field(
        default=CharacterTemplate.ZISHU_BASE, description="基于的模板"
    )
    ...
    emotion_history: List[Dict[str, Any]] = Field(
        default_factory=list, description="情绪历史"
    )
...
class CharacterState(BaseModel):
    ...
    learned_preferences: Dict[str, Any] = Field(
        default_factory=dict, description="学习到的偏好"
    )
    interaction_patterns: Dict[str, Any] = Field(
        default_factory=dict, description="交互模式"
    )
...
class CreateCharacterRequest(BaseModel):
    ...
    template: Optional[CharacterTemplate] = Field(
        CharacterTemplate.ZISHU_BASE, description="基于的模板"
    )
    personality_type: Optional[PersonalityType] = Field(
        PersonalityType.FRIENDLY, description="主要性格类型"
    )
    interaction_style: Optional[InteractionStyle] = Field(
        InteractionStyle.CASUAL, description="交互风格"
    )
...
class UpdateCharacterRequest(BaseModel):
    ...
    display_name: Optional[str] = Field(
        None, min_length=1, max_length=50, description="显示名称"
    )
    ...
    emotion_stability: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="情绪稳定性"
    )
    ...
    personalization_level: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="个性化程度"
    )

    # 安全设置
    content_filter_level: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="内容过滤级别"
    )
    max_response_length: Optional[int] = Field(
        None, ge=100, le=10000, description="最大回复长度"
    )
...
class BatchCharacterOperation(BaseModel):
    ...
    operation: Literal["activate", "deactivate", "delete", "export"] = Field(
        ..., description="操作类型"
    )
...
class EmotionAnalysisResponse(BaseModel):
    ...
    suggested_response_emotion: Optional[EmotionValue] = Field(
        None, description="建议的回复情绪"
    )
