...
class RateLimiter:
    ...
    def _local_rate_limit(self, key: str, limit: int, window: int) -> bool:
        ...
        with self.lock:
            ...
            # 清理过期记录
            while (self.local_cache[key] and 
                   current_time - self.local_cache[key][0] > window):
...
class SecurityAuditor:
    ...
    def log_security_event(self, event: SecurityEvent):
        ...
        try:
            ...
            logger.warning(f"Security event logged: {event.event_type.value} from {event.ip_address}")
        except Exception as e:
            logger.error(f"Failed to log security event: {e}")
    
    def detect_threats(self, data: str, ip_address: str, user_id: Optional[str] = None) -> List[SecurityEvent]:
        """检测威胁"""
        events = []
    ...
    def is_suspicious_activity(self, ip_address: str, user_id: Optional[str] = None) -> bool:
        """检查可疑活动"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                # 检查最近1小时内的失败尝试次数
...
class ModelSecurityManager:
    ...
    def register_model(self, model_id: str, security_level: SecurityLevel, 
...
class AdapterIsolationManager:
    ...
    def register_adapter(self, adapter_id: str, permissions: Set[PermissionType],
    ...
    def can_adapter_call(self, caller_id: str, target_id: str, 
                        operation: str) -> bool:
        """检查适配器是否可以调用其他适配器"""
    ...
    def _get_file_access_rules(self, permissions: Set[PermissionType]) -> Dict[str, Any]:
        """获取文件访问规则"""
        rules = {
            'read_paths': [],
            'write_paths': [],
    ...
    def _is_allowed_cross_call(self, caller_id: str, target_id: str, 
                              operation: str) -> bool:
        """检查是否允许跨适配器调用"""
        # 定义允许的跨适配器操作
        allowed_operations = {
            'get_status', 'ping', 'get_info', 'validate_input'
        }
...
class SecurityManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.jwt_secret = self.config.get('jwt_secret', 
                                        self.crypto_utils.generate_secure_token())
    ...
    def sanitize_input(self, data: str, context: SecurityContext) -> str:
        ...
        if threats:
            # 如果检测到威胁，拒绝请求
            raise SecurityError(f"Detected threats: {[t.event_type.value for t in threats]}")
    ...
    def _verify_credentials(self, username: str, password: str) -> Optional[Dict[str, Any]]:
        ...
        mock_users = {
            'admin': {
                ...
                'permissions': [PermissionType.ADMIN, PermissionType.MODEL_ACCESS, 
                              PermissionType.TRAINING_ACCESS, PermissionType.ADAPTER_MANAGE],
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if user_level_value < required_level_value:
                raise SecurityError(f"Security level insufficient: required {level.value}")
...
def audit_access(operation: str):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            context = kwargs.get('security_context')
            if context:
                logger.info(f"User {context.user_id} accessed {operation} from {context.ip_address}")
...
if __name__ == "__main__":
    ...
    # 用户认证示例
    context = security_manager.authenticate_user("admin", "admin_password", "192.168.1.100")
