...
from fastapi import (
    ...
    Request, Response, BackgroundTasks, Header
)
...
from ..schemas.auth import (
    ...
    PasswordChangeRequest, TokenRefreshRequest, VerificationRequest,
    TwoFactorSetupRequest, UserUpdateRequest,
    
    # 响应模型  
    LoginResponse, RegisterResponse, TokenResponse, UserInfoResponse,
    VerificationResponse, SessionListResponse,
    
    # 枚举和基础模型
    UserStatus, UserRole, AuthProvider, TokenType, AuthAction,
    LoginMethod, SessionStatus, generate_token_expiry, create_auth_log
)
from ..schemas.user import (
    UserInfo, UserCreateRequest, UserUpdateRequest as UserProfileUpdateRequest,
    UserPrivacyUpdateRequest, UserNotificationUpdateRequest,
    UserAvatarUpdateRequest, UserContactUpdateRequest,
    UserPasswordUpdateRequest, UserStatusUpdateRequest, UserRoleUpdateRequest
)
from ..schemas.responses import BaseResponse, ErrorResponse
from ..models.user import (
    User, UserProfile, UserSecurity, UserSession, UserToken, AuthLog,
    get_user_by_identifier, create_user_with_profile
)
...
async def register_user(
    ...
    try:
        ...
        # 速率限制检查
        if not security_manager.rate_limiter.is_allowed(f"register:{client_ip}", 3, 3600):
            raise HTTPException(
        ...
        # 发送验证邮件
        background_tasks.add_task(send_verification_email, request.email, email_code, logger)
        
        # 发送验证短信
        if phone_code:
            background_tasks.add_task(send_verification_sms, request.phone, phone_code, logger)
...
async def forgot_password(
    ...
    try:
        ...
        # 发送重置邮件
        if request.email:
            background_tasks.add_task(send_verification_email, request.email, reset_token, logger)
        
        logger.info(f"密码重置请求: {identifier}")
        
        return BaseResponse(
            success=True,
            message="如果该账户存在，重置链接已发送到您的邮箱"
        )
...
async def send_verification(
    ...
    try:
        ...
        # 发送验证码
        if request.verification_type == "email":
            background_tasks.add_task(send_verification_email, request.target, verification_code, logger)
        elif request.verification_type == "phone":
            background_tasks.add_task(send_verification_sms, request.target, verification_code, logger)
