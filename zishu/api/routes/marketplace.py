...
from fastapi import (
    ...
    Request, Response, BackgroundTasks, Query, Path, Body, File, UploadFile
)
...
from ..schemas.marketplace import (
    ...
    PaginatedResponse, SearchFilters, MarketplaceStats, VendorStats,
    
    # 基础模型
    Product, Vendor, Order, Payment, Review, Transaction, Campaign, MoneyAmount,
...
from ..schemas.adapter import (
    AdapterType, AdapterStatus, AdapterMetadata, AdapterInfo, AdapterValidationResult,
    validate_adapter_metadata, validate_adapter_file_integrity
)
...
# 工具函数
def calculate_platform_commission(amount: MoneyAmount, rate: float = 0.05) -> MoneyAmount:
    """计算平台佣金"""
...
async def list_products(
    ...
    try:
        ...
        products = [
            ProductListItem(
                ...
                pricing={
                    ...
                    "base_price": {"amount": Decimal("199.00"), "currency": CurrencyType.CNY},
                    "sale_price": {"amount": Decimal("149.00"), "currency": CurrencyType.CNY}
                },
                average_rating=4.8,
                rating_count=256,
                download_count=1850,
            ...
            ProductListItem(
                ...
                pricing={
                    ...
                    "base_price": {"amount": Decimal("0.00"), "currency": CurrencyType.CNY}
                },
                average_rating=4.5,
                rating_count=128,
                download_count=3200,
            ...
            ProductListItem(
                ...
                pricing={
                    ...
                    "base_price": {"amount": Decimal("999.00"), "currency": CurrencyType.CNY},
        ...
        if search:
            ...
            filtered_products = [
                ...
                if search_lower in p.name.lower() or 
                   search_lower in (p.short_description or "").lower()
            ]
        ...
        if rating_min is not None:
            filtered_products = [p for p in filtered_products if p.average_rating >= rating_min]
        ...
        # 排序
        if sort_by == "rating":
            filtered_products.sort(key=lambda x: x.average_rating, reverse=(sort_order == "desc"))
        elif sort_by == "price":
            filtered_products.sort(key=lambda x: x.pricing["base_price"]["amount"], reverse=(sort_order == "desc"))
        elif sort_by == "downloads":
            filtered_products.sort(key=lambda x: x.download_count, reverse=(sort_order == "desc"))
        else:  # created_at
...
async def create_product(
    ...
    try:
        ...
        logger.info(f"创建商品成功: {clean_name} ({request.slug}) by vendor {request.vendor_id}")
...
适合各种规模的项目使用，从个人开发到企业级应用都能完美支持。""",
            ...
            "pricing": {
                ...
                "sale_price": {"amount": Decimal("149.00"), "currency": CurrencyType.CNY},
                "license_type": "商业许可",
                "usage_limits": {"concurrent_users": 1000, "api_calls_per_month": 100000}
            },
            "download_count": 1850,
            "purchase_count": 520,
            "view_count": 8450,
        ...
        related_products = [
            ProductListItem(
                ...
                pricing={
                    "pricing_model": PricingModel.ONE_TIME,
                    "base_price": {"amount": Decimal("99.00"), "currency": CurrencyType.CNY}
                },
                average_rating=4.6,
                rating_count=125,
                download_count=680,
    ...
    try:
        ...
        if update_data:
            ...
            if "description" in update_data:
                update_data["description"] = sanitize_product_content(update_data["description"])
...
async def upload_product_file(
    ...
    try:
        ...
        logger.info(f"上传商品文件成功: {product_id}, 文件: {file.filename}, 大小: {len(file_content)}")
...
async def list_popular_tags(
    ...
    try:
        ...
        logger.info(f"获取热门标签: limit={limit}, min_count={min_count}, 结果数={len(result_tags)}")
        return result_tags
    ...
    try:
        ...
        orders = [
            OrderListItem(
                ...
                total_amount=MoneyAmount(amount=Decimal("149.00"), currency=CurrencyType.CNY),
            ...
            OrderListItem(
                ...
                total_amount=MoneyAmount(amount=Decimal("999.00"), currency=CurrencyType.CNY),
            ...
            product_details=[
                {
                    ...
                    "total_price": request.items[0]["price"] * request.items[0]["quantity"]
                }
            ],
            payment_history=[],
            can_cancel=True,
            ...
            product_details=[
                {
                    ...
                    "download_token": generate_download_token(
                        order_data["items"][0]["product_id"], 
                        current_user.user_id
                    ) if order_data["status"] == OrderStatus.COMPLETED else None
                }
            ],
            payment_history=[
                PaymentListItem(
...
async def refund_payment(
    ...
    try:
        ...
        refund_data = {
            "id": refund_id,
            "payment_id": payment_id,
            "amount": MoneyAmount(amount=actual_refund_amount, currency=CurrencyType.CNY),
        ...
        logger.info(f"申请退款: {payment_id}, 金额: {actual_refund_amount}, 原因: {refund_reason}")
...
async def list_product_reviews(
    ...
    sort_by: Literal["created_at", "rating", "helpful"] = Query("created_at", description="排序字段"),
    sort_order: Literal["asc", "desc"] = Query("desc", description="排序方向"),
    ...
    try:
        ...
        # 排序
        if sort_by == "rating":
            filtered_reviews.sort(key=lambda x: x.rating.value, reverse=(sort_order == "desc"))
        elif sort_by == "helpful":
            filtered_reviews.sort(key=lambda x: x.helpful_count, reverse=(sort_order == "desc"))
        else:  # created_at
            filtered_reviews.sort(key=lambda x: x.created_at, reverse=(sort_order == "desc"))
...
async def list_vendors(
    ...
    sort_by: Literal["joined_at", "rating", "product_count"] = Query("joined_at", description="排序字段"),
    sort_order: Literal["asc", "desc"] = Query("desc", description="排序方向"),
    db: Session = Depends(get_db_session),
    logger: logging.Logger = Depends(get_logger)
):
    ...
    try:
        ...
        if is_verified is not None:
            filtered_vendors = [v for v in filtered_vendors if v.is_verified == is_verified]
        
        if search:
            search_lower = search.lower()
            filtered_vendors = [
                v for v in filtered_vendors 
                if search_lower in v.name.lower() or search_lower in v.display_name.lower()
            ]
        
        # 排序
        if sort_by == "rating":
            filtered_vendors.sort(key=lambda x: x.average_rating, reverse=(sort_order == "desc"))
        elif sort_by == "product_count":
            filtered_vendors.sort(key=lambda x: x.product_count, reverse=(sort_order == "desc"))
        else:  # joined_at
            filtered_vendors.sort(key=lambda x: x.joined_at, reverse=(sort_order == "desc"))
    ...
    try:
        ...
        vendor_data = {
            **request.model_dump(),
            ...
            "product_count": 0,
            "total_sales": MoneyAmount(amount=Decimal("0.00"), currency=CurrencyType.CNY),
            "total_orders": 0,
            "average_rating": 0.0,
            "rating_count": 0,
            "is_verified": False,
...
async def get_vendor(
    ...
    try:
        ...
        vendor_data = {
            ...
            "product_count": 12,
            "total_sales": MoneyAmount(amount=Decimal("125000.00"), currency=CurrencyType.CNY),
            "total_orders": 850,
            "average_rating": 4.7,
            "rating_count": 89,
            "is_verified": True,
            "verification_level": 3,
            "balance": MoneyAmount(amount=Decimal("8500.00"), currency=CurrencyType.CNY),
            ...
            products = [
                {
                    ...
                    "price": {"amount": Decimal("199.00"), "currency": CurrencyType.CNY},
                    "rating": 4.8,
                    "vendor_name": "AI创新工作室",
...
async def get_marketplace_stats(
    ...
    try:
        ...
        stats = MarketplaceStats(
            ...
            total_vendors=156,
            total_orders=8950,
            total_revenue=MoneyAmount(amount=Decimal("2850000.00"), currency=CurrencyType.CNY),
            ...
            revenue_today=MoneyAmount(amount=Decimal("25600.00"), currency=CurrencyType.CNY),
            revenue_this_week=MoneyAmount(amount=Decimal("185000.00"), currency=CurrencyType.CNY),
            revenue_this_month=MoneyAmount(amount=Decimal("685000.00"), currency=CurrencyType.CNY),
