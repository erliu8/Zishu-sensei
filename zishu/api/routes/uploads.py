    ...
    try:
        ...
        validation_result["upload_options"] = {
            ...
            "recommended_chunk_size": min(
                CHUNK_SIZE, max(1024 * 1024, filesize // 100)
            ),
            "estimated_chunks": (filesize + CHUNK_SIZE - 1) // CHUNK_SIZE
            if chunk_upload
            else 1,
...
async def upload_multiple_files(
    ...
    try:
        ...
        if not files:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="没有选择任何文件"
            )
        ...
        for i, file in enumerate(files):
            if not file.filename:
                invalid_files.append(
                    {"index": i, "filename": "unknown", "error": "文件名为空"}
                )
                continue
            ...
            if not FileValidator.validate_filename(file.filename):
                invalid_files.append(
                    {"index": i, "filename": file.filename, "error": "文件名包含非法字符"}
                )
                continue

            if FileValidator.is_dangerous_file(file.filename):
                invalid_files.append(
                    {"index": i, "filename": file.filename, "error": "不允许上传此类型的文件"}
                )
                continue
...
async def init_chunk_upload(
    ...
    chunk_size: int = Form(
        CHUNK_SIZE, ge=1024 * 1024, le=CHUNK_SIZE, description="分块大小"
    ),
    folder_id: Optional[str] = Form(None, description="目标文件夹ID"),
...
async def upload_chunk(
    ...
    try:
        ...
        if upload_id not in upload_sessions:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="上传会话不存在或已过期"
            )
        ...
        if session_info["user_id"] != current_user["user_id"]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="无权限访问此上传会话"
            )

        # 3. 检查会话是否过期
        if datetime.now() > session_info["expires_at"]:
            raise HTTPException(status_code=status.HTTP_410_GONE, detail="上传会话已过期")

        # 4. 验证分块索引
        if chunk_index >= session_info["total_chunks"]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="分块索引超出范围"
            )
        ...
        if chunk_hash:
            ...
            if actual_hash != chunk_hash:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail="分块数据校验失败"
                )
        ...
        if not success:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="分块上传失败"
            )
...
async def complete_chunk_upload(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="分块合并失败"
            )
        ...
        background_tasks.add_task(
            _cleanup_upload_session, upload_id, delay_seconds=300  # 5分钟后清理
        )
...
async def get_upload_statistics(
    ...
    try:
        ...
        return {
            ...
            "upload_types": {"single": 120, "batch": 20, "chunk": 10},
            "file_types": {
                "image": 80,
                "document": 40,
                "video": 15,
...
async def _upload_single_file_async(
    ...
    async with semaphore:
        try:
            ...
            # 保存到最终位置
            final_path, metadata = await file_manager.save_file(
                open(temp_path, "rb"), file.filename, folder=folder_id
            )
...
async def _log_upload_activity(
    file_id: str, user_id: str, description: str, file_size: int
):
    """记录上传活动"""
    try:
        file_service = FileService()
...
async def _log_batch_upload_activity(
    batch_id: str, user_id: str, total_files: int, success_count: int, failed_count: int
):
    """记录批量上传活动"""
    try:
        print(
            f"批量上传完成: batch_id={batch_id}, user={user_id}, "
...
async def _log_upload_error(
    upload_id: str, user_id: str, error: str, context: Dict[str, Any]
):
    """记录上传错误"""
    try:
        print(
            f"上传错误: upload_id={upload_id}, user={user_id}, error={error}, context={context}"
        )
    except Exception as e:
        print(f"错误记录失败: {e}")
...
async def cleanup_upload_resources(
    ...
    try:
        ...
        if cleanup_expired_sessions:
            background_tasks.add_task(_cleanup_expired_sessions, max_age_hours)
            cleanup_tasks.append("过期会话")

        if cleanup_temp_files:
            background_tasks.add_task(_cleanup_temp_files, max_age_hours)
            cleanup_tasks.append("临时文件")

        if cleanup_orphaned_chunks:
            background_tasks.add_task(_cleanup_orphaned_chunks, max_age_hours)
