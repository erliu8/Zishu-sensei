...
from ..schemas.user import (
    # 基础模型
    UserInfo, UserBase, UserPublicInfo, UserStatistics, UserSecurityInfo,
    
    # 请求模型
    UserCreateRequest, UserUpdateRequest, UserPrivacyUpdateRequest,
    UserNotificationUpdateRequest, UserAvatarUpdateRequest, 
    UserContactUpdateRequest, UserPasswordUpdateRequest,
    UserStatusUpdateRequest, UserRoleUpdateRequest, UserQueryParams,
    UserBatchOperationRequest,
    
    # 响应模型
    UserResponse, UserListResponse, UserPublicResponse, 
    UserStatsResponse, UserActivityResponse, UserBatchOperationResponse,
    UserValidationResponse, SystemUserStatsResponse,
    
    # 枚举和工具函数
    UserStatus, UserRole, AuthProvider, ProfileVisibility,
    validate_user_permissions, check_username_availability,
    calculate_user_score, format_user_display_name, create_user_activity_log
)
...
from ..utils.auth_decorators import (
    require_auth, require_permissions, require_roles, require_admin, 
    rate_limit, audit_log, security_headers, validate_input,
...
async def require_user_access(
    ...
    if (current_user.user_id != user_id and 
        PermissionType.ADMIN not in current_user.permissions):
        raise HTTPException(
    ...
    try:
        ...
        if current_user:
            ...
            can_view_private = (
                ...
                PermissionType.ADMIN in current_user.permissions
            )
...
async def delete_user(
    ...
    try:
        ...
        logger.info(f"用户删除成功: {user_id}, 类型: {delete_type}, 操作人: {current_user.user_id}")
        
        return BaseResponse(
            success=True,
            message=f"用户{'软删除' if soft_delete else '硬删除'}成功"
        )
...
async def update_user_status(
    ...
    try:
        ...
        logger.info(f"用户状态更新: {user_id} -> {request.status.value}, 操作人: {current_user.user_id}")
        
        return BaseResponse(
            success=True,
            message=f"用户状态已更新为: {request.status.value}"
        )
...
async def update_user_role(
    ...
    try:
        ...
        logger.info(f"用户角色更新: {user_id} -> {request.role.value}, 操作人: {current_user.user_id}")
        
        return BaseResponse(
            success=True,
            message=f"用户角色已更新为: {request.role.value}"
        )
...
async def suspend_user(
    ...
    try:
        ...
        logger.info(f"用户暂停: {user_id}, 期限: {duration_days}天, 操作人: {current_user.user_id}")
        ...
        for user_id in request.user_ids:
            try:
                ...
                if request.operation == "activate":
                    # 激活用户
                    result = {"user_id": user_id, "status": "success", "message": "用户已激活"}
                elif request.operation == "deactivate":
                    # 停用用户
                    result = {"user_id": user_id, "status": "success", "message": "用户已停用"}
                elif request.operation == "suspend":
                    # 暂停用户
                    result = {"user_id": user_id, "status": "success", "message": "用户已暂停"}
                elif request.operation == "delete":
                    # 删除用户
                    result = {"user_id": user_id, "status": "success", "message": "用户已删除"}
                    ...
                    result = {"user_id": user_id, "status": "success", "message": "通知已发送"}
                elif request.operation == "update_role":
                    # 更新角色
                    new_role = request.parameters.get("role")
                    result = {"user_id": user_id, "status": "success", "message": f"角色已更新为{new_role}"}
                elif request.operation == "add_tags":
                    # 添加标签
                    tags = request.parameters.get("tags", [])
                    result = {"user_id": user_id, "status": "success", "message": f"已添加标签: {tags}"}
                elif request.operation == "remove_tags":
                    # 移除标签
                    tags = request.parameters.get("tags", [])
                    result = {"user_id": user_id, "status": "success", "message": f"已移除标签: {tags}"}
                else:
                    result = {"user_id": user_id, "status": "error", "message": "不支持的操作类型"}
                    failed_count += 1
                    continue
...
async def export_users(
    ...
    try:
        ...
        logger.info(f"用户数据导出任务启动: {export_id}, 格式: {format}, 操作人: {current_user.user_id}")
...
async def check_email_availability(
    ...
    try:
        ...
        return UserValidationResponse(
            ...
            message=message,
            suggestions=["@gmail.com", "@qq.com", "@163.com"] if not format_valid else []
        )
