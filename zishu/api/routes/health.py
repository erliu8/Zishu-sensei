...
class ModelChecker(HealthChecker):
    ...
    async def _do_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            details = {"gpu_available": gpu_available, "gpu_count": gpu_count}
                ...
                for i in range(gpu_count):
                    ...
                    details[f"gpu_{i}"] = {
                        ...
                        "memory_usage_percent": round(
                            (memory_allocated / total_memory) * 100, 2
                        ),
                    }

            return {"message": "Model check successful", "details": details}
        except Exception as e:
            return {
...
class SystemChecker(HealthChecker):
    ...
    async def _do_check(self) -> Dict[str, Any]:
        try:
            ...
            details = {
                "cpu": {"usage_percent": cpu_percentage, "count": psutil.cpu_count()},
                "memory": {
            ...
            message = (
                ...
                else f"System is degraded: {', '.join(status_issues)}"
            )
...
async def get_health_status(
    ...
    unhealthy_count = sum(
        1 for result in final_results if result.status == HealthStatus.UNHEALTHY
    )
    degraded_count = sum(
        1 for result in final_results if result.status == HealthStatus.DEGRADED
    )
    ...
    summary = {
        ...
        "healthy_count": sum(
            1 for result in final_results if result.status == HealthStatus.HEALTHY
        ),
...
async def deep_health_check(
    ...
    try:
        ...
        logger.info(
            f"Deep health check completed successfully,status: %s", result.status
        )
        return result
    except Exception as e:
