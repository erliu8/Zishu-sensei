...
from fastapi import APIRouter, Depends, HTTPException, status, Query, Path, Body, BackgroundTasks, File, UploadFile
...
from ..schemas.packaging import (
    ...
    SignRequest, SignatureResponse, PackageStatistics,
    
    # 枚举和状态
    PackageStatus, BuildStatus, TestType, PackageType,
    DistributionChannel, PackageFormat,
    
    # 分页和过滤
    PaginatedResponse, SearchFilters,
    
    # 文件和版本
    FileInfo, VersionInfo, DependencyInfo
)
...
from ..tasks.packaging_tasks import (
    build_package_task, test_package_task, publish_package_task,
    batch_build_task, package_pipeline_task, cleanup_task
)
...
@router.post("/packages", response_model=PackageDetailResponse, status_code=status.HTTP_201_CREATED)
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for file in files:
                ...
                # 如果是压缩文件且需要解压
                if extract_archives and file.filename.endswith(('.zip', '.tar.gz', '.tar')):
...
async def download_package(
    ...
    try:
        ...
        # 构建文件路径
        package_file = PathLib(settings.PACKAGES_DIR) / package_id / target_version / f"package.{format.value}"
...
@router.post("/packages/{package_id}/build", response_model=BuildTaskResponse, status_code=status.HTTP_202_ACCEPTED)
...
@router.post("/builds/batch", response_model=Dict[str, Any], status_code=status.HTTP_202_ACCEPTED)
...
@router.post("/packages/{package_id}/test", response_model=TestResultResponse, status_code=status.HTTP_202_ACCEPTED)
...
@router.post("/packages/{package_id}/publish", response_model=PublishResultResponse, status_code=status.HTTP_202_ACCEPTED)
...
@router.post("/packages/{package_id}/pipeline", response_model=Dict[str, Any], status_code=status.HTTP_202_ACCEPTED)
...
async def get_package_statistics(
    ...
    try:
        result = await packaging_service.get_package_statistics(
            user_id=current_user.id if user_scope and current_user else None,
            db=db
        )
