        ...
        if hasattr(model_manager, "get_loaded_adapters"):
            ...
            for adapter_name, adapter_info in loaded_adapters.items():
                adapters.append(AdapterInfo(
                    ...
                    config=adapter_info.get("config", {}) if include_details else {}
                ))
        ...
        if include_unloaded:
            ...
            if os.path.exists(adapter_dir):
                for item in os.listdir(adapter_dir):
                    ...
                    # 检查是否为适配器文件或目录
                    valid_extensions = (".tar.gz", ".zip", ".7z", ".rar", ".gz", ".tar", ".safetensors", ".bin")
                    if os.path.isdir(item_path) or any(item.endswith(ext) for ext in valid_extensions):
                        # 检查是否已在加载列表中
                        if not any(adapter.name == item for adapter in adapters):
                            file_info = await get_adapter_file_info(item_path) if include_details else {}
                            adapters.append(AdapterInfo(
                                name=item,
                                path=item_path,
...
async def load_adapter(
    ...
    try:
        ...
        if hasattr(model_manager, "is_adapter_loaded"):
            if model_manager.is_adapter_loaded(request.adapter_name) and not request.force_reload:
    ...
    try:
        ...
        if hasattr(model_manager, "unload_adapter"):
            result = await model_manager.unload_adapter(
                adapter_name=request.adapter_name,
                force=request.force_unload
            )
            
            execution_time = time.time() - start_time
            
            if result.get('success', True):
                logger.info(f"Successfully unloaded adapter: {request.adapter_name} in {execution_time:.2f} seconds")
...
async def switch_adapter(
    ...
    """切换适配器"""
    try:
        logger.info(f"Switching adapter: {request.from_adapter} -> {request.to_adapter}")
        start_time = time.time()
        ...
        if hasattr(model_manager, "switch_adapter"):
            ...
            if result.get('success', True):
                logger.info(f"Successfully switched adapter: {request.from_adapter} -> {request.to_adapter} in {execution_time:.2f} seconds")
        ...
        execution_time = time.time() - start_time
        logger.error(f"Failed to switch adapter: {request.from_adapter} -> {request.to_adapter} - {str(e)}")
            ...
            "model_manager": {
                ...
                "methods": [
                    method for method in [
                        "load_adapter", "unload_adapter", "switch_adapter", 
                        "get_loaded_adapters", "get_status", "is_adapter_loaded"
                    ] if hasattr(model_manager, method)
                ] if model_manager else []
            }
        }
