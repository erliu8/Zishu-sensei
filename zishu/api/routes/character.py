...
from ..schemas.chat import (
    # 模型类
    CharacterConfig, CharacterState, EmotionTransition, CharacterTemplate_Model,
    # 枚举类
    EmotionType, PersonalityType, VoiceStyle, AnimationType, CharacterTemplate,
...
PREDEFINED_TEMPLATES = {
    CharacterTemplate.ZISHU_BASE: CharacterTemplate_Model(
        ...
        config=CharacterConfig(
            ...
            animations=[AnimationType.HAPPY, AnimationType.THINKING, AnimationType.SURPRISED]
        ),
        tags=["可爱", "温柔", "害羞", "日系"]
    ),
...
async def analyze_emotion_from_text(text: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
    """从文本分析情绪（简化版实现）"""
...
async def update_character_state(character_id: str, emotion: EmotionType, 
                               intensity: float = 0.7, reason: str = None) -> CharacterState:
    """更新角色状态"""
...
async def create_character(request: CreateCharacterRequest):
    ...
    try:
        ...
        return CharacterResponse(
            success=True,
            message=f"角色 '{base_config.name}' 创建成功",
...
async def list_characters(
    ...
    try:
        ...
        if personality_type:
            characters = [c for c in characters if c.personality_type == personality_type]
...
async def delete_character(
    ...
    try:
        ...
        return CharacterResponse(
            success=True,
            message=f"角色 '{character.name}' 已删除",
            character=None
        )
...
@router.get("/{character_id}/emotion", response_model=EmotionResponse, summary="获取角色当前情绪")
...
@router.post("/{character_id}/emotion", response_model=EmotionResponse, summary="设置角色情绪")
async def set_character_emotion(
    ...
    try:
        ...
        previous_emotion = current_state.current_emotion if current_state else EmotionType.NEUTRAL
...
@router.post("/analyze-emotion", response_model=EmotionAnalysisResponse, summary="分析文本情绪")
...
@router.get("/{character_id}/state", response_model=CharacterStateResponse, summary="获取角色状态")
async def get_character_state(
    ...
    performance_metrics = {
        ...
        "last_active": state.last_interaction.isoformat() if state.last_interaction else None,
...
@router.post("/{character_id}/interact", response_model=CharacterInteractionResponse, summary="与角色交互")
async def interact_with_character(
    ...
    try:
        ...
        # 分析用户情绪
        user_emotion_analysis = await analyze_emotion_from_text(request.message, request.context)
        ...
        return CharacterInteractionResponse(
            ...
            tokens_used=len(request.message.split()) + len(response_text.split()),  # 简化计算
...
@router.get("/templates/{template_id}", response_model=CharacterTemplate_Model, summary="获取模板详情")
...
@router.post("/{character_id}/apply-template", response_model=CharacterResponse, summary="应用模板到角色")
...
async def get_system_stats():
    ...
    try:
        ...
        popular_personalities = [
            {"type": k, "count": v} 
            for k, v in sorted(personality_counts.items(), key=lambda x: x[1], reverse=True)
        ]
...
async def init_character_system():
    ...
    if not CHARACTER_STORE:
        default_character = PREDEFINED_TEMPLATES[CharacterTemplate.ZISHU_BASE].config.model_copy()
