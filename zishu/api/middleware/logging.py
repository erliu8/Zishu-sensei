...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    def _log_response(
        ...
        """记录响应信息"""
        self.logger.info(
            f"Response {request_id}: {response.status_code} " f"in {process_time:.4f}s"
        )
...
class PerformanceLoggingMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self, app, slow_request_threshold: float = 1.0, log_all_requests: bool = False
    ):
        super().__init__(app)
...
def setup_logging_middleware(
    ...
    if enable_performance_logging:
        app.add_middleware(
            PerformanceLoggingMiddleware, slow_request_threshold=slow_threshold
        )
    ...
    # 添加基础日志中间件
    app.add_middleware(LoggingMiddleware, logger_name="api", log_level=log_level)
