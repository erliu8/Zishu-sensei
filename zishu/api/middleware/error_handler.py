...
def setup_error_handler(
    app, include_traceback: bool = False, log_errors: bool = True
) -> ErrorHandlerMiddleware:
    """设置错误处理中间件"""

    # 创建错误处理中间件
    error_handler = ErrorHandlerMiddleware(
        app, include_traceback=include_traceback, log_errors=log_errors
    )

    # 添加自定义处理器
    error_handler.add_custom_handler(
        ValueError, ValidationErrorHandler.handle_validation_error
    )

    error_handler.add_custom_handler(
        TimeoutError, TimeoutErrorHandler.handle_timeout_error
    )
