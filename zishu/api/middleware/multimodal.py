...
class AudioProcessor:
    ...
    async def process_audio(self, audio_file: MediaFile) -> ProcessingResult:
        ...
        try:
            ...
            if not self._validate_audio(audio_file):
                return ProcessingResult(success=False, error="音频文件验证失败")
    ...
    async def text_to_speech(
        ...
        try:
            voice = (
                voice_settings.get("voice", "zh-CN-XiaoxiaoNeural")
                if voice_settings
    ...
    async def process_image(self, image_file: MediaFile) -> ProcessingResult:
        ...
        try:
            ...
            if not self._validate_image(image_file):
                return ProcessingResult(success=False, error="图像文件验证失败")
...
class AnimationGenerator:
    ...
    async def generate_animation(
        self, character_config: CharacterConfig, emotion: str, text: str = ""
    ) -> ProcessingResult:
        """生成角色动画"""
        start_time = time.time()
...
class MultimodalProcessor:
    ...
    def __init__(self, config: MultimodalConfig = None):
        ...
        self.cache = (
            CacheManager(max_size=self.config.max_cache_size, ttl=self.config.cache_ttl)
    ...
    async def process_media(
        self, media_file: MediaFile, character_config: Optional[CharacterConfig] = None
    ) -> ProcessingResult:
        """处理媒体文件"""
        ...
        if media_file.media_type == MediaType.AUDIO:
            ...
            result = ProcessingResult(
                success=False, error=f"不支持的媒体类型: {media_file.media_type}"
            )
    ...
    async def generate_response(
        ...
        try:
            ...
            if self.config.audio_enabled and response["text"]:
                voice_settings = (
                    character_config.voice_settings if character_config else {}
                )
...
class MultimodalMiddleware(BaseHTTPMiddleware):
    ...
    def _is_multimodal_request(self, request: Request) -> bool:
        ...
        return (
            ...
            or "/multimodal" in str(request.url)
            or "X-Multimodal-Request" in request.headers
        )
    ...
    async def _create_error_response(
        self, error: Exception, processing_time: float
    ) -> Response:
        """创建错误响应"""
...
def get_character_config_from_request(
    character_id: Optional[str] = None,
) -> Optional[CharacterConfig]:
    """从请求中获取角色配置"""
    if not character_id:
        return None
    ...
    return CharacterConfig(
        ...
        voice_settings={"voice": "zh-CN-XiaoxiaoNeural", "rate": "+0%", "pitch": "+0%"},
        emotion_mapping={"happy": "smile", "sad": "frown", "neutral": "normal"},
        animation_settings={"style": "anime", "quality": "medium"},
...
async def process_multimodal_input(
    media_files: List[MediaFile], character_id: Optional[str] = None
...
async def generate_multimodal_response(
    input_data: Dict[str, Any], character_id: Optional[str] = None
) -> Dict[str, Any]:
    """生成多模态响应的便捷函数"""
    processor = get_multimodal_processor()
    character_config = get_character_config_from_request(character_id)
