...
class MetricsCollector:
    ...
    def __init__(self, config: MonitoringConfig):
        self.config = config
        self._metrics: Dict[str, deque] = defaultdict(lambda: deque(maxlen=config.max_data_points))
    ...
    def get_metric_history(self, name: str, hours: int = 1) -> List[MetricPoint]:
        ...
        with self._lock:
            ...
            return [point for point in self._metrics[name] 
        ...
        try:
            ...
            if self.config.system_metrics_enabled:
                ...
                if hasattr(psutil, 'getloadavg'):
                    ...
                    self.metrics_collector.record_metric("system.cpu.load_avg_1m", load_avg[0])
                    self.metrics_collector.record_metric("system.cpu.load_avg_5m", load_avg[1])
                    self.metrics_collector.record_metric("system.cpu.load_avg_15m", load_avg[2])
            
            # 内存指标
            if self.config.system_metrics_enabled:
                memory = psutil.virtual_memory()
                self.metrics_collector.record_metric("system.memory.total_bytes", memory.total)
                self.metrics_collector.record_metric("system.memory.available_bytes", memory.available)
                self.metrics_collector.record_metric("system.memory.used_bytes", memory.used)
                self.metrics_collector.record_metric("system.memory.usage_percent", memory.percent)
                
                # 交换内存
                swap = psutil.swap_memory()
                self.metrics_collector.record_metric("system.swap.total_bytes", swap.total)
                self.metrics_collector.record_metric("system.swap.used_bytes", swap.used)
                self.metrics_collector.record_metric("system.swap.usage_percent", swap.percent)
            
            # 磁盘指标
            if self.config.system_metrics_enabled:
                disk_usage = psutil.disk_usage('/')
                self.metrics_collector.record_metric("system.disk.total_bytes", disk_usage.total)
                self.metrics_collector.record_metric("system.disk.used_bytes", disk_usage.used)
                self.metrics_collector.record_metric("system.disk.free_bytes", disk_usage.free)
                ...
                if disk_io:
                    self.metrics_collector.record_metric("system.disk.read_bytes", disk_io.read_bytes)
                    self.metrics_collector.record_metric("system.disk.write_bytes", disk_io.write_bytes)
                    self.metrics_collector.record_metric("system.disk.read_count", disk_io.read_count)
                    self.metrics_collector.record_metric("system.disk.write_count", disk_io.write_count)
            ...
            if self.config.network_metrics_enabled:
                ...
                if network_io:
                    ...
                    self.metrics_collector.record_metric("system.network.packets_sent", network_io.packets_sent)
                    self.metrics_collector.record_metric("system.network.packets_recv", network_io.packets_recv)
                    ...
                    if self._last_network_stats and self._last_network_time:
                        ...
                        if time_delta > 0:
                            ...
                            self.metrics_collector.record_metric("system.network.bytes_sent_rate", bytes_sent_rate)
                            self.metrics_collector.record_metric("system.network.bytes_recv_rate", bytes_recv_rate)
            ...
            if self.config.process_metrics_enabled:
                ...
                self.metrics_collector.record_metric("process.cpu.usage_percent", cpu_percent)
                
                # 内存使用
                memory_info = process.memory_info()
                self.metrics_collector.record_metric("process.memory.rss_bytes", memory_info.rss)
                self.metrics_collector.record_metric("process.memory.vms_bytes", memory_info.vms)
                ...
                self.metrics_collector.record_metric("process.threads.count", num_threads)
                
                # 连接数
                try:
                    connections = process.connections()
                    self.metrics_collector.record_metric("process.connections.count", len(connections))
...
class HTTPMonitor:
    ...
    def record_request(self, request: Request, response: Response, 
                      duration: float, error: Optional[Exception] = None) -> None:
        """记录HTTP请求指标"""
        ...
        # 请求计数
        self.metrics_collector.record_metric("http.requests.total", 1, 
                                            MetricType.COUNTER, labels)
        
        # 响应时间
        self.metrics_collector.record_metric("http.request.duration_seconds", duration,
        ...
        if error or response.status_code >= 400:
            self._error_count += 1
            self.metrics_collector.record_metric("http.requests.errors", 1,
                                                MetricType.COUNTER, labels)
        ...
        if self.config.track_request_size:
            ...
            if content_length:
                try:
                    ...
                    self.metrics_collector.record_metric("http.request.size_bytes", size,
                                                        MetricType.HISTOGRAM, labels)
                except ValueError:
                    pass
        ...
        if self.config.track_response_size:
            ...
            if content_length:
                try:
                    size = int(content_length)
                    self.metrics_collector.record_metric("http.response.size_bytes", size,
                                                        MetricType.HISTOGRAM, labels)
                except ValueError:
                    pass
        ...
        if duration > self.config.slow_request_threshold:
            self.metrics_collector.record_metric("http.requests.slow", 1,
                                                MetricType.COUNTER, labels)
...
class AlertManager:
    ...
    def _setup_default_alert_rules(self) -> None:
        ...
        def memory_usage_alert(metrics_collector: MetricsCollector) -> List[Alert]:
            alerts = []
            memory_metric = metrics_collector.get_latest_metric("system.memory.usage_percent")
...
class MonitoringManager:
    ...
    def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics_count": sum(len(points) for points in self.metrics_collector._metrics.values()),
...
class MonitoringMiddleware(BaseHTTPMiddleware):
        ...
        try:
            if config_path:
                ...
                if config_file.exists():
                    ...
                    monitoring_config = config_data.get('multimodal', {}).get('monitoring', {})
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        self.monitoring_manager.http_monitor.record_request(request, response, duration, error)
