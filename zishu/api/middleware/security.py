        ...
        if not api_key:
            return JSONResponse(
                ...
                content={
                    ...
                    "message": f"API key required in {self.header_name} header",
                },
            )
        ...
        if self.include_request_body and request.method in ["POST", "PUT", "PATCH"]:
            try:
                ...
                if len(body) <= self.max_body_size:
                    ...
                    request_info[
                        "request_body"
                    ] = f"<truncated, size: {len(body)} bytes>"
            ...
            if self.include_response_body and hasattr(response, "body"):
                try:
                    if len(response.body) <= self.max_body_size:
                        ...
                        response_info[
                            "response_body"
                        ] = f"<truncated, size: {len(response.body)} bytes>"
                except Exception as e:
...
def create_api_key_middleware(
    ...
    return lambda app: APIKeyMiddleware(
        app, api_keys=api_keys, header_name=header_name, exempt_paths=exempt_paths
    )
