...
class CacheManager:
    ...
    def __init__(self, config: CacheConfig):
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=4, thread_name_prefix="cache-worker"
        )
    ...
    async def get(
        self, key: Union[str, Dict[str, Any]], default: Any = None
    ) -> Tuple[Any, bool]:
        """获取缓存值"""
...
class HTTPCacheManager:
    ...
    async def cache_response(
        ...
        """缓存响应"""
        if not self._is_cacheable_request(request) or not self._is_cacheable_response(
            response
        ):
            return
        ...
        headers[
            "cache-control"
        ] = f"public, max-age={self.config.cache_control_max_age}"
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        try:
            ...
            if self.config.http_cache_enabled:
                cached_response = await self.http_cache_manager.get_cached_response(
                    request
                )
                if cached_response:
...
async def cache_get(
    key: Union[str, Dict[str, Any]], default: Any = None
) -> Tuple[Any, bool]:
    """获取缓存值的便捷函数"""
