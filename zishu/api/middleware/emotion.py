...
class RuleBasedEmotionAnalyzer(EmotionAnalyzer):
    ...
    def __init__(self, custom_patterns: Dict = None):
        ...
        self.context_modifiers = {
            ...
            EmotionContext.QUESTION: {
                EmotionType.CURIOUS: 0.2,
                EmotionType.CONFUSED: 0.1,
            },
            EmotionContext.COMPLIMENT: {
                EmotionType.HAPPY: 0.3,
                EmotionType.EXCITED: 0.2,
            },
            ...
            if emotion_type in self.emotion_patterns:
                ...
                if "keywords" in pattern_data:
                    existing_keywords = set(
                        self.emotion_patterns[emotion_type]["keywords"]
                    )
                    new_keywords = set(pattern_data["keywords"])
                    self.emotion_patterns[emotion_type]["keywords"] = list(
                ...
                if "patterns" in pattern_data:
                    existing_patterns = set(
                        self.emotion_patterns[emotion_type]["patterns"]
                    )
                    new_patterns = set(pattern_data["patterns"])
                    self.emotion_patterns[emotion_type]["patterns"] = list(
                        existing_patterns | new_patterns
                    )

                # 更新权重
                if "weight" in pattern_data:
                    self.emotion_patterns[emotion_type]["weight"] = pattern_data[
                        "weight"
                    ]
            else:
                # 添加新的情绪类型模式
...
class MLEmotionAnalyzer(EmotionAnalyzer):
    ...
    def __init__(self):
        ...
        if self.model_available:
            ...
            self.logger.info(
                "ML emotion analyzer not available, will use rule-based fallback"
            )
...
class EmotionTransitionEngine:
    ...
    def should_transition(
        ...
        # 获取转换概率
        transition_prob = self.transition_rules.get(current_emotion, {}).get(
            new_emotion, 0.1
        )
...
class EmotionMiddleware:
    ...
    def update_character_config(self, new_config: Dict):
        ...
        if "emotion_patterns" in new_config:
            if hasattr(self.analyzers["rule_based"], "_merge_custom_patterns"):
                self.analyzers["rule_based"]._merge_custom_patterns(
                    new_config["emotion_patterns"]
                )
    ...
    async def analyze_user_emotion(
        self, text: str, user_id: str, context: Optional[Dict] = None
    ) -> EmotionState:
        ...
        try:
            ...
            if "ml_based" in self.analyzers:
                ...
                if (
                    hasattr(ml_analyzer, "model_available")
                    and ml_analyzer.model_available
                ):
            ...
            if emotion_state is None:
                ...
                if rule_analyzer:
                    ...
                    # 最后的备用方案：创建默认情绪状态
                    emotion_state = EmotionState(
                        emotion=EmotionType.NEUTRAL, confidence=0.3
                    )
                    analyzer_used = "default"
                    self.logger.warning(
                        "No emotion analyzer available, using default neutral state"
                    )
            ...
            self.logger.debug(
                f"User emotion analyzed using {analyzer_used}: {emotion_state.emotion} "
    ...
    async def generate_response_emotion(
        ...
        try:
            ...
            self.logger.debug(
                f"Response emotion generated: {response_emotion.emotion} "
    ...
    async def _apply_emotion_memory(
        self, emotion_state: EmotionState, user_id: str, memory_type: str
    ) -> EmotionState:
        """应用情绪记忆"""
        ...
        if recent_emotions:
            # 如果最近有相似情绪，增强置信度
            similar_emotions = [
                e for e in recent_emotions if e.emotion == emotion_state.emotion
            ]
            if similar_emotions:
                emotion_state.confidence = min(emotion_state.confidence + 0.1, 1.0)
                emotion_state.intensity = min(
                    emotion_state.intensity
                    + sum(e.intensity for e in similar_emotions)
                    / len(similar_emotions)
                    * 0.1,
                    1.0,
                )
    ...
    async def _get_character_current_emotion(
    ...
    async def _calculate_response_emotion(
        self,
        user_emotion: EmotionState,
    ...
    async def _update_character_emotion_memory(
        self, character_id: str, user_id: str, emotion_state: EmotionState
    ):
        """更新角色情绪记忆"""
        memory_key = f"{user_id}_{character_id}"
...
def load_character_emotion_config(character_config_path: str = None) -> Dict:
    ...
    try:
        ...
        if not config_path.exists():
            # 尝试相对路径
            config_path = (
                Path(__file__).parent.parent.parent.parent / character_config_path
            )
        ...
        # 转换为中间件配置格式
        middleware_config = {
            "enable_memory": emotion_config.get("analyzer", {}).get(
                "enable_memory", True
            ),
            "memory_ttl": emotion_config.get("analyzer", {}).get("memory_ttl", 3600),
            "enable_transition": emotion_config.get("transition", {}).get(
            ...
            "transition_speed": emotion_config.get("transition", {}).get(
                "transition_speed", 0.5
            ),
            # 响应生成设置
            "enable_tone_modification": emotion_config.get("response", {}).get(
                "enable_tone_modification", True
            ),
            "intensity_threshold": emotion_config.get("response", {}).get(
                "intensity_threshold", 0.3
            ),
            "style_variation": emotion_config.get("response", {}).get(
...
def initialize_emotion_middleware(
    config: Optional[Dict] = None, character_config_path: str = None
) -> EmotionMiddleware:
    """
    初始化情绪中间件
