...
from ..schemas.file import (
    FileType, FileStatus, FileAccessLevel, StorageProvider, ScanStatus,
    FileMetadata, FileSecurityInfo, FileStorageInfo, FileValidator, FileHelper
)
...
DANGEROUS_EXTENSIONS = {
    ...
    '.ps1', '.sh', '.app', '.deb', '.rpm', '.dmg', '.pkg', '.msi'
}
...
class FileProcessor:
    ...
    def __init__(self, temp_dir: Optional[str] = None, max_workers: int = 4):
        ...
        """
        self.temp_dir = Path(temp_dir) if temp_dir else Path(tempfile.gettempdir()) / "zishu_files"
        self.temp_dir.mkdir(parents=True, exist_ok=True)
        self.max_workers = max_workers
        self.logger = logging.getLogger(f"{__name__}.{self.__class__.__name__}")
    
    async def validate_file(self, file_path: Union[str, Path], 
                          check_content: bool = True) -> Tuple[bool, List[str]]:
        """
        验证文件是否安全和有效
    ...
    async def verify_file_integrity(self, file_path: Union[str, Path], 
    ...
    async def calculate_file_hash(self, file_path: Union[str, Path], 
    ...
    async def generate_thumbnail(self, file_path: Union[str, Path], 
        ...
        try:
            ...
            if mime_type.startswith('image/') and HAS_PIL:
                return await self._generate_image_thumbnail(file_path, output_path, size)
            elif mime_type.startswith('video/') and HAS_FFMPEG:
                return await self._generate_video_thumbnail(file_path, output_path, size)
    ...
    async def _generate_video_thumbnail(self, file_path: Path, 
        ...
        try:
            ...
            (
                ffmpeg
                .input(str(file_path), ss=1)  # 从第1秒开始
...
class FileManager:
    ...
    def __init__(self, base_path: Union[str, Path], processor: Optional[FileProcessor] = None):
        """
        初始化文件管理器
    ...
    async def save_file(self, file_data: BinaryIO, filename: str, 
                       folder: Optional[str] = None,
                       generate_unique_name: bool = True) -> Tuple[Path, FileMetadata]:
        """
        保存文件并生成元数据
    ...
    async def copy_file(self, src_path: Union[str, Path], 
    ...
    async def move_file(self, src_path: Union[str, Path], 
    ...
    async def list_files(self, dir_path: Union[str, Path], 
                        pattern: str = "*",
...
class ChunkedUploadManager:
    ...
    def __init__(self, temp_dir: Optional[str] = None, chunk_size: int = MAX_CHUNK_SIZE):
        """
        初始化分块上传管理器
        ...
        Args:
            ...
            chunk_size: 分块大小
        """
        self.temp_dir = Path(temp_dir) if temp_dir else Path(tempfile.gettempdir()) / "zishu_chunks"
        self.temp_dir.mkdir(parents=True, exist_ok=True)
        self.chunk_size = chunk_size
        self.uploads = {}  # 存储上传会话信息
        self.logger = logging.getLogger(f"{__name__}.{self.__class__.__name__}")
    
    def create_upload_session(self, filename: str, total_size: int, 
    ...
    async def complete_upload(self, upload_id: str, 
        ...
        try:
            ...
            if actual_size != upload_info['total_size']:
                raise ValueError(f"文件大小不匹配: 期望 {upload_info['total_size']}, 实际 {actual_size}")
            ...
            if upload_info['file_hash']:
                ...
                if actual_hash != upload_info['file_hash']:
                    raise ValueError(f"文件哈希不匹配: 期望 {upload_info['file_hash']}, 实际 {actual_hash}")
    ...
    def get_upload_progress(self, upload_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'progress': len(upload_info['chunks_received']) / upload_info['total_chunks'],
    ...
    async def cleanup_expired_sessions(self, max_age_hours: int = 24):
        ...
        expired_sessions = [
            upload_id for upload_id, info in self.uploads.items()
...
async def validate_uploaded_file(file_path: Union[str, Path], 
                               max_size: Optional[int] = None) -> Tuple[bool, List[str]]:
    """
    验证上传的文件
