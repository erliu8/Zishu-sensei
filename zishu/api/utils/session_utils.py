...
class SessionManager:
    ...
    def validate_session(
        ...
        if not self.config.allow_session_sharing and session.ip_address != ip_address:
            logger.warning(f"会话IP不匹配: {session_id}, 期望 {session.ip_address}, 实际 {ip_address}")
    ...
    def get_session_stats(self, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if user_id:
            ...
            return {
                ...
                "locations": list(set(s.location.get("country", "Unknown") if s.location else "Unknown" for s in user_sessions)),
            ...
            return {
                ...
                "device_types": dict(self._count_by_field(all_sessions, lambda s: s.device_type.value)),
                "status_distribution": dict(self._count_by_field(all_sessions, lambda s: s.status.value)),
    ...
    def cleanup_expired_sessions(self) -> int:
        ...
        for session_id, session in self._sessions.items():
            if self._is_session_expired(session) or session.status != SessionStatus.ACTIVE:
    ...
    def _enforce_session_limits(self, user_id: str):
        ...
        for session_id in user_session_ids:
            session = self._sessions.get(session_id)
            if session and session.status == SessionStatus.ACTIVE and not self._is_session_expired(session):
        ...
        if len(active_sessions) >= self.config.max_concurrent_sessions:
            ...
            sessions_to_terminate = active_sessions[:-self.config.max_concurrent_sessions + 1]
    ...
    def _count_by_field(self, sessions: List[SessionInfo], field_func) -> Dict[str, int]:
        """按字段统计"""
        counts = {}
