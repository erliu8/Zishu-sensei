...
def require_auth(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            # 执行授权检查
            await _authorize_request(user_context, permissions, roles, security_level)
...
def audit_log(action: str, resource: Optional[str] = None, sensitive: bool = False):
    """
    审计日志装饰器
...
def validate_input(schema: Optional[Any] = None, sanitize: bool = True):
    """
    输入验证装饰器
...
def cache_response(
    ttl: int = 300, key_func: Optional[Callable] = None, vary_on_user: bool = True
):
    """
    响应缓存装饰器
...
async def _authenticate_request(
    request: Request, allow_api_key: bool = False, require_verified: bool = False
) -> Dict[str, Any]:
    """认证请求"""
    # 获取认证头
...
async def _validate_jwt_token(
    request: Request, token: str, require_verified: bool = False
) -> Dict[str, Any]:
    """验证JWT令牌"""
    security_manager = get_security_manager()
    if not security_manager:
...
def secure_endpoint(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        if permissions or roles:
            wrapped_func = require_auth(permissions=permissions, roles=roles)(
                wrapped_func
            )
