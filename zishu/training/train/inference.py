...
class InferenceEngine:
    ...
    def __init__(
        self, config: Optional[ConfigManager] = None, testing_mode: bool = False
    ):
        """
        初始化推理引擎
        Args:
    ...
    def load_model(self, model_id: str) -> bool:
        ...
        try:
            ...
            self.performance_monitor.record_response_time(
                "model", f"load_{model_id}", elapsed_time
            )
        ...
        try:
            ...
            if stream:
                return self._generate_stream(
                    model, tokenizer, input_ids, attention_mask, gen_params
                )
            # 批量生成
            with torch.no_grad():
    ...
    def _generate_stream(self, model, tokenizer, input_ids, attention_mask, gen_params):
        """
        流式文本生成
    ...
    def batch_generate(
        self, prompts: List[str], model_id: Optional[str] = None, **kwargs
    ) -> List[str]:
        """
        批量生成文本
        ...
        try:
            ...
            avg_sequence_length = sum(
                len(output) - input_lengths[i] for i, output in enumerate(outputs)
            ) / len(outputs)
            self.performance_monitor.record_model_metrics(
    ...
    def teacher_ensemble_generate(
        ...
        **kwargs,
        ...
        for theacher_id in theacher_ids:
            try:
                response = self.generate(prompts, model_id=theacher_id, **kwargs)
                responses.append(response)
            except Exception as e:
        ...
        if ensemble_method == "weighted_choice":
            ...
            selected_idx = random.choices(range(len(responses)), weights=weights, k=1)[
                0
            ]
            ...
            for response in responses:
                ...
                length_score = 1.0 - min(
                    abs(len(words) - target_length) / target_length, 1.0
                )
            ...
            if detected_tasks:
                ...
                selected_idx = random.choices(
                    range(len(responses)), weights=weights, k=1
                )[0]
                return responses[selected_idx]
    ...
    def chat_generate(
        ...
        **kwargs,
        ...
        try:
            ...
            if character_id:
                try:
                    system_prompt = self.prompt_manager.create_character_prompt(
                        character_id
                    )
                except Exception as e:
            ...
            formatted_prompt = self._format_chat_prompt(
                messages, system_prompt, model_id
            )
            ...
            # 生成响应
            response = self.generate(formatted_prompt, model_id=model_id, **kwargs)
    ...
    def _generate_test_response(
        self, messages: List[Dict[str, str]], character_id: Optional[str] = None
    ) -> str:
        """生成测试响应"""
        if not messages:
