...
class Live2DAppearance:
    ...
    def __init__(
        ...
        Args:
            ...
            log_file: 日志文件路径
        """
        self.config = config or Live2DAppearanceConfig(
            name="default", model_path="hiyori_vts/hiyori.model3.json"
        )
        self._character_config = {}
        self._desktop_config = {}

        self._logger = setup_logger(
            f"live2d.{self.config.name}", level=log_level or "INFO", log_file=log_file
        )
        ...
        def _update_from_configs(self) -> None:
            ...
            if self._character_config:
                ...
                if appearance:
                    ...
                    self.config.enable_physics = appearance.get(
                        "enable_physics", self.config.enable_physics
                    )
                    self.config.enable_breath = appearance.get(
                        "enable_breath", self.config.enable_breath
                    )
                    self.config.enable_eye_blink = appearance.get(
                        "enable_eye_blink", self.config.enable_eye_blink
                    )
                    self.config.default_expression = appearance.get(
                        "default_expression", self.config.default_expression
                    )
                    self.config.accessory_settings = appearance.get(
                        "accessory_settings", self.config.accessory_settings
                    )
            ...
            if self._desktop_config:
                ...
                if animation:
                    self.config.motion_fade_duration = animation.get(
                        "transition_time", self.config.motion_fade_duration
                    )
        ...
        def initialize(self, view_instance: t.Any = None) -> bool:
            ...
            try:
                ...
                # 如果需要显示问候动画
                if self._desktop_config.get("startup", {}).get(
        ...
        def _start_update_loop(self) -> None:
            ...
            # 启用空闲动作线程
            idle_interval = self._desktop_config.get("animation", {}).get(
                "idle_interval", 5
            )
            self._idle_task_id = self._thread_factory.submit_task(
                self._idle_motion_loop, idle_interval
            )
            ...
            def handle_interaction(
                self, event: str, event_data: t.Dict[str, t.Any]
            ) -> bool:
                """处理用户与Live2D模型的交互事件"""
