                ...
                metrics.append(
                    {
                        ...
                        "power_usage": torch.cuda.power_usage(i)
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        if self.metrics["response_time"]:
            ...
            summary["response_time_by_module"] = {
                moudle: sum(times) / len(times) for moudle, times in by_module.items()
            }
        ...
        if self.metrics["gpu"]:
            ...
            summary["gpu_usage_by_device"] = {
                name: sum(usages) / len(usages) for name, usages in by_gpu.items()
            }

        return summary

    def benchmark_attention_performance(
        model, seq_length=512, batch_size=4, num_runs=10
    ):
        """
        测试模型注意力机制的性能
