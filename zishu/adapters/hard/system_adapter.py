...
class SystemAdapter(BaseAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.logger.info(
            f"SystemAdapter initialized with safety config: {self.safety_config}"
        )
    ...
    def _validate_security(
        self, request: SystemRequest, context: ExecutionContext
    ) -> None:
        """验证操作安全性"""
        ...
        if (
            operation
            in [
                SystemOperation.PROCESS_KILL,
                SystemOperation.COMMAND_EXEC,
                SystemOperation.SHELL_EXEC,
            ]
        ...
        if operation in [SystemOperation.COMMAND_EXEC, SystemOperation.SHELL_EXEC]:
            ...
            if any(
                ...
                raise AdapterSecurityError(
                    ...
                    context={
                        "command": command,
                        "forbidden_commands": self.safety_config.forbidden_commands,
                    },
                )
        ...
        if operation == SystemOperation.PROCESS_KILL:
            ...
            if any(
                ...
                for protected in self.safety_config.protected_processes
            ):
                raise AdapterSecurityError(
                    ...
                    context={
                        ...
                        "protected_processes": self.safety_config.protected_processes,
                    },
                )
            ...
            if var_name in self.safety_config.protected_env_vars:
                raise AdapterSecurityError(
                    ...
                    context={
                        ...
                        "protected_vars": self.safety_config.protected_env_vars,
                    },
                )
    ...
    async def _get_process_info(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if pid:
                ...
                if not found_procs:
                    ...
                    return {
                        ...
                        "processes": [
                            {"pid": p.pid, "name": p.name()} for p in found_procs
                        ],
            ...
            raise AdapterSecurityError(
                f"Access denied to process {pid or process_name}"
            )
        except Exception as e:
        ...
        try:
            if pid:
                ...
                if not found_procs:
                    ...
                    raise AdapterValidationError(
                        f"Multiple processes found with name '{process_name}'. Use PID instead."
                    )
            ...
            raise AdapterSecurityError(
                f"Access denied to kill process {pid or process_name}"
            )
        except Exception as e:
    ...
    async def _get_network_info(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if include_io:
                ...
                result["network_io"] = {
                    interface: stats._asdict() for interface, stats in net_io.items()
                }
    ...
    async def _delete_environment_variable(
        self, params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """删除环境变量"""
        import os
    ...
    async def _get_service_status(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise AdapterExecutionError(
                f"Service status check timed out for: {service_name}"
            )
        except FileNotFoundError:
            raise AdapterExecutionError(
                "systemctl not found. Service management not available."
            )
        except Exception as e:
    ...
    async def _restart_service(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise AdapterExecutionError(
                f"Service restart timed out for: {service_name}"
            )
        except FileNotFoundError:
            raise AdapterExecutionError(
                "systemctl not found. Service management not available."
    ...
    async def _monitor_resources(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 计算统计信息
            cpu_values = [
                s["cpu_percent"] for s in samples if s["cpu_percent"] is not None
            ]
