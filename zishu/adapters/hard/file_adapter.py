...
from ..base.metadata import (
    ...
    SecurityLevel, CapabilityCategory, AdapterPermissions, AdapterConfiguration
)
from ..base.exceptions import (
    BaseAdapterException, AdapterConfigurationError, AdapterExecutionError,
    AdapterValidationError, ErrorCode, ExceptionSeverity
)
...
class FileOperation:
    ...
    def __post_init__(self):
        """验证操作参数"""
        valid_operations = {
            'read', 'write', 'append', 'delete', 'move', 'copy', 
            'list', 'exists', 'stat', 'mkdir', 'rmdir', 'chmod'
        }
    ...
    def __post_init__(self):
        ...
        if not self.forbidden_extensions:
            self.forbidden_extensions = {
                '.exe', '.bat', '.cmd', '.ps1', '.sh', '.scr', '.com',
                '.pif', '.vbs', '.js', '.jar', '.app', '.dmg', '.pkg'
            }
...
class FileSystemAdapter(BaseAdapter):
        ...
        return [
                ...
                examples=[
                    {
                        ...
                        "output": {"content": "Hello World", "size": 11, "encoding": "utf-8"}
                    }
                ],
                tags={'read', 'file', 'text'}
            ),
            ...
            AdapterCapability(
                ...
                input_schema={
                    ...
                    "properties": {
                        "operation": {
                            "type": "string", 
                            "enum": ["delete", "move", "copy", "mkdir", "rmdir", "chmod"]
                        },
                        "path": {"type": "string"},
                        "target_path": {"type": "string", "description": "目标路径（移动、复制时需要）"},
                        "permissions": {"type": "string", "description": "权限设置（chmod时需要）"}
                    },
                    "required": ["operation", "path"]
                },
            ...
            AdapterCapability(
                ...
                input_schema={
                    "type": "object",
                    "properties": {
                        "operation": {"type": "string", "enum": ["exists", "stat", "list"]},
                        "path": {"type": "string"},
        ...
        try:
            ...
            if self.safety_config.enable_backup:
                ...
                if not backup_path.exists():
                    ...
                    recommendations.append("Create backup directory or disable backup functionality")
    ...
    async def _cleanup_impl(self) -> None:
        ...
        try:
            ...
            if self.active_operations:
                self.logger.info(f"Waiting for {len(self.active_operations)} active operations to complete")
                ...
                if self.active_operations:
                    self.logger.warning(f"Forced cleanup with {len(self.active_operations)} active operations")
    ...
    async def _execute_file_operation(self, operation: FileOperation, context: ExecutionContext) -> Dict[str, Any]:
        """执行具体的文件操作"""
    ...
    async def _read_file(self, operation: FileOperation, context: ExecutionContext) -> Dict[str, Any]:
        """读取文件"""
        try:
    ...
    async def _write_file(self, operation: FileOperation, context: ExecutionContext) -> Dict[str, Any]:
        """写入文件"""
        try:
            path = Path(operation.path)
            backup_path = None
            ...
            # 写入文件
            content_bytes = operation.content.encode(operation.encoding) if isinstance(operation.content, str) else operation.content
    ...
    async def _append_file(self, operation: FileOperation, context: ExecutionContext) -> Dict[str, Any]:
        """追加文件内容"""
        try:
            path = Path(operation.path)
    ...
    async def _delete_file(self, operation: FileOperation, context: ExecutionContext) -> Dict[str, Any]:
        """删除文件或目录"""
        try:
            path = Path(operation.path)
    ...
    async def _list_directory(self, operation: FileOperation, context: ExecutionContext) -> Dict[str, Any]:
        """列出目录内容"""
        try:
            path = Path(operation.path)
            ...
            for item in path.iterdir():
                try:
                    ...
                    file_info = {
                        ...
                        'permissions': oct(stat_info.st_mode)[-3:] if hasattr(stat_info, 'st_mode') else 'unknown'
                    }
    ...
    async def _check_exists(self, operation: FileOperation, context: ExecutionContext) -> Dict[str, Any]:
        """检查文件或目录是否存在"""
        path = Path(operation.path)
        exists = path.exists()
    ...
    async def _get_file_stat(self, operation: FileOperation, context: ExecutionContext) -> Dict[str, Any]:
        """获取文件统计信息"""
        try:
            path = Path(operation.path)
            ...
            return {
                ...
                'is_symlink': path.is_symlink(),
                'permissions': oct(stat_info.st_mode)[-3:] if hasattr(stat_info, 'st_mode') else 'unknown',
                'owner': stat_info.st_uid if hasattr(stat_info, 'st_uid') else 'unknown',
                'group': stat_info.st_gid if hasattr(stat_info, 'st_gid') else 'unknown',
    ...
    async def _create_directory(self, operation: FileOperation, context: ExecutionContext) -> Dict[str, Any]:
        """创建目录"""
        try:
            path = Path(operation.path)
    ...
    async def _move_file(self, operation: FileOperation, context: ExecutionContext) -> Dict[str, Any]:
        """移动文件或目录"""
        try:
    ...
    async def _copy_file(self, operation: FileOperation, context: ExecutionContext) -> Dict[str, Any]:
        """复制文件或目录"""
        try:
            source_path = Path(operation.path)
            target_path = Path(operation.target_path)
    ...
    async def _remove_directory(self, operation: FileOperation, context: ExecutionContext) -> Dict[str, Any]:
        """删除目录"""
        try:
            path = Path(operation.path)
            
            if not path.exists():
                return {'removed': False, 'reason': 'Directory not found', 'path': str(path)}
    ...
    async def _change_permissions(self, operation: FileOperation, context: ExecutionContext) -> Dict[str, Any]:
        """修改文件权限"""
        try:
            path = Path(operation.path)
    ...
    async def _security_check(self, operation: FileOperation, context: ExecutionContext) -> None:
        """执行安全检查"""
        # 路径安全检查
        await self._validate_path(operation.path, self._get_operation_access_type(operation.operation))
    ...
    def _get_operation_access_type(self, operation: str) -> str:
        ...
        write_operations = {'write', 'append', 'delete', 'move', 'copy', 'mkdir', 'rmdir', 'chmod'}
        ...
        if self.safety_config.allowed_paths:
            ...
            if not allowed:
                raise AdapterValidationError(
                    ...
                    error_code=ErrorCode.SECURITY_VIOLATION,
                    adapter_id=self.adapter_id,
                    context={"path": str(path), "allowed_paths": self.safety_config.allowed_paths}
                )
    ...
    def _validate_file_extension(self, path: str) -> None:
        ...
        # 检查允许扩展名（如果配置了）
        if self.safety_config.allowed_extensions and extension not in self.safety_config.allowed_extensions:
            raise AdapterValidationError(
                f"File extension not allowed: {extension}",
                error_code=ErrorCode.SECURITY_VIOLATION,
                adapter_id=self.adapter_id,
                context={"path": path, "extension": extension, "allowed": list(self.safety_config.allowed_extensions)}
            )
