...
try:
    ...
    logging.warning(
        "aiofiles/aiohttp not available, some async operations may be limited"
    )
...
class ResourceManager:
    ...
    async def _check_limits(self):
        ...
        if (
            ...
            and latest_usage["process_memory_mb"] > self.limits.max_memory_mb
        ):
            alerts.append(
                f"Memory usage {latest_usage['process_memory_mb']:.1f}MB exceeds limit {self.limits.max_memory_mb}MB"
            )

        # 检查CPU限制
        if (
            self.limits.max_cpu_percent
            and latest_usage["process_cpu_percent"] > self.limits.max_cpu_percent
        ):
            alerts.append(
                f"CPU usage {latest_usage['process_cpu_percent']:.1f}% exceeds limit {self.limits.max_cpu_percent}%"
            )
    ...
    def check_resource_availability(
        ...
        required_cpu_percent: Optional[float] = None,
    ) -> Tuple[bool, List[str]]:
        """检查资源可用性"""
        issues = []
        ...
        if required_memory_mb:
            available_memory = (
                self.limits.max_memory_mb or float("inf")
            ) - current_usage.get("process_memory_mb", 0)
            if available_memory < required_memory_mb:
                issues.append(
                    f"Insufficient memory: need {required_memory_mb}MB, available {available_memory:.1f}MB"
                )

        # 检查CPU可用性
        if required_cpu_percent:
            available_cpu = (self.limits.max_cpu_percent or 100) - current_usage.get(
                "process_cpu_percent", 0
            )
            if available_cpu < required_cpu_percent:
                issues.append(
                    f"Insufficient CPU: need {required_cpu_percent}%, available {available_cpu:.1f}%"
                )

        return len(issues) == 0, issues

    async def execute_with_monitoring(
        self, coro_or_func: Callable, *args, **kwargs
    ) -> Any:
        """在资源监控下执行函数或协程"""
        ...
        try:
            if asyncio.iscoroutinefunction(coro_or_func):
                ...
                result = await loop.run_in_executor(
                    self._thread_pool, coro_or_func, *args, **kwargs
                )
            ...
            execution_stats = {
                ...
                "memory_delta_mb": end_usage.get("process_memory_mb", 0)
                - start_usage.get("process_memory_mb", 0),
...
class CommandExecutor:
    ...
    async def execute_command(
        ...
        operation_result = OperationResult(
            operation_id=context.operation_id, success=False
        )
        ...
        try:
            ...
            if context.execution_mode == ExecutionMode.ASYNCHRONOUS:
                ...
                result = await self._execute_background(
                    processed_command, context, exec_env
                )
            else:
    ...
    def _validate_command(
        self, command: Union[str, List[str]], context: OperationContext
    ) -> bool:
        """验证命令安全性"""
        ...
        if context.permissions:
            ...
            if not all(perm in context.permissions for perm in required_perms):
                self.logger.warning(
                    f"Insufficient permissions for command: {command_str}"
                )
                return False
    ...
    def _get_command_permissions(self, command: str) -> List[str]:
        ...
        if any(
            keyword in command.lower() for keyword in ["rm", "del", "delete", "unlink"]
        ):
    ...
    async def _execute_sync(
        ...
        try:
            ...
            return OperationResult(
                operation_id=context.operation_id, success=False, error_message=str(e)
            )
    ...
    async def _execute_background(
        self, command: List[str], context: OperationContext, env: Dict[str, str]
    ) -> OperationResult:
        """后台执行命令"""
        try:
            # 创建后台进程
...
class HardAdapter(BaseAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.security_policy = SecurityPolicy(
            config.get("security_policy", SecurityPolicy.MODERATE)
        )
        ...
        self.allowed_operations = set(
            config.get("allowed_operations", self.SUPPORTED_OPERATIONS)
        )
        ...
        self.logger.info(
            f"HardAdapter initialized with security policy: {self.security_policy}"
        )
    ...
    async def _initialize_impl(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"System info: {self.system_info.platform}, "
            ...
            if SECURITY_AVAILABLE:
                ...
                self.logger.warning(
                    "Security modules not available, running with reduced security"
                )
    ...
    def _parse_operation_request(self, input_data: Any) -> Dict[str, Any]:
        ...
        if isinstance(input_data, dict):
            ...
            try:
                ...
                # 作为简单命令处理
                return {"operation_type": "system_command", "command": input_data}
        else:
    ...
    def _create_operation_context(
        self, operation_request: Dict[str, Any], execution_context: ExecutionContext
    ) -> OperationContext:
        """创建操作上下文"""
        return OperationContext(
            operation_type=OperationType(
                operation_request.get("operation_type", "system_command")
            ),
            execution_mode=ExecutionMode(
                operation_request.get("execution_mode", "sync")
            ),
            security_policy=self.security_policy,
            timeout_seconds=operation_request.get(
                "timeout_seconds", execution_context.timeout
            ),
            user_id=execution_context.user_id,
            environment=operation_request.get("environment"),
            working_directory=operation_request.get(
    ...
    async def _validate_operation_permissions(
        ...
        if self.permission_manager:
            ...
            if not self.permission_manager.check_permission(
                "hard_adapter_execute", permission_context
            ):
                raise PermissionDeniedError(
    ...
    async def _check_resource_availability(self, operation_context: OperationContext):
        ...
        # 估算操作所需资源
        estimated_memory = (
            operation_context.resource_limits.get("memory_mb", 64)
            if operation_context.resource_limits
            else 64
        )
        estimated_cpu = (
            operation_context.resource_limits.get("cpu_percent", 20)
            if operation_context.resource_limits
            else 20
        )

        available, issues = self.resource_manager.check_resource_availability(
            required_memory_mb=estimated_memory, required_cpu_percent=estimated_cpu
        )
    ...
    async def _health_check_impl(self) -> HealthCheckResult:
        ...
        try:
            ...
            if self.resource_manager:
                ...
                if current_usage:
                    metrics["current_memory_mb"] = current_usage.get(
                        "process_memory_mb", 0
                    )
                    metrics["current_cpu_percent"] = current_usage.get(
                        "process_cpu_percent", 0
                    )
                    ...
                    if (
                        current_usage.get("process_memory_mb", 0)
                        > (self.resource_limits.max_memory_mb or 1024) * 0.9
                    ):
            ...
            if SECURITY_AVAILABLE:
                ...
                recommendations.append(
                    "Install security dependencies for enhanced security"
                )
            ...
            if self.temp_directory.exists() and self.temp_directory.is_dir():
                ...
                issues.append(
                    f"Temporary directory not accessible: {self.temp_directory}"
                )
    ...
    async def execute_command(
        ...
        return (
            ...
            if isinstance(result, OperationResult)
                ...
                operation_id=context.execution_id, success=True, output_data=result
            )
        )
    ...
    async def _execute_system_command(
        ...
        if not self.command_executor:
            raise SystemOperationError(
                "Command executor not available", adapter_id=self.adapter_id
            )
        ...
        return await self.command_executor.execute_command(
            command=command, context=operation_context, capture_output=True
        )
...
async def example_usage():
    ...
    try:
        ...
        success = await adapter.write_file(
            test_file, "Hello from Zishu-sensei HardAdapter!\n测试中文内容。"
        )
        if success:
