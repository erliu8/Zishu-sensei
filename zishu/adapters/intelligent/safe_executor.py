...
class ResourceLimits:
    ...
    blocked_syscalls: List[str] = field(default_factory=lambda: [
        'exec', 'fork', 'clone', 'ptrace', 'mount', 'unmount'
    ])
...
class SecurityScanner:
    ...
    def _compile_patterns(self):
        ...
        self.dangerous_patterns = {
            ...
            'file_operations': re.compile(r'\b(open|file|read|write|remove|delete)\s*\('),
            'network_operations': re.compile(r'\b(socket|urllib|requests|http)\s*\('),
            'process_operations': re.compile(r'\b(subprocess|multiprocessing|threading)\s*\('),
            'import_statements': re.compile(r'^\s*(?:from\s+\S+\s+)?import\s+(.+)$', re.MULTILINE),
        }
    ...
    async def scan_code(self, code: str, language: CodeLanguage) -> SecurityScanResult:
        ...
        try:
            ...
            logger.info(f"Security scan completed: risk_score={result.risk_score:.2f}, "
    ...
    async def _perform_static_analysis(self, code: str, language: CodeLanguage, result: SecurityScanResult):
        """执行静态分析"""
        try:
                ...
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        if alias.name in self.config.dangerous_imports:
                            ...
                            result.threats_detected.append(f"Dangerous import: {alias.name}")
                
                elif isinstance(node, ast.ImportFrom):
                    if node.module:
                        module_name = node.module
                        if any(dangerous in module_name for dangerous in self.config.dangerous_imports):
                            result.dangerous_imports.append(module_name)
                            result.threats_detected.append(f"Dangerous module import: {module_name}")
                    ...
                    if isinstance(node.func, ast.Name):
                        ...
                        if func_name in self.config.dangerous_functions:
                            ...
                            result.threats_detected.append(f"Dangerous function call: {func_name}")
    ...
    async def _analyze_javascript_code(self, code: str, result: SecurityScanResult):
        ...
        for pattern in dangerous_js_patterns:
            if pattern in code:
                ...
                result.threats_detected.append(f"Dangerous JavaScript pattern: {pattern}")
    ...
    async def _perform_dynamic_analysis(self, code: str, language: CodeLanguage, result: SecurityScanResult):
        """执行动态分析（基于模式匹配）"""
        # 检查危险关键词
        ...
        if self.config.block_network:
            network_indicators = ['http://', 'https://', 'ftp://', 'socket', 'urllib', 'requests']
            for indicator in network_indicators:
                if indicator in code.lower():
                    result.threats_detected.append(f"Network access detected: {indicator}")
...
class SandboxManager:
    ...
    def __init__(self, base_path: Union[str, Path]):
        ...
        logger.info(f"Sandbox manager initialized. Docker available: {self.docker_available}")
    ...
    async def create_sandbox(self, request: ExecutionRequest) -> 'Sandbox':
        ...
        try:
            ...
            # 选择沙盒实现
            if request.environment == ExecutionEnvironment.CONTAINER and self.docker_available:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.container = self.client.containers.create(
                ...
                cpu_quota=int(self.request.cpu_limit * 100000) if self.request.cpu_limit else None,
                detach=True,
                remove=True,
...
class SafeCodeExecutor:
    ...
    async def execute_code(self, request: ExecutionRequest) -> ExecutionResult:
        ...
        try:
            ...
            try:
                ...
                # 5. 记录审计日志
                self.audit_logger.info(f"Code execution completed", extra={
                    'request_id': request.request_id,
    ...
    async def batch_execute(self, requests: List[ExecutionRequest]) -> List[ExecutionResult]:
        """批量执行代码"""
        results = []
