...
from ..base.exceptions import (
    LearningEngineError, ModelLoadingError, BaseAdapterException, 
...
class LearningDataManager:
    ...
    async def get_data_by_type(self, data_type: DataType, limit: int = 100) -> List[LearningData]:
        """按类型获取数据"""
        data_ids = self.data_by_type.get(data_type, [])[:limit]
        return [self.data_index[data_id] for data_id in data_ids if data_id in self.data_index]
    
    async def get_high_quality_data(self, min_quality: float = 0.8, limit: int = 100) -> List[LearningData]:
        """获取高质量数据"""
        high_quality_data = []
        for quality_score, data_ids in self.data_by_quality.items():
            if quality_score >= min_quality:
                high_quality_data.extend([
                    self.data_index[data_id] for data_id in data_ids 
...
class LearningEngineAdapter:
    ...
    async def collect_code_feedback(self, 
                                   session_id: str,
                                   generated_code: str,
                                   user_feedback: str,
        ...
        try:
            ...
            feedback_data = LearningData(
                ...
                quality_score=0.8 if execution_result and execution_result.get('success') else 0.5
            )
    ...
    async def collect_execution_result(self,
                                     session_id: str, 
                                     code: str,
    ...
    async def request_model_improvement(self, 
                                      session_id: str,
                                      improvement_type: str = "incremental") -> Optional[str]:
        """请求模型改进"""
        try:
            ...
            if len(session.collected_data) < 5:
                logger.info(f"Not enough data for improvement: {len(session.collected_data)} samples")
                return None
        ...
        try:
            ...
            insights = {
                ...
                'learning_progress': {
                    ...
                    'next_improvement_eta': '10 samples' if len(session.collected_data) % 10 > 5 else 'ready'
                }
            }
