...
class SystemMonitor:
    ...
    def collect_system_metrics(self) -> SystemSnapshot:
        ...
        try:
            ...
            return SystemSnapshot(
                timestamp=datetime.now(), cpu_percent=0, memory_percent=0, disk_usage=0
            )
    ...
    def _record_system_metrics(self, snapshot: SystemSnapshot) -> None:
        ...
        metrics = [
            ...
            PerformanceMetric(
                "system.process.count", snapshot.process_count, MetricType.GAUGE
            ),
            PerformanceMetric(
                "system.thread.count", snapshot.thread_count, MetricType.GAUGE
            ),
        ]
...
class AdapterMonitor:
    ...
    def monitor_execution(
        self, adapter_name: str, metadata: Optional[Dict[str, Any]] = None
    ):
        """监控适配器执行的上下文管理器"""
        ...
        try:
            ...
            cpu_usage = (
                (end_cpu_time - start_cpu_time) / execution_time * 100
                if execution_time > 0
                else 0
            )
    ...
    async def monitor_async_execution(
        self, adapter_name: str, metadata: Optional[Dict[str, Any]] = None
    ):
        """异步适配器执行监控的上下文管理器"""
        execution_id = str(uuid.uuid4())
    ...
    def _record_adapter_metrics(self, perf_info: AdapterPerformanceInfo) -> None:
        """记录适配器性能指标"""
        base_labels = {
            "adapter": perf_info.adapter_name,
            "success": str(perf_info.success),
        }
        ...
        if not perf_info.success:
            metrics.append(
                PerformanceMetric(
                    ...
                    labels={
                        **base_labels,
                        "error": perf_info.error_message or "unknown",
                    },
                    resource_type=ResourceType.ADAPTER,
                )
            )
    ...
    def get_adapter_statistics(
        self, adapter_name: str, hours: int = 24
    ) -> Dict[str, Any]:
        """获取适配器统计信息"""
        execution_stats = self.collector.get_statistics(
            f"adapter.{adapter_name}.execution_time", hours
        )
        memory_stats = self.collector.get_statistics(
            f"adapter.{adapter_name}.memory_used", hours
        )

        # 计算成功率
        execution_history = self.collector.get_history(
            f"adapter.{adapter_name}.executions", hours
        )
        error_history = self.collector.get_history(
            f"adapter.{adapter_name}.errors", hours
        )
...
class PerformanceMonitor:
    ...
    def __init__(
        ...
        logger.info(
            f"性能监控器初始化完成 - GPU监控: {'启用' if self.enable_gpu_monitoring else '禁用'}"
        )
        ...
        def decorator(func):
            ...
            if asyncio.iscoroutinefunction(func):
                ...
                async def async_wrapper(*args, **kwargs):
                    async with await self.adapter_monitor.monitor_async_execution(
    ...
    def measure_time(self, metric_name: str, labels: Optional[Dict[str, str]] = None):
        ...
        try:
            ...
            self.collector.record(
                PerformanceMetric(
                    metric_name, execution_time, MetricType.TIMER, labels=labels or {}
                )
            )
    ...
    def measure_memory(self, metric_name: str, labels: Optional[Dict[str, str]] = None):
        ...
        try:
            ...
            self.collector.record(
                PerformanceMetric(
                    metric_name, memory_delta, MetricType.GAUGE, labels=labels or {}
                )
            )
...
if __name__ == "__main__":
    ...
    monitor = create_performance_monitor(
        {"collection_interval": 30, "retention_hours": 48}  # 30秒收集一次  # 保留48小时数据
    )
