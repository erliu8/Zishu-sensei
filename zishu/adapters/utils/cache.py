...
class TTLStrategy(CacheEvictionStrategy):
    ...
    def should_evict(self, entries: Dict[str, CacheEntry], max_size: int) -> List[str]:
        ...
        if len(entries) - len(expired_keys) > max_size:
            ...
            sorted_keys = sorted(remaining_entries.keys(), 
                               key=lambda k: remaining_entries[k].created_at + (remaining_entries[k].ttl or 0))
            ...
            if meta_path.exists():
                try:
                    async with aiofiles.open(meta_path, 'r') as f:
                        ...
                        if 'expires_at' in meta_data and time.time() > meta_data['expires_at']:
                            await self.delete(key)
                            return None
                except Exception as e:
...
try:
    ...
    class RedisBackend(CacheBackend):
        ...
        def __init__(self, redis_url: str = "redis://localhost:6379", db: int = 0, prefix: str = "cache:"):
        ...
        async def _get_redis(self) -> aioredis.Redis:
            ...
            if self._redis is None:
                self._redis = aioredis.from_url(self.redis_url, db=self.db, decode_responses=False)
            return self._redis
...
class Cache:
    ...
    async def set(self, key: Any, value: Any, ttl: Optional[int] = None, tags: Optional[Set[str]] = None) -> bool:
        """设置缓存值"""
        cache_key = self._generate_cache_key(key)
        ...
        if self.config.async_enabled:
            ...
            with self._lock:
                ...
                asyncio.set_event_loop(loop)
                try:
                    return loop.run_until_complete(self._set_impl(cache_key, value, ttl, tags))
                finally:
                    loop.close()
    
    async def _set_impl(self, cache_key: str, value: Any, ttl: Optional[int], tags: Set[str]) -> bool:
        """设置实现"""
        try:
            # 序列化数据
...
def cached(cache: Optional[Cache] = None, ttl: Optional[int] = None, 
...
async def cache_set(key: Any, value: Any, ttl: Optional[int] = None, 
                   tags: Optional[Set[str]] = None, cache_name: str = "default") -> bool:
    """便捷设置缓存"""
    cache = get_cache(cache_name)
...
__all__ = [
    # 枚举
    'CacheStrategy', 'StorageBackend', 'SerializationType', 'CompressionType',
    # 数据类
    'CacheEntry', 'CacheStats', 'CacheConfig',
    # 核心类
    'Cache', 'CacheBackend', 'CacheSerializer', 'CacheCompressor', 'CacheEvictionStrategy',
    # 实现类
    'MemoryBackend', 'FileBackend', 'RedisBackend',
    'JsonSerializer', 'PickleSerializer', 'MsgPackSerializer',
    'NoCompressor', 'ZlibCompressor', 'GzipCompressor', 'Lz4Compressor',
    'LRUStrategy', 'LFUStrategy', 'FIFOStrategy', 'TTLStrategy',
    # 装饰器和工具函数
    'cached', 'cache_get', 'cache_set', 'cache_delete', 'cache_clear',
    'get_cache', 'get_default_cache', 'create_cache',
    'cache_transaction'
]
