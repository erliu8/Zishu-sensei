...
class ExampleTextAdapter(BaseAdapter):
    ...
    async def _health_check_impl(self) -> HealthCheckResult:
        ...
        return HealthCheckResult(
            ...
            message="Adapter is working properly"
            if is_healthy
            else "Adapter not initialized",
        )
...
async def basic_usage_example():
    ...
    try:
        ...
        logger.info(
            f"Total adapters: {status['registry_status']['registered_adapters']}"
        )
...
async def batch_operations_example():
    ...
    try:
        ...
        logger.info(
            f"Batch start results: {[(k, v.success) for k, v in start_results.items()]}"
        )
        ...
        logger.info(
            f"Batch stop results: {[(k, v.success) for k, v in stop_results.items()]}"
        )
...
async def monitoring_example():
    ...
    try:
        ...
        logger.info("性能报告:")
        logger.info(
            f"  总操作数: {performance_report['manager_metrics']['total_operations']}"
        )
        logger.info(
            f"  成功率: {performance_report['operation_performance']['success_rate']:.2f}"
        )
        logger.info(
            f"  平均执行时间: {performance_report['operation_performance']['avg_operation_time']:.3f}s"
        )
...
async def event_handling_example():
    ...
    def on_operation_completed(event):
        logger.info(
            f"✅ Operation completed: {event['data']['operation_type']} - Success: {event['data']['success']}"
        )
...
async def configuration_management_example():
    ...
    try:
        ...
        config_result = await manager.update_config(
            {"max_concurrent_operations": 10, "health_check_interval_seconds": 15}
        )
        ...
        if config_result.success:
            ...
            for field in config_result.results["updated_fields"]:
                logger.info(
                    f"  {field['field']}: {field['old_value']} -> {field['new_value']}"
                )
