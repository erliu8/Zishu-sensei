...
class TextValidatorAdapter(BaseAdapter):
    ...
    async def _health_check_impl(self) -> HealthCheckResult:
        return HealthCheckResult(
            is_healthy=True, status="healthy", checks={"config_valid": True}
        )
...
class TextAnalyzerAdapter(BaseAdapter):
    ...
    async def _process_impl(self, input_data: Any, context: ExecutionContext) -> Any:
        ...
        if not isinstance(input_data, str):
            return {"error": f"Expected string input, got {type(input_data).__name__}"}
    ...
    async def _health_check_impl(self) -> HealthCheckResult:
        ...
        return HealthCheckResult(is_healthy=True, status="healthy", checks=checks)
...
class RegistryEventListener:
    ...
    async def on_adapter_error(self, event: RegistryEvent):
        """适配器错误事件处理器"""
        self.logger.error(
            f"❌ Adapter error: {event.adapter_id} - {event.data.get('error', 'Unknown error')}"
        )
        self.events_received.append(event)
...
async def demonstrate_adapter_execution(registry: AdapterRegistry):
    ...
    for i, text in enumerate(test_texts, 1):
        ...
        if validator:
            ...
            logger.info(
                f"  🔍 Validation: {validation_result.output['valid']} - {validation_result.output['reason']}"
            )
        ...
        if analyzer:
            ...
            logger.info(
                f"  📊 Analysis: {analysis['word_count']} words, {analysis['character_count']} chars"
            )
...
async def demonstrate_event_system():
    ...
    registry.event_bus.subscribe(
        EventType.ADAPTER_REGISTERED, event_listener.on_adapter_registered
    )
    registry.event_bus.subscribe(
        EventType.ADAPTER_STARTED, event_listener.on_adapter_started
    )
    registry.event_bus.subscribe(
        EventType.ADAPTER_STOPPED, event_listener.on_adapter_stopped
    )
    registry.event_bus.subscribe(
        EventType.ADAPTER_ERROR, event_listener.on_adapter_error
    )
    registry.event_bus.subscribe(
        EventType.ADAPTER_HEALTH_CHANGED, event_listener.on_health_changed
    )
    ...
    try:
        ...
        for event in recent_events[-5:]:  # 显示最后5个事件
            logger.info(
                f"  {event.timestamp.strftime('%H:%M:%S')} - {event.event_type.value}: {event.adapter_id}"
            )
...
async def demonstrate_error_handling():
    ...
    try:
        ...
        try:
            ...
            await registry.register_adapter(
                "test_adapter", SampleAdapter, config
            )  # 重复注册
        ...
        registry_status = registry.get_registry_status()
        logger.info(
            f"📊 Error Statistics: {registry_status['stats']['total_errors']} total errors"
        )
