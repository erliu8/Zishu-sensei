...
class HelloWorldAdapter(BaseAdapter):
    ...
    # 支持的语言映射
    LANGUAGE_GREETINGS = {
        "zh": {"formal": "您好", "casual": "你好", "friendly": "嗨", "world": "世界"},
        "en": {
            "formal": "Good day",
            "casual": "Hello",
            "friendly": "Hi",
        ...
        "ja": {"formal": "こんにちは", "casual": "おはよう", "friendly": "やあ", "world": "世界"},
        "ko": {"formal": "안녕하세요", "casual": "안녕", "friendly": "하이", "world": "세계"},
        "fr": {
            "formal": "Bonjour",
            "casual": "Salut",
            "friendly": "Coucou",
    ...
    # 表情符号映射
    EMOJI_MAP = {"formal": "", "casual": "", "friendly": "", "world": "", "time": ""}
        ...
        try:
            ...
            if self.language not in self.LANGUAGE_GREETINGS:
                raise AdapterConfigurationError(
                    ...
                    adapter_id=self.adapter_id,
                    context={
                        "supported_languages": list(self.LANGUAGE_GREETINGS.keys())
                    },
                )
            ...
            self.logger.info(
                f"Hello World适配器初始化成功 - 风格: {self.greeting_style}, "
            ...
            if isinstance(e, (AdapterConfigurationError, AdapterValidationError)):
                ...
                raise AdapterConfigurationError(
                    f"初始化过程中发生意外错误: {str(e)}", adapter_id=self.adapter_id, cause=e
                )
    ...
    async def _process_impl(self, input_data: Any, context: ExecutionContext) -> Any:
        ...
        try:
            ...
            # 生成问候内容
            greeting_data = await self._generate_greeting(
                name, effective_config, context
            )

            # 格式化输出
            formatted_output = self._format_output(
                greeting_data, effective_config["output_format"]
            )
    ...
    def _merge_config(self, request_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in request_config.items():
            if key in config:
                if key == "greeting_style" and value in [
                    "formal",
                    "casual",
                    "friendly",
                ]:
                    ...
                    config[key] = value
                elif key == "output_format" and value in [
                    "text",
                    "json",
                    "xml",
                    "yaml",
    ...
    async def _generate_greeting(
        self, name: Optional[str], config: Dict[str, Any], context: ExecutionContext
    ) -> Dict[str, Any]:
        """生成问候内容"""
        # 获取语言设置
    ...
    def _get_capabilities_impl(self) -> List[AdapterCapability]:
        ...
        return [
            AdapterCapability(
                ...
                description="生成基础问候语",
                input_schema={"type": "string", "description": "可选的接收者名称"},
                output_schema={
                    "type": "object",
                    "properties": {
            ...
            AdapterCapability(
                ...
                input_schema={
                    ...
                    "properties": {
                        ...
                        "language": {
                            "type": "string",
                            "enum": list(self.LANGUAGE_GREETINGS.keys()),
                        },
                    },
                },
                output_schema={
    ...
    async def _health_check_impl(self) -> HealthCheckResult:
        ...
        try:
            ...
            checks["style_valid"] = self.greeting_style in [
                "formal",
                "casual",
                "friendly",
            ]
            checks["format_valid"] = self.output_format in [
                "text",
                "json",
                "xml",
                "yaml",
            ...
            if not checks["language_supported"]:
                ...
                recommendations.append(
                    "请使用支持的语言代码: " + ", ".join(self.LANGUAGE_GREETINGS.keys())
                )
        ...
        try:
            ...
            for lang in ["zh", "en"]:
                try:
                    ...
                    await self._generate_greeting(
                        "测试", {**self._merge_config({}), **lang_config}, lang_context
                    )
    ...
    async def _cleanup_impl(self) -> None:
        ...
        try:
            # 记录最终统计信息
            self.logger.info(
                f"Hello World适配器清理中 - 总问候次数: {self.greeting_count}, "
...
async def simple_usage_example():
    ...
    # 最简单的使用方式
    config = {"adapter_id": "simple_hello", "adapter_type": "soft"}
    ...
    async with HelloWorldAdapter(config) as adapter:
        ...
        result = await adapter.process(
            {"name": "Alice", "language": "en", "output_format": "json"}
        )
...
def quick_test():
    ...
    async def test():
        config = {"adapter_id": "quick_test", "adapter_type": "soft", "language": "zh"}
