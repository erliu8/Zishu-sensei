...
class MessageQueueAdapter(BaseAdapter):
    ...
    async def publish_message(
        ...
        try:
            ...
            logger.debug(f"Message published to queue {queue_name}: {wrapped_message['id']}")
            return True
            ...
            for i, subscriber in enumerate(subscribers):
                if subscriber['id'] == subscription_id:
                    ...
                    if consumer_group and consumer_group in self._consumer_groups:
                        try:
                            self._consumer_groups[consumer_group]['members'].remove(subscription_id)
                        except ValueError:
                            pass
                    
                    logger.info(f"Unsubscribed from queue {queue_name}: {subscription_id}")
                    return True
        ...
        while True:
            try:
                ...
                for queue_name, queue in self._queues.items():
                    ...
                    if expired_count > 0:
                        logger.debug(f"Cleaned {expired_count} expired messages from queue {queue_name}")
