...
async def main():
    ...
    manager = AdapterManager(
        {"event_queue_size": 1000, "auto_restart_on_health_failure": True}
    )
    ...
    try:
        ...
        http_adapter_id = await manager.register_adapter(
            HttpAdapter, http_identity, http_config
        )
        ...
        db_adapter_id = await manager.register_adapter(
            DatabaseAdapter, db_identity, db_config
        )
        ...
        subscription_id = manager.subscribe_event(
            [EventType.ADAPTER_STARTED, EventType.OPERATION_COMPLETED], event_handler
        )
        ...
        for adapter in adapters:
            logger.info(
                f"  - {adapter.name} ({adapter.adapter_type.value}): {adapter.status.value}"
            )
...
async def event_filtering_example():
    ...
    # 创建事件过滤器
    type_filter = EventTypeFilter(
        [EventType.ADAPTER_STARTED, EventType.ADAPTER_STOPPED]
    )
...
async def discovery_example():
    ...
    try:
        ...
        # 搜索适配器
        search_results = discovery_service.search_adapters(
            "http", fields=["name", "description"]
        )
