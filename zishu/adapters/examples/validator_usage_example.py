...
class GoodExampleAdapter(BaseAdapter):
        ...
        return AdapterMetadata(
            ...
            configuration=[
                AdapterConfiguration(
                    ...
                    validation_rules={"enum": ["upper", "lower", "reverse"]},
                ),
                AdapterConfiguration(
    ...
    async def _initialize_impl(self) -> bool:
        ...
        try:
            ...
            if self.default_operation not in valid_operations:
                self.logger.error(
                    f"Invalid default operation: {self.default_operation}"
                )
                return False
            ...
            self.logger.info(
                f"Good adapter initialized with operation={self.default_operation}"
            )
            return True
    ...
    async def _process_impl(self, input_data: Any, context: ExecutionContext) -> Any:
        ...
        if not isinstance(text, str):
            raise AdapterExecutionError(
                "Input 'text' field must be a string", adapter_id=self.adapter_id
            )
        ...
        if operation == "upper":
            ...
            raise AdapterExecutionError(
                f"Unknown operation: {operation}", adapter_id=self.adapter_id
            )
    ...
    async def _health_check_impl(self) -> HealthCheckResult:
        ...
        # 检查配置有效性
        checks["valid_operation"] = self.default_operation in [
            "upper",
            "lower",
            "reverse",
        ]
...
class BadExampleAdapter(BaseAdapter):
    ...
    def _load_metadata(self):  # 缺少返回类型注解
        ...
        return AdapterMetadata(
            ...
            dependencies=[
                AdapterDependency(
                    name="nonexistent_package", type="package", optional=False  # 不存在的包
                )
            ],
            permissions=AdapterPermissions(
...
async def demonstrate_different_validators():
    ...
    result = await dev_validator.validate_adapter(
        BadExampleAdapter, {"adapter_id": "bad"}
    )
    ...
    result = await prod_validator.validate_adapter(
        BadExampleAdapter, {"adapter_id": "bad"}
    )
    print(f"结果: {'有效' if result.is_valid else '无效'}")
    print(f"问题数量: {len(result.issues)}")
...
async def demonstrate_custom_validation():
    ...
    class TestProcessor(BaseAdapter):  # 不以Adapter结尾
        def _load_metadata(self):
            return AdapterMetadata(
                ...
                adapter_type=AdapterType.SOFT,
                version=AdapterVersion(
                    version="1.0.0", release_date=datetime.now(timezone.utc)
                ),
            )
    ...
    result = await custom_validator.validate_adapter(
        TestProcessor, {"adapter_id": "test"}
    )
...
async def demonstrate_performance_validation():
    ...
    class SlowAdapter(BaseAdapter):
        ...
        def _get_capabilities_impl(self):
            return [
                AdapterCapability(
                    name="slow", category=CapabilityCategory.TEXT_PROCESSING
                )
            ]
...
async def demonstrate_security_validation():
    ...
    class InsecureAdapter(BaseAdapter):
        ...
        def _get_capabilities_impl(self):
            return [
                AdapterCapability(
                    name="file_ops", category=CapabilityCategory.FILE_OPERATIONS
                )
            ]
...
async def demonstrate_quick_validation():
    ...
    print("\n快速验证有问题的适配器:")
    is_valid = await quick_validate(
        BadExampleAdapter, {"adapter_id": "bad_example"}, print_report=False
    )
    print(f"验证结果: {'通过' if is_valid else '失败'}")
