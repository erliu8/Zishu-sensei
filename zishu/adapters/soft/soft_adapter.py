...
class SoftAdapter(BaseAdapter):
    ...
    def _load_metadata(self) -> AdapterMetadata:
        ...
        return AdapterMetadata(
            ...
            configuration_schema={
                "type": "object",
                "properties": {
                    "knowledge_base": {"type": "object", "description": "知识库配置"},
                    "rag_engine": {"type": "object", "description": "RAG引擎配置"},
                    "prompt_engine": {"type": "object", "description": "提示引擎配置"},
                    "enable_caching": {
                        "type": "boolean",
                        "default": True,
                        "description": "是否启用缓存",
    ...
    async def _initialize_impl(self) -> bool:
        ...
        try:
            ...
            raise AdapterLoadingError(
                f"软适配器初始化失败: {str(e)}", adapter_id=self.adapter_id, cause=e
            )
    ...
    async def _health_check_impl(self) -> HealthCheckResult:
        ...
        try:
            ...
            with self._stats_lock:
                metrics.update(
                    {
                        "requests_processed": self._processing_stats[
                            "requests_processed"
                        ],
        ...
        try:
            ...
            if self.knowledge_base:
                cleanup_tasks.append(
                    self._cleanup_component(self.knowledge_base, "knowledge_base")
                )

            if self.rag_engine:
                cleanup_tasks.append(
                    self._cleanup_component(self.rag_engine, "rag_engine")
                )

            if self.prompt_engine:
                cleanup_tasks.append(
                    self._cleanup_component(self.prompt_engine, "prompt_engine")
                )
            ...
            raise AdapterExecutionError(
                f"软适配器清理失败: {str(e)}", adapter_id=self.adapter_id, cause=e
            )
    ...
    async def _initialize_components(self) -> bool:
        ...
        try:
            ...
            if (
                ...
                and self._get_component_status("knowledge_base").is_available
            ):
            ...
            if (
                ...
                and self._get_component_status("rag_engine").is_available
            ):
                initialization_tasks.append(self._initialize_rag_engine())

            # 初始化提示引擎
            if (
                PROMPT_ENGINE_AVAILABLE
                and self._get_component_status("prompt_engine").is_available
            ):
            ...
            initialized_components = [
                status.is_initialized for status in self._component_status.values()
            ]
    ...
    async def _initialize_knowledge_base(self) -> bool:
        ...
        try:
            ...
            raise KnowledgeBaseError(
                f"知识库初始化失败: {str(e)}", adapter_id=self.adapter_id, cause=e
            )
    ...
    async def _initialize_rag_engine(self) -> bool:
        ...
        try:
            ...
            # 如果RAG引擎有初始化方法，调用它
            if hasattr(self.rag_engine, "initialize") and callable(
            ...
            raise RAGEngineError(
                f"RAG引擎初始化失败: {str(e)}", adapter_id=self.adapter_id, cause=e
            )
    ...
    async def _initialize_prompt_engine(self) -> bool:
        ...
        try:
            ...
            # 如果提示引擎有初始化方法，调用它
            if hasattr(self.prompt_engine, "initialize") and callable(
                self.prompt_engine.initialize
            ):
            ...
            raise PromptTemplateError(
                f"提示引擎初始化失败: {str(e)}", adapter_id=self.adapter_id, cause=e
            )
    ...
    async def _setup_component_connections(self) -> None:
        ...
        try:
            ...
            if (
                self.knowledge_base
                and self.rag_engine
                and hasattr(self.rag_engine, "set_knowledge_base")
                and callable(self.rag_engine.set_knowledge_base)
            ):
            ...
            if (
                ...
                and hasattr(self.prompt_engine, "register_function")
                and callable(self.prompt_engine.register_function)
            ):

                async def kb_search_func(query: str, top_k: int = 10):
                    if hasattr(self.knowledge_base, "search") and callable(
                        self.knowledge_base.search
                    ):
                        return await self.knowledge_base.search(query, top_k=top_k)
                    return []

                self.prompt_engine.register_function(
                    "kb_search", kb_search_func, safe=True
                )
                ...
                for template_id, template_content in default_templates.items():
                    try:
                        if hasattr(self.prompt_engine, "load_template_from_string"):
                            await self.prompt_engine.load_template_from_string(
...
助手：""",
    ...
    def _get_component_status(self, component_name: str) -> ComponentStatus:
        """获取组件状态"""
        with self._components_lock:
            return self._component_status.get(
                component_name,
                ComponentStatus(
                    name=component_name, is_available=False, error_message="组件未初始化"
                ),
            )
        ...
        try:
            ...
            # 如果是字典，解析各个字段
                ...
                query = input_data.get(
                    "query", input_data.get("question", input_data.get("input", ""))
                )
                if not query:
            ...
            raise AdapterExecutionError(
                f"输入数据解析失败: {str(e)}", adapter_id=self.adapter_id, cause=e
            )

    async def _execute_processing(
        self, request: SoftAdapterRequest, context: ExecutionContext
    ) -> SoftAdapterResponse:
        """执行智能处理逻辑"""
        try:
            # 根据处理模式选择处理策略
        ...
        try:
            ...
            if hasattr(self.knowledge_base, "search") and callable(
                ...
                search_results = await self.knowledge_base.search(
                    query=request.query, top_k=request.top_k, **request.options
                )
            else:
            ...
            # 构建响应内容
            if hasattr(search_results, "items") and hasattr(
                ...
                for item in search_results.items:
                    ...
                    content_parts.append(
                        f"[{source_info['source']}] {source_info['content'][:200]}..."
                    )
            ...
            return SoftAdapterResponse(
                content=content, confidence=confidence, sources=sources
            )

        except Exception as e:
            logger.error(f"知识库查询失败: {e}")
            raise KnowledgeBaseError(
                f"知识库查询失败: {str(e)}", adapter_id=self.adapter_id, cause=e
            )

    async def _process_rag_generation(
        self, request: SoftAdapterRequest
    ) -> SoftAdapterResponse:
        """处理RAG生成请求"""
        ...
        try:
            # 调用RAG引擎处理
            if hasattr(self.rag_engine, "process") and callable(
                self.rag_engine.process
            ):
                rag_result = await self.rag_engine.process(
                    query=request.query, context=request.context, **request.options
                )
            elif hasattr(self.rag_engine, "generate") and callable(
                self.rag_engine.generate
            ):
                rag_result = await self.rag_engine.generate(
                    query=request.query, **request.options
                )
            else:
            ...
            if hasattr(rag_result, "response") and hasattr(rag_result, "sources"):
                ...
                return SoftAdapterResponse(
                    ...
                    sources=[
                        {
                            "content": getattr(source.item, "content", "")
                            if hasattr(source, "item")
                            else str(source),
                            "source": getattr(source.item, "source", "")
                            if hasattr(source, "item")
                            else "",
                ...
                return SoftAdapterResponse(content=content, confidence=0.7, sources=[])

        except Exception as e:
            logger.error(f"RAG生成失败: {e}")
            raise RAGEngineError(
                f"RAG生成失败: {str(e)}", adapter_id=self.adapter_id, cause=e
            )

    async def _process_prompt_rendering(
        self, request: SoftAdapterRequest
    ) -> SoftAdapterResponse:
        """处理提示渲染请求"""
        ...
        try:
            ...
            variables.update(
                {"query": request.query, **request.options.get("variables", {})}
            )
            ...
            if template_content:
                ...
                if hasattr(self.prompt_engine, "render_string"):
                    render_result = await self.prompt_engine.render_string(
                ...
                if hasattr(self.prompt_engine, "render_template"):
                    render_result = await self.prompt_engine.render_template(
        ...
        try:
            ...
            if self.prompt_engine:
                try:
                    ...
                    prompt_response = await self._process_prompt_rendering(
                        prompt_request
                    )
            ...
            raise SoftAdapterError(
                f"混合处理失败: {str(e)}", adapter_id=self.adapter_id, cause=e
            )
    ...
    async def _process_conversation(
        ...
        try:
            ...
            raise SoftAdapterError(
                f"对话处理失败: {str(e)}", adapter_id=self.adapter_id, cause=e
            )
...
注意：完整的文档分析需要配置提示引擎组件。"""
            ...
            raise SoftAdapterError(
                f"文档分析失败: {str(e)}", adapter_id=self.adapter_id, cause=e
            )

    async def _post_process_response(
        self, response: SoftAdapterResponse, request: SoftAdapterRequest
    ) -> SoftAdapterResponse:
        """后处理响应"""
        try:
            # 内容格式化
            if request.content_type == ContentType.JSON:
                ...
                try:
                    ...
                    response.content = json.dumps(
                        response_dict, ensure_ascii=False, indent=2
                    )
                except Exception as e:
            ...
            if (
                request.max_tokens
                and len(response.content.split()) > request.max_tokens
            ):
                # 如果超过最大令牌数，进行截断
    ...
    async def query_knowledge(
        ...
        context = ExecutionContext(execution_id=f"kb_query_{uuid.uuid4().hex[:8]}")

        result = await self._process_impl(request, context)
        return (
            result
            if isinstance(result, SoftAdapterResponse)
            else SoftAdapterResponse(content=str(result))
        )
    ...
    async def generate_response(
        ...
        exec_context = ExecutionContext(execution_id=f"rag_gen_{uuid.uuid4().hex[:8]}")
    ...
    async def render_template(
        ...
        context = ExecutionContext(execution_id=f"template_{uuid.uuid4().hex[:8]}")
    ...
    async def chat(
        ...
        **kwargs,
        ...
        context = ExecutionContext(execution_id=f"chat_{uuid.uuid4().hex[:8]}")
    ...
    async def analyze_document(
        self, document: str, requirements: str, **kwargs
    ) -> SoftAdapterResponse:
        """文档分析的便捷接口"""
        request = SoftAdapterRequest(
            query=requirements,
        ...
        context = ExecutionContext(execution_id=f"doc_analysis_{uuid.uuid4().hex[:8]}")
...
def create_simple_soft_adapter(
    name: str = "simple_soft_adapter", **kwargs
) -> SoftAdapter:
    """创建简单配置的软适配器"""
    config = {
...
def create_production_soft_adapter(
    ...
    **kwargs,
    ...
    config = {
        ...
        "rag_engine": rag_config
        or {
            "mode": "hybrid",
            "max_context_length": 4000,
            "retrieval_strategy": "semantic",
        },
        "prompt_engine": prompt_config
        or {"enable_caching": True, "max_template_size": 10000},
        **kwargs,
    }
    return SoftAdapter(config)
