...
class FileTemplateLoader(TemplateLoader):
    ...
    async def load_template(self, template_id: str) -> Tuple[str, TemplateMetadata]:
        ...
        if not template_path.exists():
            raise TemplateNotFoundError(f"Template '{template_id}' not found at {template_path}")
        ...
        try:
            ...
            # 更新缓存
            with self._cache_lock:
                self._template_cache[template_id] = (content, metadata, datetime.now().timestamp())
    ...
    def get_template_list(self) -> List[str]:
        ...
        for pattern in ["*.txt", "*.jinja2", "*.j2", "*.template"]:
            templates.extend([
                f.stem for f in self.template_dir.rglob(pattern)
            ])
        ...
        for line in lines:
            ...
            if line.startswith('#META:') or line.startswith('<!--META:'):
                try:
                    ...
                    self.logger.warning(f"Invalid metadata in template {template_id}: {e}")
...
class DefaultFunctionRegistry(FunctionRegistry):
    ...
    def _register_builtin_functions(self):
        ...
        self.register_function('replace', lambda s, old, new: str(s).replace(old, new), safe=True)
        ...
        # 列表处理函数
        self.register_function('join', lambda sep, items: sep.join(map(str, items)), safe=True)
        self.register_function('split', lambda s, sep=None: str(s).split(sep), safe=True)
        
        # 日期时间函数
        self.register_function('now', lambda: datetime.now().isoformat(), safe=True)
        self.register_function('today', lambda: datetime.now().date().isoformat(), safe=True)
        
        # 格式化函数
        self.register_function('format_number', lambda n, decimals=2: f"{float(n):.{decimals}f}", safe=True)
        self.register_function('truncate', lambda s, length=100: str(s)[:length] + "..." if len(str(s)) > length else str(s), safe=True)
...
class AdvancedTemplateProcessor(TemplateProcessor):
    ...
    def __init__(self, 
        ...
        if JINJA2_AVAILABLE:
            ...
            self.jinja_env = None
            self.logger.warning("Jinja2 not available, using simple template processing")
    ...
    async def _process_simple_conditions(self, template: str, context: TemplateContext) -> str:
        """处理简单的条件语句"""
        # 这里实现简单的条件逻辑处理
        # 为了保持代码简洁，这里只实现基本功能
        condition_pattern = re.compile(r'\{%\s*if\s+(\w+)\s*%\}(.*?)\{%\s*endif\s*%\}', re.DOTALL)
...
class DynamicPromptEngine:
    ...
    def __init__(self,
        ...
        self.logger.info(f"Initialized DynamicPromptEngine (mode={execution_mode.value})")
    ...
    async def render_template(self,
                            ...
                            force_reload: bool = False) -> RenderResult:
        """渲染模板
        ...
        try:
            ...
            # 加载模板
            try:
                template_content, template_metadata = await self.template_loader.load_template(template_id)
            ...
            # 验证模板语法
            validation_errors = self.template_processor.validate_template(template_content)
            if validation_errors:
                errors.extend(validation_errors)
                raise TemplateSyntaxError(f"Template validation failed: {validation_errors}")
            
            # 准备渲染上下文
            context = await self._prepare_context(variables, session_id, context_metadata)
            ...
            # 渲染模板
            rendered_content = await self.template_processor.process(template_content, context)
            
            # 后处理
            rendered_content = await self._post_process(rendered_content, template_metadata, context)
            ...
            self.logger.info(f"Template rendered successfully: {template_id} ({execution_time:.3f}s)")
            return result
    ...
    async def batch_render(self,
                         ...
                         session_id: Optional[str] = None,
                         max_concurrent: int = 10) -> List[RenderResult]:
        """批量渲染模板
        ...
        with self._context_lock:
            if session_id not in self._session_contexts:
                session_context = TemplateContext(
                    ...
                    parent_context=self._global_context
                )
    ...
    async def _prepare_context(self,
                             variables: Dict[str, Any] = None,
                             session_id: Optional[str] = None,
                             metadata: Dict[str, Any] = None) -> TemplateContext:
        """准备渲染上下文"""
        # 创建局部上下文
    ...
    async def _process_template_inheritance(self,
                                          template_content: str,
                                          parent_template_id: str,
                                          context: TemplateContext) -> str:
        """处理模板继承"""
        try:
            parent_content, parent_metadata = await self.template_loader.load_template(parent_template_id)
            ...
            self.logger.warning(f"Template inheritance failed for {parent_template_id}: {e}")
    ...
    def _register_system_functions(self) -> None:
        ...
        try:
            ...
            kb = KnowledgeBase()
            self.register_function('kb_search', lambda query: kb.search(query), safe=True)
        except ImportError:
            pass
...
# 便捷函数
async def render(template_id: str, 
                variables: Dict[str, Any] = None,
                **kwargs) -> str:
    """便捷渲染函数"""
