...
from typing import Any, Dict, List, Optional, Union, Tuple, Set, Callable, Type, NamedTuple
...
class Retriever(ABC):
    ...
    @abstractmethod
    async def retrieve(self, query: str, config: RetrievalConfig) -> List[RetrievalResult]:
        """执行检索"""
        pass
...
class Generator(ABC):
    ...
    @abstractmethod
    async def generate(self, query: str, context: List[RetrievalResult], 
...
class SimilarityRetriever(Retriever):
    ...
    def __init__(self, knowledge_base: KnowledgeBase, embedding_model: Optional[str] = None):
...
class PromptBasedGenerator(Generator):
    ...
    def _build_context(self, context: List[RetrievalResult], config: GenerationConfig) -> str:
        """构建上下文文本"""
        context_parts = []
    ...
    def _generate_citations(self, context: List[RetrievalResult], config: GenerationConfig) -> str:
        """生成引用"""
        citations = []
        for result in context:
...
class RAGEngine:
    ...
    async def query(self, question: str, **kwargs) -> RAGResult:
        ...
        try:
            ...
            # 性能监控结束
            if self.performance_monitor:
                await self.performance_monitor.end_operation('rag_query', {'processing_time': processing_time})
    ...
    async def _retrieve_phase(self, question: str) -> List[RetrievalResult]:
        ...
        try:
            ...
            if self.config.retrieval.enable_query_expansion:
                ...
                queries.extend(expanded_queries[:self.config.retrieval.expansion_terms])
    ...
    async def _rerank_phase(self, question: str, results: List[RetrievalResult]) -> List[RetrievalResult]:
        """重排序阶段"""
        try:
    ...
    async def _generation_phase(self, question: str, context: List[RetrievalResult]) -> str:
        """生成阶段"""
        try:
            return await self.generator.generate(question, context, self.config.generation)
        except Exception as e:
            logger.error(f"生成阶段失败: {e}")
            return "抱歉，无法生成合适的回答。"
    
    async def _evaluate_quality(self, question: str, response: str, 
    ...
    def _update_stats(self, processing_time: float) -> None:
        ...
        self.stats['avg_response_time'] = (current_avg * (total - 1) + processing_time) / total
        ...
        for doc_idx, doc in enumerate(docs):
            ...
            for term in query_terms:
                ...
                if tf > 0:
                    ...
                    idf = np.log((len(docs) - df + 0.5) / (df + 0.5)) if NUMPY_AVAILABLE else 1.0
...
class ChainOfThoughtGenerator(Generator):
    ...
    async def generate(self, query: str, context: List[RetrievalResult], 
        ...
        try:
            ...
            # 第二步：逐步推理
            reasoning_prompt = await self._build_reasoning_prompt(query, context, analysis)
            reasoning = await self._call_llm(reasoning_prompt, max_length=400)
            
            # 第三步：生成最终答案
            answer_prompt = await self._build_answer_prompt(query, context, analysis, reasoning)
            final_answer = await self._call_llm(answer_prompt, max_length=config.max_length)
    ...
    async def _build_analysis_prompt(self, query: str, context: List[RetrievalResult]) -> str:
        """构建问题分析提示"""
        context_text = "\n".join([f"{i+1}. {r.item.content[:200]}..." 
...
"""
    ...
    async def _build_reasoning_prompt(self, query: str, context: List[RetrievalResult], 
                                    analysis: str) -> str:
        """构建推理提示"""
...
"""
    ...
    async def _build_answer_prompt(self, query: str, context: List[RetrievalResult],
                                 analysis: str, reasoning: str) -> str:
        """构建最终答案提示"""
        return f"""
...
class HybridGenerator(Generator):
    ...
    async def _extract_relevant_parts(self, query: str, context: List[RetrievalResult]) -> str:
        """抽取相关部分"""
        if not context:
            return ""
    ...
    async def _generate_new_content(self, query: str, context: List[RetrievalResult], 
                                  config: GenerationConfig) -> str:
        """生成新内容"""
...
class FactCheckingGenerator(Generator):
    ...
    async def generate(self, query: str, context: List[RetrievalResult], 
        ...
        try:
            # 第一步：生成初始回答
            initial_response = await self._generate_initial_response(query, context, config)
            
            # 第二步：提取声明进行事实核查
            if config.enable_fact_checking:
                fact_check_result = await self._fact_check_response(initial_response, query)
    ...
    async def _generate_initial_response(self, query: str, context: List[RetrievalResult],
                                       config: GenerationConfig) -> str:
        """生成初始回答"""
        context_text = self._build_context_text(context)
...
"""
    ...
    async def _generate_conservative_response(self, query: str, context: List[RetrievalResult],
                                            issues: List[str]) -> str:
        """生成保守的回答"""
        context_text = self._build_context_text(context)
...
class RAGPerformanceOptimizer:
    ...
    async def _smart_cache_lookup(self, query: str, kwargs: Dict[str, Any]) -> Optional[RAGResult]:
        """智能缓存查找"""
    ...
    async def _record_performance(self, query: str, result: RAGResult, execution_time: float):
        """记录性能数据"""
        performance_data = {
    ...
    async def _smart_cache_store(self, query: str, result: RAGResult, kwargs: Dict[str, Any]):
        """智能缓存存储"""
        if not self.rag_engine.config.enable_caching:
            return
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'cache_hit_rate': len(self.query_cache) / max(len(self.performance_history), 1),
...
class HybridRetriever(Retriever):
    ...
    async def retrieve(self, query: str, config: RetrievalConfig) -> List[RetrievalResult]:
        ...
        try:
            ...
            retriever_results = await asyncio.gather(*retrieval_tasks, return_exceptions=True)
    ...
    async def _rrf_fusion(self, results: List[RetrievalResult], top_k: int) -> List[RetrievalResult]:
        """RRF融合"""
        # 按检索器分组
    ...
    async def _score_average_fusion(self, results: List[RetrievalResult], top_k: int) -> List[RetrievalResult]:
        """分数平均融合"""
        # 按项目ID分组
    ...
    async def _simple_fusion(self, results: List[RetrievalResult], top_k: int) -> List[RetrievalResult]:
        """简单融合，去重并按原始分数排序"""
        # 去重
        unique_results = {}
        for result in results:
            item_id = result.item.id
            if item_id not in unique_results or result.score > unique_results[item_id].score:
...
class RAGUtils:
    ...
    @staticmethod
    def export_rag_results(results: List[RAGResult], 
                          filepath: str, 
        ...
        if format.lower() == "json":
            ...
            with open(filepath, 'w', encoding='utf-8', newline='') as f:
                writer = csv.writer(f)
                # 写入表头
                writer.writerow(['Response', 'Confidence', 'Processing Time', 'Sources Count'])
