...
class MetricAggregator:
    ...
    def aggregate(
        self, samples: List[MetricSample], aggregation_type: AggregationType
    ) -> Optional[float]:
        """聚合样本"""
        if not samples:
            return None
    ...
    def aggregate_by_time_window(
        ...
        while current_time <= end_time:
            ...
            window_samples = [
                s for s in sorted_samples if current_time <= s.timestamp < window_end
            ]
        ...
        for family in metrics:
            ...
            for metric in family.get_all_metrics():
                for sample in metric.samples:
                    ...
                    lines.append(
                        f"{metric.name}{labels_str} {sample.value} {timestamp_ms}"
                    )
        ...
        for family in metrics:
            for metric in family.get_all_metrics():
                for sample in metric.samples:
                    ...
                    line = (
                        f"{metric.name}{tags_str} value={sample.value} {timestamp_ns}"
                    )
                    lines.append(line)
...
class AdapterMetricsService(AsyncService):
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        """获取指标摘要"""
        with self._metrics_lock:
            summary = {"total_families": len(self._metric_families), "families": {}}
    ...
    async def _setup_default_metrics(self) -> None:
        ...
        self.create_metric_family(
            "system_memory_percent", MetricType.GAUGE, "内存使用率", "%"
        )
        self.create_metric_family("system_disk_percent", MetricType.GAUGE, "磁盘使用率", "%")

        # 适配器指标
        self.create_metric_family(
            "adapter_execution_count", MetricType.COUNTER, "适配器执行次数"
        )
        self.create_metric_family(
            "adapter_execution_time", MetricType.HISTOGRAM, "适配器执行时间", "seconds"
        )
        ...
        self.create_metric_family(
            "service_response_time", MetricType.HISTOGRAM, "服务响应时间", "seconds"
        )
    ...
    async def _collection_loop(self) -> None:
        """指标收集循环"""
        logger.info(
            f"Starting metrics collection loop with interval: {self.config.collection_interval}s"
        )
        ...
        while self._status == ServiceStatus.RUNNING:
            try:
                ...
                for collector in self._collectors:
                    if hasattr(collector, "collect"):
                        task = asyncio.create_task(
                            self._collect_from_collector(collector)
                        )
                ...
                self._collection_tasks = {
                    t for t in self._collection_tasks if not t.done()
                }
