...
class MemoryMetricsStorage(MetricsStorage):
                    ...
                    if family.name in self._metrics:
                        ...
                        for metric in family.get_all_metrics():
                            ...
                            if labels_key in existing_family.metrics:
                                ...
                                # 限制样本数量
                                if (
                                    len(existing_metric.samples)
                                    > self.max_samples_per_metric
                                ):
                                    existing_metric.samples = existing_metric.samples[
                                        -self.max_samples_per_metric :
                                    ]
                            else:
                                # 添加新指标
                    ...
                    for name in names:
                        if name in self._metrics:
                            if start_time or end_time:
                                ...
                                for metric in family.get_all_metrics():
                                    ...
                                    metric.samples = [
                                        ...
                                        if not (
                                            (
                                                start_time is None
                                                or s.timestamp >= start_time
                                            )
                                            and (
                                                end_time is None
                                                or s.timestamp <= end_time
                                            )
                                        )
                                    ]
                                ...
                                deleted_count += sum(
                                    len(m.samples)
                                    for m in self._metrics[name].get_all_metrics()
                                )
    ...
    def _match_labels(
        self, metric_labels: Dict[str, str], filter_labels: Dict[str, str]
    ) -> bool:
        """检查标签是否匹配"""
            ...
            while len(self._metrics) > self.max_metrics * 0.8:  # 保留80%的空间
                if families_by_time:
                    ...
                    if name_to_delete in self._metrics:
                        ...
                        self.logger.info(
                            f"Cleaned up old metric family: {name_to_delete}"
                        )
                else:
                    break
...
class FileMetricsStorage(MetricsStorage):
    ...
    async def _create_tables(self) -> None:
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_metrics_family ON metrics (family_name)"
        )
                ...
                if names:
                    ...
                    for name in names:
                        if start_time or end_time:
                            ...
                            cursor.execute(
                                "DELETE FROM metrics WHERE family_name = ?", (name,)
                            )
                            cursor.execute(
                                "DELETE FROM metric_families WHERE name = ?", (name,)
                            )
                else:
                    # 删除所有指标
                    if start_time or end_time:
        ...
        try:
            ...
            for family in metrics:
                ...
                for metric in family.get_all_metrics():
                    ...
                    metric_key = (
                        f"{self.key_prefix}metric:{family.name}:{hash(labels_key)}"
                    )

                    # 存储指标信息
                    metric_data = {"labels": labels_key, "name": metric.name}
    ...
    async def read_metrics(
        ...
        try:
            ...
            for family_name in family_names:
                ...
                family_data = await loop.run_in_executor(
                    None, self._redis.hgetall, family_key
                )
                ...
                metric_keys = await loop.run_in_executor(
                    None, self._redis.keys, metric_pattern
                )

                for metric_key in metric_keys:
                    # 获取指标信息
                    metric_data = await loop.run_in_executor(
                        None, self._redis.hgetall, metric_key
                    )
                    if not metric_data:
                        continue
                    ...
                    if samples:
                        ...
                        labels_key = json.dumps(
                            sorted(metric_labels), ensure_ascii=False
                        )
                        family.metrics[labels_key] = metric
                    ...
                    if start_time or end_time:
                        ...
                        if keys_to_delete:
                            deleted_count += await loop.run_in_executor(
                                None, self._redis.delete, *keys_to_delete
                            )
            else:
                # 删除所有指标
    ...
    async def get_storage_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            metric_keys = [
                k
                for k in keys
                if k.startswith(f"{self.key_prefix}metric:")
                and not k.endswith(":samples")
            ]
...
class InfluxDBMetricsStorage(MetricsStorage):
        ...
        try:
            ...
            if points:
                await loop.run_in_executor(
                    None, self._write_api.write, self.bucket, self.org, points
                )
    ...
    async def read_metrics(
        ...
        try:
            ...
            if end_time:
                query_parts[-1] = query_parts[-1].replace(
            ...
            if names:
                measurement_filter = " or ".join(
                    [f'r._measurement == "{name}"' for name in names]
                )
            ...
            tables = await loop.run_in_executor(
                None, self._query_api.query, query, self.org
            )
            ...
            for table in tables:
                for record in table.records:
                    ...
                    # 创建或获取指标
                    labels_key = json.dumps(
                        sorted(metric_labels.items()), ensure_ascii=False
                    )
                    if labels_key not in family.metrics:
                        family.metrics[labels_key] = Metric(
                            name=measurement,
    ...
    async def delete_metrics(
        ...
        try:
            ...
            # 执行删除
            await loop.run_in_executor(
                None, delete_api.delete, start, stop, predicate, self.bucket, self.org
            )
    ...
    async def get_storage_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            bucket_info = await loop.run_in_executor(
                None, buckets_api.find_bucket_by_name, self.bucket
            )
...
class MetricsStorageManager:
    ...
    def register_storage(
        self, name: str, storage: MetricsStorage, is_primary: bool = False
    ) -> None:
        """注册存储后端"""
    ...
    async def read_from_primary(
        ...
        if primary:
            return await primary.read_metrics(
                names, start_time, end_time, labels, limit
            )
        return []
    ...
    async def get_all_storage_info(self) -> Dict[str, Any]:
        """获取所有存储信息"""
        info = {"primary_storage": self._primary_storage, "storages": {}}
