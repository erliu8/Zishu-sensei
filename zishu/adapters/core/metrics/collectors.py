    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                'success_rate': (self.collection_count - self.error_count) / max(self.collection_count, 1) * 100
            }
        ...
        try:
            ...
            for partition in disk_partitions:
                try:
                    ...
                    self.set_gauge("system_disk_percent", usage.used / usage.total * 100, labels)
            ...
            if network_io:
                ...
                if self._last_network_io and self._last_network_time:
                    ...
                    if time_delta > 0:
                        ...
                        self.set_gauge("system_network_bytes_sent_rate", bytes_sent_rate)
                        self.set_gauge("system_network_bytes_recv_rate", bytes_recv_rate)
    ...
    async def _collect_gpu_metrics(self, timestamp: datetime) -> None:
        ...
        try:
            ...
            for i in range(device_count):
                ...
                self.set_gauge("system_gpu_total_memory", device_props.total_memory, labels)
...
class AdapterMetricsCollector(MetricsCollector):
    ...
    def __init__(
        ...
        # 适配器管理器引用(弱引用)
        self._adapter_manager = weakref.ref(adapter_manager) if adapter_manager else None
        ...
        try:
            ...
            if hasattr(manager, 'get_registered_adapters'):
                ...
                for adapter_info in adapters.values():
                    adapter_type = getattr(adapter_info.get('adapter'), 'adapter_type', 'unknown')
                    type_counts[adapter_type] += 1
                
                for adapter_type, count in type_counts.items():
                    self.set_gauge("adapter_count_by_type", count, {"type": adapter_type})
        ...
        with self._stats_lock:
            for adapter_name, stats in self._execution_stats.items():
                ...
                if stats['count'] > 0:
                    ...
                    self.set_gauge("adapter_min_execution_time", stats['min_time'], labels)
                    self.set_gauge("adapter_max_execution_time", stats['max_time'], labels)
                
                # 最后执行时间
                if stats['last_execution']:
                    self.set_gauge("adapter_last_execution_timestamp", stats['last_execution'], labels)
    ...
    def record_execution(
        ...
        if not success:
            error_labels = {**labels, "error": error_message[:100] if error_message else "unknown"}
    ...
    async def _collect_service_metrics(self, service, timestamp: datetime) -> None:
        ...
        try:
            ...
            if hasattr(service, '_status'):
                status = service._status.value if hasattr(service._status, 'value') else str(service._status)
            ...
            if hasattr(service, 'health_check'):
                try:
                    ...
                    self.set_gauge("service_health", 1, {**labels, "health": health_status})
                    
                    # 其他健康指标
                    if 'response_time' in health_info:
                        self.observe_histogram("service_health_check_time", health_info['response_time'], labels)
                    
                except Exception as e:
                    logger.error(f"Failed to get health info for service {service_name}: {e}")
            ...
            if hasattr(service, '_service_metrics'):
                ...
                self.set_gauge("service_avg_response_time", metrics.avg_response_time, labels)
    ...
    async def collect(self) -> None:
        ...
        for collector_func in self._custom_collectors:
            try:
                ...
                logger.error(f"Error in custom business collector {collector_func.__name__}: {e}")
...
class CollectorManager:
    ...
    async def collect_once(self, collector_name: Optional[str] = None) -> Dict[str, bool]:
        """执行一次性收集"""
        results = {}
