...
from .core import (
    Metric, MetricFamily, MetricSample, MetricType, AggregationType,
    MetricsCollector, MetricsService
)
from .collectors import (
    SystemMetricsCollector, AdapterMetricsCollector, 
    CustomMetricsCollector, PerformanceMetricsCollector
)
...
def track_execution_time(metric_name: str = "execution_time", labels: Optional[Dict[str, str]] = None):
    """
    装饰器：跟踪函数执行时间
...
def count_calls(metric_name: str = "function_calls", labels: Optional[Dict[str, str]] = None):
    """
    装饰器：统计函数调用次数
...
def record_summary(name: str, value: float, labels: Optional[Dict[str, str]] = None,
                  description: str = "", unit: str = "") -> None:
    """
    记录摘要指标
...
def _record_metric(name: str, metric_type: MetricType, value: float, 
                  labels: Optional[Dict[str, str]], description: str, unit: str) -> None:
    """内部函数：记录指标"""
    sample = MetricSample(
        value=value,
        timestamp=datetime.now(timezone.utc),
...
class MetricsBatch:
    ...
    def counter(self, name: str, value: float = 1, labels: Optional[Dict[str, str]] = None,
               description: str = "", unit: str = "") -> 'MetricsBatch':
        """添加计数器指标"""
        return self._add_metric(name, MetricType.COUNTER, value, labels, description, unit)
    
    def gauge(self, name: str, value: float, labels: Optional[Dict[str, str]] = None,
             description: str = "", unit: str = "") -> 'MetricsBatch':
        """添加仪表盘指标"""
        return self._add_metric(name, MetricType.GAUGE, value, labels, description, unit)
    
    def histogram(self, name: str, value: float, labels: Optional[Dict[str, str]] = None,
                 description: str = "", unit: str = "") -> 'MetricsBatch':
        """添加直方图指标"""
        return self._add_metric(name, MetricType.HISTOGRAM, value, labels, description, unit)
    
    def summary(self, name: str, value: float, labels: Optional[Dict[str, str]] = None,
               description: str = "", unit: str = "") -> 'MetricsBatch':
        """添加摘要指标"""
        return self._add_metric(name, MetricType.SUMMARY, value, labels, description, unit)
    
    def _add_metric(self, name: str, metric_type: MetricType, value: float,
                   labels: Optional[Dict[str, str]], description: str, unit: str) -> 'MetricsBatch':
        """添加指标到批次"""
        sample = MetricSample(
            value=value,
            timestamp=datetime.now(timezone.utc),
    ...
    async def record(self) -> bool:
        ...
        if global_service:
            ...
            logger.warning("No global metrics service available to record batch metrics")
            return False
...
async def check_metrics_health() -> Dict[str, Any]:
    ...
    if not service:
        return {
            'status': 'error',
            'message': 'No global metrics service available'
        }
...
async def generate_sample_metrics(count: int = 100, interval: float = 1.0) -> None:
    ...
    for i in range(count):
        ...
        batch.gauge("test_cpu_usage", cpu_usage, {"host": "test-server"}, "Test CPU usage", "%")
        
        # 内存使用率
        memory_usage = random.uniform(30, 85)
        batch.gauge("test_memory_usage", memory_usage, {"host": "test-server"}, "Test memory usage", "%")
        
        # 请求计数
        batch.counter("test_requests_total", random.randint(1, 10), {"method": "GET", "status": "200"})
        
        # 响应时间
        response_time = random.uniform(0.1, 2.0)
        batch.histogram("test_response_time", response_time, {"endpoint": "/api/test"}, "Test response time", "seconds")
...
async def run_metrics_benchmark(duration: int = 60) -> Dict[str, Any]:
    ...
    while time.time() < end_time:
        try:
            ...
            batch.histogram("benchmark_histogram", time.time() % 10, {"test": "benchmark"})
    ...
    result = {
        ...
        'errors': errors,
        'metrics_per_second': metrics_recorded / actual_duration if actual_duration > 0 else 0,
        'error_rate': errors / (metrics_recorded + errors) if (metrics_recorded + errors) > 0 else 0
    }
