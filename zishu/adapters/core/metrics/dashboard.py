    ...
    def matches(self, data: Dict[str, Any]) -> bool:
        ...
        if self.operator == QueryOperator.EQ:
            ...
            return (
                ...
                and self.value.lower() in field_value.lower()
            )
        elif self.operator == QueryOperator.REGEX:
            import re

            return isinstance(field_value, str) and bool(
                re.search(self.value, field_value)
            )
...
class MetricsQueryEngine:
    ...
    def _apply_conditions(
        self, metrics: List[MetricFamily], conditions: List[QueryCondition]
    ) -> List[MetricFamily]:
        """应用查询条件"""
        if not conditions:
            return metrics
        ...
        for family in metrics:
            ...
            for metric in family.get_all_metrics():
                ...
                for sample in metric.samples:
                    ...
                    # 检查所有条件
                    matches_all = all(
                        condition.matches(sample_data) for condition in conditions
                    )
    ...
    def _apply_aggregation(
        self, metrics: List[MetricFamily], query: MetricsQuery
    ) -> List[MetricFamily]:
        """应用聚合"""
        ...
        for family in metrics:
            ...
            for group_key, group_metrics in groups.items():
                ...
                if query.time_window:
                    ...
                    # 单值聚合
                    aggregated_value = self.aggregator.aggregate(
                        all_samples, query.aggregation
                    )
                ...
                if aggregated_samples:
                    ...
                    labels_key = json.dumps(
                        sorted(aggregated_metric.labels.items()), ensure_ascii=False
                    )
    ...
    def _group_metrics(
        self, metrics: List[Metric], group_by: List[str]
                ...
                for metric in family.get_all_metrics():
                    if metric.samples:
                        latest_timestamp = max(
                            latest_timestamp, metric.samples[-1].timestamp
                        )
...
class ChartData:
    ...
    def add_series(self, name: str, data: List[Any], **kwargs) -> None:
        """添加数据系列"""
        series_data = {"name": name, "data": data, **kwargs}
...
class MetricsVisualizer:
    ...
    def create_chart_data(
        self, metrics: List[MetricFamily], chart_config: ChartConfig
    ) -> ChartData:
        """创建图表数据"""
    ...
    def _create_line_chart_data(self, metrics: List[MetricFamily]) -> ChartData:
        ...
        for family in metrics:
            for metric in family.get_all_metrics():
                ...
                for sample in metric.samples:
                    series_data.append(
                        {"x": sample.timestamp.isoformat(), "y": sample.value}
                    )
        ...
        for family in metrics:
            for metric in family.get_all_metrics():
                if metric.samples:
                    ...
                    pie_data.append({"name": label, "value": metric.samples[-1].value})
            ...
            for metric in family.get_all_metrics():
                if metric.samples:
                    ...
                    for label_key, label_value in metric.labels.items():
                        ...
                        heatmap_data.append(
                            {"x": x_category, "y": y_category, "value": value}
                        )
    ...
    def generate_chart_json(
        self, chart_data: ChartData, chart_config: ChartConfig
    ) -> str:
        """生成图表JSON配置"""
        chart_json = {"config": chart_config.to_dict(), "data": chart_data.to_dict()}
...
class MetricsDashboard:
    ...
    def create_dashboard(self, name: str, description: str = "") -> DashboardConfig:
        ...
        dashboard = DashboardConfig(id=dashboard_id, name=name, description=description)
    ...
    async def _render_widget(
        ...
        chart_data = self.visualizer.create_chart_data(
            query_result.metrics, widget.chart_config
        )
    ...
    def create_system_dashboard(self) -> DashboardConfig:
        ...
        cpu_chart = ChartConfig(
            chart_type=ChartType.LINE, title="CPU Usage", y_axis_label="Percentage (%)"
        )
        ...
        adapter_query = (
            QueryBuilder()
            .metrics("adapter_execution_count")
            .last(timedelta(hours=24))
            .build()
        )
        adapter_chart = ChartConfig(
            chart_type=ChartType.BAR, title="Adapter Executions", y_axis_label="Count"
        )
    ...
    def create_adapter_dashboard(self) -> DashboardConfig:
        ...
        exec_time_query = (
            QueryBuilder()
            .metrics("adapter_execution_time")
            .last(timedelta(hours=6))
            .build()
        )
        ...
        error_query = (
            QueryBuilder()
            .metrics("adapter_error_count")
            .last(timedelta(hours=24))
            .build()
        )
        error_chart = ChartConfig(chart_type=ChartType.PIE, title="Error Distribution")
        ...
        status_chart = ChartConfig(chart_type=ChartType.TABLE, title="Adapter Status")
