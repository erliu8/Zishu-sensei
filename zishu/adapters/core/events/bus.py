...
class EventBus:
    ...
    def get_subscriptions(self, event_type: Optional[EventType] = None) -> List[EventSubscription]:
        """
        获取订阅列表
            ...
            if tasks:
                ...
                for i, result in enumerate(results):
                    if isinstance(result, Exception):
                        logger.error(f"Handler {handlers[i].subscription_id} failed: {result}")
                        self._stats['events_failed'] += 1
    ...
    async def _invoke_handler(self, subscription: EventSubscription, event: Event) -> None:
        """调用事件处理器"""
        try:
