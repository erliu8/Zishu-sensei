...
class EventTypeFilter(EventFilter):
    ...
    def __init__(self, event_types: Union[EventType, List[EventType]], include: bool = True):
        """
        初始化事件类型过滤器
...
class PriorityFilter(EventFilter):
    ...
    def matches(self, event: Event) -> bool:
        ...
        # 检查优先级范围
        if self.min_priority is not None and event.priority.value < self.min_priority.value:
            return False
        
        if self.max_priority is not None and event.priority.value > self.max_priority.value:
            return False
...
class CustomFilter(EventFilter):
    ...
    def __init__(self, filter_func: Callable[[Event], bool], name: Optional[str] = None):
        """
        初始化自定义过滤器
