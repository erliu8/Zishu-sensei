...
from ..types import AdapterRegistration, AdapterStatus, AdapterType, LifecycleState
...
class DiscoveryService:
    ...
    def list_adapters(
        ...
        if adapter_type:
            registrations = [
                r for r in registrations if r.adapter_type.value == adapter_type
            ]
        ...
        if lifecycle_state:
            registrations = [
                r for r in registrations if r.lifecycle_state == lifecycle_state
            ]

        if tags:
            registrations = [r for r in registrations if tags.issubset(r.identity.tags)]
    ...
    def search_adapters(
        ...
        for registration in registrations:
            ...
            # 搜索适配器ID
            if (
                "adapter_id" in fields
                and query_lower in registration.adapter_id.lower()
            ):
                match = True
    ...
    def _apply_filter(
        self, registrations: List[AdapterRegistration], key: str, value: Any
    ) -> List[AdapterRegistration]:
        """应用过滤条件"""
        if key == "name":
