...
class AdapterServiceOrchestrator:
    ...
    def add_dependency(self, service_name: str, dependency_name: str, 
                      is_hard: bool = True, is_optional: bool = False) -> None:
        """添加服务依赖"""
        ...
        if self._has_circular_dependency(service_name, dependency_name):
            raise ValueError(f"Circular dependency detected: {service_name} -> {dependency_name}")
        ...
        # 记录依赖关系
        dependency = ServiceDependency(service_name, dependency_name, is_hard, is_optional)
    ...
    async def start_all(self) -> None:
        ...
        async with self._lock:
            if self._status != ServiceStatus.READY:
                raise RuntimeError(f"Orchestrator must be initialized before starting, current status: {self._status}")
    ...
    def _calculate_startup_order(self) -> List[List[str]]:
        ...
        while remaining:
            ...
            if not current_layer:
                # 存在循环依赖
                raise ValueError(f"Circular dependency detected in remaining services: {remaining}")
        ...
        for attempt in range(self.config.retry_attempts):
            try:
                ...
                if not await self._check_dependencies(service_name):
                    raise RuntimeError(f"Dependencies not satisfied for service '{service_name}'")
                ...
                logger.warning(f"Failed to start service '{service_name}' (attempt {attempt + 1}): {e}")
                
                if attempt < self.config.retry_attempts - 1:
                    await asyncio.sleep(self.config.retry_delay * (2 ** attempt))  # 指数退避
                else:
                    logger.error(f"Failed to start service '{service_name}' after {self.config.retry_attempts} attempts")
                    raise
            ...
            try:
                if node.service.is_running:
                    ...
                    if not result.is_healthy:
                        logger.warning(f"Service '{service_name}' is unhealthy: {result.message}")
            except Exception as e:
    ...
    async def _perform_recovery_checks(self) -> None:
        ...
        for service_name, node in self._services.items():
            try:
                # 检查服务是否需要恢复
                if (node.service.status == ServiceStatus.ERROR or 
                    node.service.health == ServiceHealth.UNHEALTHY):
