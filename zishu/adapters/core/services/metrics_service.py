...
from ..metrics import (
    MetricsService, MetricsConfig, MetricsStorage, MetricsStorageManager,
    MemoryMetricsStorage, MetricsCollector, SystemMetricsCollector,
    AdapterMetricsCollector, PerformanceMetricsCollector,
    MetricsExporterManager, ExportConfig, ExportFormat,
    MetricsAlertManager, AlertRule, AlertCondition,
    MetricsDashboard, MetricsQueryEngine,
    setup_metrics_system, teardown_metrics_system,
    set_global_metrics_service, clear_global_metrics_service
)
...
class AdapterMetricsService(AsyncService):
    ...
    async def _setup_exporters(self) -> None:
        ...
        if self.config.enable_prometheus_export:
            ...
            logger.info(f"Prometheus exporter started on port {self.config.prometheus_port}")
    ...
    async def _register_event_handlers(self) -> None:
        ...
        await self.event_bus.subscribe(
            ...
            self._handle_adapter_registered
        )
        
        # 适配器注销事件
        await self.event_bus.subscribe(
            EventType.ADAPTER_UNREGISTERED,
            self._handle_adapter_unregistered
        )
        
        # 适配器启动事件
        await self.event_bus.subscribe(
            EventType.ADAPTER_STARTED,
            self._handle_adapter_started
        )
        
        # 适配器停止事件
        await self.event_bus.subscribe(
            EventType.ADAPTER_STOPPED,
            self._handle_adapter_stopped
        )
            ...
            try:
                ...
                if self.metrics_service:
                    ...
                    if storage and hasattr(storage, 'cleanup_old_metrics'):
                        ...
                        logger.info(f"Cleaned up metrics older than {self.config.metrics_retention_days} days")
        ...
        try:
            ...
            for family in metrics:
                result[family.name] = {
                    ...
                    'unit': family.unit,
                    'metrics': [metric.to_dict() for metric in family.get_all_metrics()]
                }
