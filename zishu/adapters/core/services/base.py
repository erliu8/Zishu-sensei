...
class AsyncService(ABC):
    ...
    def __init__(self, name: str, config: Optional[Dict[str, Any]] = None):
        ...
        # 事件回调
        self._status_callbacks: List[Callable[[ServiceStatus, ServiceStatus], None]] = []
        self._health_callbacks: List[Callable[[ServiceHealth, ServiceHealth], None]] = []
    ...
    def metrics(self) -> ServiceMetrics:
        """获取性能指标"""
        if self._start_time:
            self._metrics.uptime = (datetime.now(timezone.utc) - self._start_time).total_seconds()
    ...
    async def initialize(self) -> None:
        ...
        async with self._lock:
            if self._status != ServiceStatus.CREATED:
                logger.warning(f"Service '{self.name}' already initialized, current status: {self._status}")
                return
    ...
    async def start(self) -> None:
        ...
        async with self._lock:
            ...
            if self._status != ServiceStatus.READY:
                raise RuntimeError(f"Service '{self.name}' must be initialized before starting")
    ...
    async def pause(self) -> None:
        ...
        async with self._lock:
            if self._status != ServiceStatus.RUNNING:
                raise RuntimeError(f"Cannot pause service '{self.name}' in status: {self._status}")
    ...
    async def resume(self) -> None:
        ...
        async with self._lock:
            if self._status != ServiceStatus.PAUSED:
                raise RuntimeError(f"Cannot resume service '{self.name}' in status: {self._status}")
    ...
    def add_status_callback(self, callback: Callable[[ServiceStatus, ServiceStatus], None]) -> None:
        """添加状态变化回调"""
        self._status_callbacks.append(callback)
    
    def add_health_callback(self, callback: Callable[[ServiceHealth, ServiceHealth], None]) -> None:
        """添加健康状态变化回调"""
