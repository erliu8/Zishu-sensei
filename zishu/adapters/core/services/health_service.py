...
class HealthMonitor:
    ...
    async def check_health(self, adapter_id: str, registration: AdapterRegistration) -> List[HealthMetric]:
        """执行健康检查"""
...
class AdapterHealthService(AsyncService):
    ...
    async def _initialize_impl(self) -> None:
        ...
        logger.info(f"Adapter health service initialized with {len(self._health_monitors)} monitors")
    ...
    async def _start_impl(self) -> None:
        ...
        if self._event_bus:
            await self._event_bus.emit(Event(
                ...
                data={"service": "adapter_health", "timestamp": datetime.now(timezone.utc)}
            ))
    ...
    async def _health_check_impl(self) -> HealthCheckResult:
        ...
        try:
            ...
            unhealthy_count = sum(1 for status in self._health_status.values() 
            ...
            # 检查监控器状态
            disabled_monitors = sum(1 for monitor in self._health_monitors.values() 
    ...
    async def check_adapter_health(self, adapter_id: str) -> Optional[AdapterHealthStatus]:
        """检查单个适配器健康状态"""
    ...
    async def get_adapter_health(self, adapter_id: str) -> Optional[AdapterHealthStatus]:
        """获取适配器健康状态"""
        return self._health_status.get(adapter_id)
    ...
    async def add_health_threshold(self, adapter_id: str, threshold: HealthThreshold) -> None:
        """添加健康阈值"""
        async with self._health_lock:
            ...
            logger.info(f"Added health threshold for adapter '{adapter_id}': {threshold.metric_name}")
    
    async def get_health_metrics_history(self, adapter_id: str, 
        ...
        # 过滤时间范围
        filtered_history = [metric for metric in history if metric.timestamp >= cutoff_time]
        
        # 过滤指标名称
        if metric_name:
            filtered_history = [metric for metric in filtered_history if metric.name == metric_name]
    ...
    async def get_health_summary(self) -> Dict[str, Any]:
        ...
        healthy_count = sum(1 for status in self._health_status.values() 
                          if status.overall_health == ServiceHealth.HEALTHY)
        degraded_count = sum(1 for status in self._health_status.values() 
        ...
        return {
            ...
            "health_rate": healthy_count / total_adapters if total_adapters > 0 else 0.0,
            "active_monitors": len([m for m in self._health_monitors.values() if m.enabled]),
            "total_checks": sum(status.check_count for status in self._health_status.values()),
    ...
    async def _register_default_monitors(self) -> None:
        ...
        logger.info("Enhanced health monitors registered: availability, response_time, error_rate, memory_usage, throughput")
    ...
    async def _update_health_status(self, adapter_id: str, metrics: List[HealthMetric]) -> None:
        """更新健康状态"""
        async with self._health_lock:
            if adapter_id not in self._health_status:
                self._health_status[adapter_id] = AdapterHealthStatus(
    ...
    async def _check_thresholds(self, adapter_id: str, metrics: List[HealthMetric]) -> List[str]:
        """检查健康阈值"""
        issues = []
        ...
        for threshold in thresholds:
            ...
            if threshold.critical_threshold is not None:
                if self._check_threshold_condition(value, threshold.critical_threshold, threshold.comparison):
                    issues.append(f"Critical: {threshold.metric_name} = {value} (threshold: {threshold.critical_threshold})")
            ...
            # 检查警告阈值
                ...
                if self._check_threshold_condition(value, threshold.warning_threshold, threshold.comparison):
                    issues.append(f"Warning: {threshold.metric_name} = {value} (threshold: {threshold.warning_threshold})")
    ...
    def _calculate_overall_health(self, metrics: List[HealthMetric], issues: List[str]) -> ServiceHealth:
        """计算总体健康状态"""
        if not metrics:
        ...
        # 检查可用性指标
        availability_metrics = [m for m in metrics if m.type == HealthMetricType.AVAILABILITY]
    ...
    async def _cleanup_old_data(self) -> None:
        """清理旧数据"""
        async with self._health_lock:
            cutoff_time = datetime.now(timezone.utc) - timedelta(seconds=self._history_retention)
            ...
            for adapter_id, history in self._health_history.items():
                ...
                if len(self._health_history[adapter_id]) > self._max_history_size:
                    self._health_history[adapter_id] = self._health_history[adapter_id][-self._max_history_size:]
...
class AvailabilityMonitor(HealthMonitor):
    ...
    async def check_health(self, adapter_id: str, registration: AdapterRegistration) -> List[HealthMetric]:
        ...
        try:
            ...
            try:
                ...
                if hasattr(registration.adapter_class, 'health_check'):
                    health_result = await asyncio.wait_for(
                        registration.adapter_class.health_check(),
                        timeout=5.0
                    )
                ...
                # 3. 尝试基本连接测试
                    ...
                    await asyncio.wait_for(
                        registration.adapter_class.ping(),
                        timeout=3.0
                    )
                    is_available = True
            ...
            availability_rate = success_count / len(recent_checks) if recent_checks else 0.0
        ...
        try:
            ...
            for i in range(5):  # 执行5次测试
                ...
                try:
                    ...
                    logger.warning(f"Response time test {i+1} failed for adapter '{adapter_id}': {e}")
                    # 对于失败的测试，记录一个很高的响应时间
            ...
            if test_results:
                ...
                if len(self._response_times[adapter_id]) > 200:  # 保持最近200次记录
                    self._response_times[adapter_id] = self._response_times[adapter_id][-200:]
                ...
                if len(all_times) >= 20:
                    ...
                    trend = ((recent_avg - historical_avg) / historical_avg) * 100 if historical_avg > 0 else 0
...
class ErrorRateMonitor(HealthMonitor):
    ...
    def __init__(self):
        ...
        self._error_types: Dict[str, Dict[str, int]] = {}  # adapter_id -> {error_type: count}
    ...
    async def check_health(self, adapter_id: str, registration: AdapterRegistration) -> List[HealthMetric]:
        ...
        try:
                ...
                # 尝试执行简单操作来检测错误
                    ...
                    for op in test_operations:
                        if hasattr(registration.adapter_class, op):
                            try:
                                if asyncio.iscoroutinefunction(getattr(registration.adapter_class, op)):
            ...
            if current_errors > 0:
                for error_type, count in error_details.items():
                    for _ in range(count):
                        self._error_history[adapter_id].append((current_time, error_type))
            ...
            self._error_history[adapter_id] = [
                (time, error_type) for time, error_type in self._error_history[adapter_id]
                if time >= cutoff_time
            ]
            ...
            recent_errors = [
                error for time, error in self._error_history[adapter_id]
                if time >= recent_cutoff
            ]
            recent_error_rate = len(recent_errors) / max(1, total_operations) if total_operations > 0 else 0.0
            ...
            prev_hour_errors = len([
                error for time, error in self._error_history[adapter_id]
                if current_time - timedelta(hours=2) <= time < current_time - timedelta(hours=1)
            ])
            
            error_trend = 0.0
            if prev_hour_errors > 0:
                error_trend = ((last_hour_errors - prev_hour_errors) / prev_hour_errors) * 100
            ...
            if self._error_types[adapter_id]:
                most_common_error = max(self._error_types[adapter_id].items(), key=lambda x: x[1])
                metrics.append(HealthMetric(
            ...
            # 错误稳定性评分
            stability_score = self._calculate_stability_score(overall_error_rate, recent_error_rate, error_trend)
            metrics.append(HealthMetric(
    ...
    def _calculate_stability_score(self, overall_rate: float, recent_rate: float, trend: float) -> float:
        """计算稳定性分数"""
        base_score = 100.0
    ...
    async def check_health(self, adapter_id: str, registration: AdapterRegistration) -> List[HealthMetric]:
        ...
        try:
            ...
            if hasattr(registration.adapter_class, 'get_memory_usage'):
                try:
                    adapter_memory_mb = await registration.adapter_class.get_memory_usage()
                except:
                    # fallback to estimation
                    adapter_memory_mb = sys.getsizeof(registration.adapter_class) / (1024 * 1024)
            else:
                # 估算适配器内存使用
                try:
                    # 触发垃圾回收获得更准确的内存状态
                    ...
                    if hasattr(registration, 'instance') and registration.instance:
                        adapter_memory_mb += sys.getsizeof(registration.instance) / (1024 * 1024)
            ...
            logger.warning(f"psutil not available for memory monitoring of adapter '{adapter_id}'")
            return []
        except Exception as e:
            logger.error(f"Memory usage check failed for adapter '{adapter_id}': {e}")
            return []
    
    def _calculate_memory_efficiency_score(self, adapter_memory: float, 
                                         system_usage: float, trend: float) -> float:
        """计算内存效率分数"""
        score = 100.0
...
class ThroughputMonitor(HealthMonitor):
    ...
    async def check_health(self, adapter_id: str, registration: AdapterRegistration) -> List[HealthMetric]:
        ...
        try:
            ...
            self._request_counts[adapter_id] = [
                (time, count) for time, count in self._request_counts[adapter_id]
                if time >= cutoff_time
            ]
            ...
            if len(self._request_counts[adapter_id]) >= 2:
                # 计算每分钟请求数
                time_span = (self._request_counts[adapter_id][-1][0] - self._request_counts[adapter_id][0][0]).total_seconds()
                total_requests = sum(count for _, count in self._request_counts[adapter_id])
                ...
                # 吞吐量评分
                performance_score = self._calculate_throughput_score(requests_per_second, throughput_trend)
                metrics.append(HealthMetric(
                ...
                return [
                    HealthMetric(
                        ...
                        value=0.0,
                        unit="info",
                        metadata={"message": "Insufficient data for throughput calculation"}
                    )
                ]
