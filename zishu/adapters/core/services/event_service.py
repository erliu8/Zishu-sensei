...
class AdapterEventService(AsyncService):
    ...
    async def _start_impl(self) -> None:
        ...
        await self.emit_event(Event(
            ...
            data={"service": "adapter_event", "timestamp": datetime.now(timezone.utc)}
        ))
    ...
    async def subscribe(self, subscriber_id: str, event_types: Union[EventType, List[EventType]], 
                       handler: EventHandler, delivery_mode: EventDeliveryMode = EventDeliveryMode.ASYNC,
                       priority: Priority = Priority.NORMAL,
        ...
        logger.info(f"Added event subscription: {subscription_id} for types {event_type_set}")
    ...
    async def emit_event(self, event: Event) -> None:
        ...
        try:
            ...
            self._metrics.events_by_priority[priority_str] = self._metrics.events_by_priority.get(priority_str, 0) + 1
    ...
    async def _process_event(self, event: Event) -> None:
        ...
        for subscription in matching_subscriptions:
            if subscription.delivery_mode == EventDeliveryMode.SYNC:
                ...
                # 异步传递
                task = asyncio.create_task(self._deliver_event_async(event, subscription))
        ...
        for subscription_id in subscription_ids:
            ...
            if subscription.filter_func:
                try:
                    ...
                    logger.error(f"Event filter failed for subscription {subscription_id}: {e}")
                    continue
    ...
    async def _deliver_event_sync(self, event: Event, subscription: EventSubscription) -> EventDeliveryResult:
        """同步传递事件"""
        ...
        try:
            ...
            self._metrics.failed_events += 1
            logger.error(f"Sync event delivery failed for {subscription.subscriber_id}: {e}")
        
        self._delivery_results.append(result)
        return result
    
    async def _deliver_event_async(self, event: Event, subscription: EventSubscription) -> EventDeliveryResult:
        """异步传递事件"""
        start_time = asyncio.get_event_loop().time()
        ...
        for attempt in range(self._retry_attempts):
            try:
                ...
                if attempt == self._retry_attempts - 1:
                    ...
                    self._metrics.failed_events += 1
                    logger.error(f"Async event delivery failed for {subscription.subscriber_id}: {e}")
                else:
                    await asyncio.sleep(self._retry_delay * (attempt + 1))
    ...
    async def _cleanup_old_delivery_results(self) -> None:
        ...
        # 移除超过1小时的结果
        while self._delivery_results and self._delivery_results[0].timestamp < cutoff_time:
...
class LoggingEventHandler(EventHandler):
    ...
    async def handle_event(self, event: Event) -> None:
        """处理事件"""
        self.logger.info(f"Received event: {event.event_type.value} from {event.source}")
        if event.data:
            self.logger.debug(f"Event data: {json.dumps(event.data, default=str, indent=2)}")
