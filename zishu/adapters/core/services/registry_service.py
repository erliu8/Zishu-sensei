...
from ..types import (
    ...
    AdapterStatus, LifecycleState, EventType, Event, Priority, AdapterType
)
...
class AdapterRegistryService(AsyncService):
    ...
    async def _start_impl(self) -> None:
        ...
        if self._event_bus:
            await self._event_bus.emit(Event(
                ...
                data={"service": "adapter_registry", "timestamp": datetime.now(timezone.utc)}
            ))
        ...
        try:
            async with self._registration_lock:
                ...
                if len(self._registrations) >= self._max_registrations:
                    raise RuntimeError(f"Maximum registrations limit reached: {self._max_registrations}")
    ...
    async def list_adapters(self, 
                           ...
                           status: Optional[AdapterStatus] = None) -> List[AdapterRegistration]:
        """列出适配器"""
        self._metrics.request_count += 1
    ...
    async def update_adapter_status(self, adapter_id: str, status: AdapterStatus) -> bool:
        ...
        try:
            async with self._registration_lock:
                ...
                logger.info(f"Adapter '{adapter_id}' status updated: {old_status} -> {status}")
    ...
    async def _validate_configuration(self, config: AdapterConfiguration) -> None:
        ...
        if not hasattr(config, 'name') or not config.name:
            logger.warning(f"Adapter name not provided for {config.identity}, using identity as name")
        
        # 可以添加更多验证逻辑
        logger.debug(f"Configuration validation passed for adapter: {config.identity}")
    
    def _update_indexes(self, registration: AdapterRegistration, add: bool = True) -> None:
        """更新索引"""
    ...
    def _estimate_memory_usage(self) -> int:
        ...
        total_size += sum(len(adapters) for adapters in self._adapters_by_type.values()) * index_size
        total_size += sum(len(adapters) for adapters in self._adapters_by_category.values()) * index_size
    ...
    async def _perform_cleanup(self) -> None:
        ...
        for adapter_id, registration in self._registrations.items():
            ...
            if registration.last_updated:
                inactive_time = (current_time - registration.last_updated).total_seconds()
                if inactive_time > max_inactive_time and registration.status == AdapterStatus.INACTIVE:
                    expired_adapters.append(adapter_id)
        ...
        if expired_adapters:
            logger.info(f"Cleanup completed, removed {len(expired_adapters)} expired adapters")
