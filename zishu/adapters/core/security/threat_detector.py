...
class ThreatDetector:
    ...
    def __init__(self):
        ...
        self._adapter_behavior: Dict[str, deque] = defaultdict(lambda: deque(maxlen=1000))
    ...
    async def analyze_code(
        ...
        try:
            ...
            # 生成建议
            if result.is_threat:
                result.recommendations = self._generate_recommendations(result.threat_events)
    ...
    async def _analyze_import_node(
        ...
        suspicious_modules = [mod for mod in imported_modules if mod in self._suspicious_imports]
        
        if suspicious_modules:
            threat_level = ThreatLevel.HIGH if any(mod in ['os', 'subprocess', 'eval'] for mod in suspicious_modules) else ThreatLevel.MEDIUM
    ...
    async def _analyze_attribute_node(
        ...
        dangerous_attributes = {
            ...
            '__import__', '__getattribute__', '__setattr__'
        }
    ...
    async def analyze_behavior(
        ...
        try:
            ...
            if 'ip_address' in context:
                ip_threats = await self._analyze_ip_behavior(context['ip_address'], action, context)
    ...
    async def _analyze_ip_behavior(
        ...
        # 检查IP频率
        recent_actions = [
            action for action in self._ip_behavior[ip_address]
        ...
        if len(recent_actions) > 100:  # 1分钟内超过100个请求
            threat = ThreatEvent(
                ...
                confidence_score=0.9,
                evidence={
                    'request_count': len(recent_actions),
                    'time_window': 60
                }
            )
            threats.append(threat)
    ...
    def _extract_code_snippet(self, code: str, match: str, context_lines: int = 2) -> str:
        """提取代码片段"""
        lines = code.split('\n')
    ...
    def _calculate_alert_severity(self, threat_events: List[ThreatEvent]) -> AuditSeverity:
        """计算警报严重程度"""
