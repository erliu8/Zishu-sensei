...
class SecurityContextManager:
    ...
    def __init__(self, default_timeout: int = 3600):
        ...
        # 验证器
        self._validators: List[Callable[[SecurityContext], ContextValidationResult]] = []
        ...
        self._context_validated_callbacks: List[Callable[[SecurityContext, bool], None]] = []
    ...
    async def create_context(
        ...
        try:
            ...
            logger.info(f"Security context created for user {user_id}, session {session_id}")
            return context
    ...
    async def validate_context(self, context: SecurityContext) -> ContextValidationResult:
        """验证安全上下文"""
    ...
    async def refresh_context(self, session_id: str, extend_timeout: Optional[int] = None) -> bool:
        """刷新上下文（延长过期时间）"""
        async with self._lock:
            context = self._contexts.get(session_id)
            if not context:
            ...
            # 延长过期时间
            if extend_timeout:
                context.expires_at = datetime.now(timezone.utc) + timedelta(seconds=extend_timeout)
            else:
                context.expires_at = datetime.now(timezone.utc) + timedelta(seconds=self.default_timeout)
    ...
    def _validate_basic_fields(self, context: SecurityContext) -> ContextValidationResult:
        """验证基本字段"""
    ...
    def _validate_permissions(self, context: SecurityContext) -> ContextValidationResult:
        """验证权限"""
        result = ContextValidationResult(is_valid=True)
        ...
        if 'admin' in context.permissions and len(context.permissions) == 1:
            result.add_warning("Admin permission should be combined with specific permissions")
    ...
    def _validate_security_level(self, context: SecurityContext) -> ContextValidationResult:
        """验证安全级别"""
        result = ContextValidationResult(is_valid=True)
        ...
        if context.security_level == SecurityLevel.CONFIDENTIAL:
            if 'admin' not in context.permissions:
                result.add_warning("Confidential security level without admin permission")
    ...
    def add_context_created_callback(self, callback: Callable[[SecurityContext], None]) -> None:
        """添加上下文创建回调"""
        self._context_created_callbacks.append(callback)
    
    def add_context_expired_callback(self, callback: Callable[[SecurityContext], None]) -> None:
        """添加上下文过期回调"""
        self._context_expired_callbacks.append(callback)
    
    def add_context_validated_callback(self, callback: Callable[[SecurityContext, bool], None]) -> None:
        """添加上下文验证回调"""
        self._context_validated_callbacks.append(callback)
    
    def add_validator(self, validator: Callable[[SecurityContext], ContextValidationResult]) -> None:
        """添加自定义验证器"""
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            expired_sessions = len([s for s in self._sessions.values() if s.is_expired()])
...
class SessionManager:
        ...
        while self._running:
            try:
                ...
                if stats['active_sessions'] > 0:
                    logger.debug(f"Session cleanup: removed {expired_count} expired sessions, "
    ...
    async def enforce_session_limits(self, user_id: str) -> None:
        ...
        if len(sessions) > self.max_sessions_per_user:
            ...
            logger.info(f"Removed {len(sessions_to_remove)} excess sessions for user {user_id}")
    ...
    def _get_health_recommendations(self, stats: Dict[str, Any], usage_ratio: float) -> List[str]:
        """获取健康建议"""
        recommendations = []
        ...
        if stats['expired_sessions'] > stats['active_sessions']:
            recommendations.append("Many expired sessions detected, check cleanup process")
        
        if stats['unique_users'] > 0:
            avg_sessions_per_user = stats['active_sessions'] / stats['unique_users']
            if avg_sessions_per_user > 5:
                recommendations.append("High average sessions per user, consider session limits")
