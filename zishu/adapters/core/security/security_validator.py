...
from typing import (
    ...
    Tuple, Protocol, TYPE_CHECKING, NamedTuple, Iterator
)
import weakref
import ipaddress
import urllib.parse
...
class SecurityValidator(ABC):
    ...
    async def _record_validation(
        ...
        self.stats['average_time'] = (total_time + execution_time) / self.stats['total_validations']
...
class InputValidator(SecurityValidator):
    ...
    async def validate(
        ...
        try:
            ...
            elif any(v.severity in [SecurityEventSeverity.HIGH, SecurityEventSeverity.CRITICAL] 
                    for v in violations):
...
class BusinessLogicValidator(SecurityValidator):
    ...
    async def _check_rate_limits(self, context: ValidationContext) -> List[SecurityViolation]:
        """检查速率限制"""
        violations = []
        ...
        if len(requests) >= limit:
            violations.append(
                SecurityViolation(
                    ...
                    details={'ip': context.ip_address, 'count': len(requests), 'limit': limit},
                    source_ip=context.ip_address,
                    user_id=context.user_id,
                    session_id=context.session_id
                )
    ...
    async def _check_failed_attempts(self, context: ValidationContext) -> List[SecurityViolation]:
        """检查失败尝试"""
        violations = []
        ...
        if len(attempts) >= limit:
            violations.append(
                SecurityViolation(
                    ...
                    details={'user_id': context.user_id, 'count': len(attempts), 'limit': limit},
                    source_ip=context.ip_address,
                    user_id=context.user_id,
                    session_id=context.session_id
                )
    ...
    async def _validate_session(self, context: ValidationContext) -> List[SecurityViolation]:
        """验证会话"""
        violations = []
    ...
    async def _validate_time_window(self, context: ValidationContext) -> List[SecurityViolation]:
        """验证时间窗口"""
        violations = []
...
class SecurityAuditor:
    ...
    async def _check_alert_conditions(self, violation: SecurityViolation) -> None:
        """检查告警条件"""
        # 高危威胁立即告警
        if violation.severity in [SecurityEventSeverity.HIGH, SecurityEventSeverity.CRITICAL]:
            await self._send_alert(violation, "High severity security violation detected")
        ...
        if len(recent_threats) >= 5:  # 5次相同威胁
            ...
            recent_count = sum(
                1 for timestamp in recent_threats
    ...
    def get_user_risk_score(self, user_id: str) -> float:
        ...
        for violation in user_violations:
            ...
            decay_factor = max(0.1, 1.0 - (age_hours - 24) * 0.1 / 24) if age_hours > 24 else 1.0
...
class ComprehensiveSecurityValidator:
    ...
    async def validate_request(
        ...
        try:
            ...
            # 2. 业务逻辑验证
            logic_result, logic_violations = await self.validators['business_logic'].validate(
                input_data, context
            )
            ...
            has_critical_violations = any(
                v.severity in [SecurityEventSeverity.HIGH, SecurityEventSeverity.CRITICAL]
            ...
            self.stats['average_validation_time'] = (total_time + validation_time) / self.stats['total_validations']
...
def validate_input(level: ValidationLevel = ValidationLevel.STANDARD):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            passed, violations = await validator.validate_request(input_data, context, False)
            
            if not passed:
                critical_violations = [
                    v for v in violations
                    if v.severity in [SecurityEventSeverity.HIGH, SecurityEventSeverity.CRITICAL]
                ]
                if critical_violations:
                    raise SecurityError(f"Input validation failed: {critical_violations[0].message}")
