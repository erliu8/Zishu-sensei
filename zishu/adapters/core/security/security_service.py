...
class AdapterSecurityService(AsyncService):
    ...
    async def create_security_context(
        ...
        try:
            ...
            # 检查会话数限制
            if len(self._user_sessions[user_id]) >= self.security_config.max_sessions_per_user:
                # 清理最旧的会话
    ...
    async def detect_threats(
        ...
        try:
            ...
            if 'code' in request_data:
                code_violations = await self._detect_malicious_code(request_data['code'])
                violations.extend(code_violations)
            
            # 检查异常行为模式
            behavior_violations = await self._detect_suspicious_behavior(user_id, request_data)
    ...
    async def _is_user_locked(self, user_id: str) -> bool:
        ...
        if user_id in self._locked_accounts:
            ...
            if (datetime.now(timezone.utc) - lock_time).seconds < self.security_config.lockout_duration:
                return True
            else:
                # 锁定时间已过，解除锁定
    ...
    async def _record_violation_obj(self, violation: SecurityViolation) -> None:
        ...
        # 检查是否需要标记可疑IP
        if (violation.ip_address and 
            violation.severity in [AuditSeverity.HIGH, AuditSeverity.CRITICAL]):
        ...
        while self.is_running:
            try:
                ...
                for key in list(self._rate_limits.keys()):
                    requests = self._rate_limits[key]
                    window_start = current_time - timedelta(seconds=self.security_config.rate_limit_window * 2)
                    while requests and requests[0] < window_start:
                        requests.popleft()
                ...
                for user_id, lock_time in self._locked_accounts.items():
                    if (current_time - lock_time).seconds > self.security_config.lockout_duration:
