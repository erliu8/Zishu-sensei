...
from typing import (
    ...
    Tuple, Protocol, TYPE_CHECKING, NamedTuple
)
...
class EnhancedPermissionManager:
    ...
    def _initialize_defaults(self) -> None:
        ...
        # 创建基础权限
        base_permissions = [
            Permission("read_basic", PermissionType.READ, PermissionScope.GLOBAL, "基础读取权限"),
            Permission("write_basic", PermissionType.WRITE, PermissionScope.GLOBAL, "基础写入权限"),
            Permission("execute_basic", PermissionType.EXECUTE, PermissionScope.GLOBAL, "基础执行权限"),
            Permission("admin_full", PermissionType.ADMIN, PermissionScope.GLOBAL, "完整管理权限"),
            Permission("system_config", PermissionType.SYSTEM_CONFIG, PermissionScope.GLOBAL, "系统配置权限"),
            Permission("adapter_manage", PermissionType.ADAPTER_CONFIGURE, PermissionScope.GLOBAL, "适配器管理权限"),
            Permission("model_access", PermissionType.MODEL_ACCESS, PermissionScope.GLOBAL, "模型访问权限"),
        ]
        ...
        developer_role = Role("developer", RoleType.DEVELOPER, "开发者", 
                            {"read_basic", "write_basic", "execute_basic", "adapter_manage"})
        admin_role = Role("admin", RoleType.ADMIN, "管理员", 
                         {"read_basic", "write_basic", "execute_basic", "adapter_manage", "system_config"})
        super_admin_role = Role("super_admin", RoleType.SUPER_ADMIN, "超级管理员", 
        ...
        # 创建基础策略规则
        basic_rules = [
            PolicyRule("deny_guest_write", "访客不能写入", 
                      "user_role == 'guest' and action in ['write', 'delete', 'create']", 
                      PolicyEffect.DENY, priority=100),
            PolicyRule("allow_admin_all", "管理员允许所有操作", 
                      "user_role in ['admin', 'super_admin']", 
                      PolicyEffect.PERMIT, priority=50),
            PolicyRule("time_restriction", "工作时间限制", 
                      "time_between('09:00', '18:00')", 
                      PolicyEffect.PERMIT, priority=10),
        ]
    ...
    def create_role(self, role: Role) -> bool:
        ...
        try:
            ...
            if invalid_permissions:
                logger.error(f"Invalid permissions in role {role.name}: {invalid_permissions}")
                return False
    ...
    def update_role(self, name: str, role: Role) -> bool:
        ...
        try:
            ...
            invalid_permissions = role.permissions - set(self.permissions.keys())
            if invalid_permissions:
                logger.error(f"Invalid permissions in role {name}: {invalid_permissions}")
                return False
    ...
    def _evaluate_rbac(self, request: AccessRequest, context: Dict[str, Any]) -> AccessResult:
        """评估基于角色的访问控制"""
        ...
        return AccessResult(
            decision=AccessDecision.ABSTAIN,
            reason="No RBAC permissions matched"
        )
    
    def _evaluate_abac(self, request: AccessRequest, context: Dict[str, Any]) -> AccessResult:
        """评估基于属性的访问控制"""
        # 简化的ABAC实现
        # 检查特殊条件
        ...
        # 时间限制检查
        if not self.condition_evaluator.evaluate("time_between('06:00', '23:00')", context):
            return AccessResult(
                decision=AccessDecision.DENY,
        ...
        return AccessResult(
            decision=AccessDecision.ABSTAIN,
            reason="No ABAC conditions matched"
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        stats.update({
            ...
            'average_evaluation_time': (
                self.stats['evaluation_time_total'] / max(self.stats['total_checks'], 1)
            )
        })
        return stats
