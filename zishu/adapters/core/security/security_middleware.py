...
class SecurityMiddleware(ABC):
    ...
    @abstractmethod
    async def process(
        self, context: RequestSecurityContext
    ...
    async def post_process(
        self, context: RequestSecurityContext, result: SecurityMiddlewareResult
    ) -> None:
        """后处理（可选重写）"""
        pass

    def update_stats(
        self, result: SecurityMiddlewareResult, processing_time: float
    ) -> None:
        """更新统计信息"""
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if stats["total_requests"] > 0:
            stats["average_processing_time"] = (
                stats["processing_time"] / stats["total_requests"]
            )
...
class AuthenticationMiddleware(SecurityMiddleware):
    ...
    async def process(
        ...
        # 验证会话
        security_context = await self.security_service.get_security_context(
            context.session_id
        )
...
class AuthorizationMiddleware(SecurityMiddleware):
    ...
    async def process(
        ...
        has_permission = await self.security_service.check_permission(
            context.session_id, context.resource, context.action, context.adapter_id
        )
        ...
        if "code" in context.payload:
            ...
            if code_analysis.is_threat:
                ...
                max_threat_level = max(
                    event.threat_level for event in code_analysis.threat_events
                )
                ...
                return SecurityMiddlewareResult(
                    action=action,
                    decision=SecurityDecision.REJECTED
                    if action in [SecurityAction.BLOCK, SecurityAction.QUARANTINE]
                    else SecurityDecision.CONDITIONAL_APPROVAL,
                    ...
                    details={
                        ...
                        "threats": [
                            event.to_dict() for event in code_analysis.threat_events
                        ],
                    },
                    processed_by=[self.name],
                )
        ...
        if context.user_id:
            ...
            if behavior_analysis.is_threat:
                return SecurityMiddlewareResult(
                    ...
                    details={
                        "behavior_threats": [
                            event.to_dict() for event in behavior_analysis.threat_events
                        ]
                    },
                    processed_by=[self.name],
                )
...
class RateLimitMiddleware(SecurityMiddleware):
    ...
    async def process(
        ...
        self.request_history[limit_key] = [
            req_time
            for req_time in self.request_history[limit_key]
...
class SecurityInterceptor:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats["middleware_stats"] = {
            middleware.name: middleware.get_stats() for middleware in self.middlewares
        }
