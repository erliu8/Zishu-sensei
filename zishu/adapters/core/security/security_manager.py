...
class SecurityManager:
    ...
    def _setup_component_callbacks(self) -> None:
        ...
        # 注册回调
        self.threat_detector.add_context_created_callback = lambda cb: None  # 威胁检测器没有这个方法
    ...
    async def authenticate_user(
        ...
        """用户认证"""
        return await self.security_service.authenticate_user(user_id, credentials, context)
    ...
    async def get_security_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'security_interceptor': {
                'stats': self.security_interceptor.get_stats()
            }
        }
    ...
    async def export_security_report(
        ...
        return {
            ...
            'resource_metrics': self.resource_monitor.export_metrics(start_time, end_time),
        ...
        try:
            ...
            if not passed:
                ...
                if critical_violations:
                    ...
                    raise SecurityError(f"Code validation failed: {critical_violations[0].message}")
    ...
    def get_user_security_profile(self, user_id: str) -> Dict[str, Any]:
        ...
        # 添加权限信息
        try:
            user_permissions = self.permission_manager.get_user_effective_permissions(user_id)
            profile["permissions"] = {
                "effective_permissions": [p.to_dict() for p in user_permissions],
                "roles": [role.name for role in self.permission_manager.get_user_roles(user_id)]
            }
        except Exception as e:
