...
class AdapterStorage(IAdapterStorage):
    ...
    async def _create_indexes(self) -> None:
        ...
        await self.backend.create_index(
            self.REGISTRATIONS_COLLECTION, ["adapter_id"], unique=True
        )
        await self.backend.create_index(self.REGISTRATIONS_COLLECTION, ["name"])
        await self.backend.create_index(self.REGISTRATIONS_COLLECTION, ["version"])
        await self.backend.create_index(self.REGISTRATIONS_COLLECTION, ["status"])

        # 配置索引
        await self.backend.create_index(
            self.CONFIGURATIONS_COLLECTION, ["adapter_id"], unique=True
        )

        # 状态索引
        await self.backend.create_index(
            self.STATES_COLLECTION, ["adapter_id"], unique=True
        )

        # 指标索引
        await self.backend.create_index(self.METRICS_COLLECTION, ["adapter_id"])
        await self.backend.create_index(self.METRICS_COLLECTION, ["timestamp"])
    ...
    async def save_registration(self, registration: AdapterRegistration) -> bool:
        ...
        try:
            ...
            result = await self.backend.create(self.REGISTRATIONS_COLLECTION, data)
    ...
    async def load_registration(self, adapter_id: str) -> Optional[AdapterRegistration]:
        ...
        try:
            ...
            result = await self.backend.read(self.REGISTRATIONS_COLLECTION, adapter_id)
    ...
    async def update_registration(
        ...
        try:
            ...
            result = await self.backend.update(
                self.REGISTRATIONS_COLLECTION, adapter_id, data
            )
    ...
    async def delete_registration(self, adapter_id: str) -> bool:
        ...
        try:
            ...
            result = await self.backend.delete(
                self.REGISTRATIONS_COLLECTION, adapter_id
            )
    ...
    async def list_registrations(
        ...
        try:
            ...
            result = await self.backend.list(self.REGISTRATIONS_COLLECTION, query)
    ...
    async def save_configuration(
        ...
        try:
            ...
            # 尝试更新，如果不存在则创建
            result = await self.backend.update(
                self.CONFIGURATIONS_COLLECTION, adapter_id, data
            )

            if not result.success:
                result = await self.backend.create(self.CONFIGURATIONS_COLLECTION, data)
    ...
    async def load_configuration(
        ...
        try:
            ...
            result = await self.backend.read(self.CONFIGURATIONS_COLLECTION, adapter_id)
    ...
    async def save_state(self, adapter_id: str, state: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # 尝试更新，如果不存在则创建
            result = await self.backend.update(self.STATES_COLLECTION, adapter_id, data)

            if not result.success:
                result = await self.backend.create(self.STATES_COLLECTION, data)
    ...
    async def load_state(self, adapter_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = await self.backend.read(self.STATES_COLLECTION, adapter_id)
    ...
    async def save_metrics(self, adapter_id: str, metrics: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            result = await self.backend.create(self.METRICS_COLLECTION, data)
    ...
    async def load_metrics(
        ...
        try:
            ...
            query = StorageQuery(
                filters=filters, sort_by="timestamp", sort_order="desc"
            )

            result = await self.backend.list(self.METRICS_COLLECTION, query)
    ...
    async def cleanup_old_metrics(self, retention_days: int = 30) -> int:
        ...
        try:
            ...
            query = StorageQuery(
                filters={"timestamp": {"$lt": cutoff_time.isoformat()}}
            )
            ...
            if result.success and result.data:
                keys_to_delete = [
                    item.get("_key") for item in result.data if item.get("_key")
                ]

                if keys_to_delete:
                    delete_result = await self.backend.batch_delete(
                        self.METRICS_COLLECTION, keys_to_delete
                    )
    ...
    async def get_adapter_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            registrations_result = await self.backend.list(
                self.REGISTRATIONS_COLLECTION, StorageQuery(filters={})
            )
            ...
            configs_result = await self.backend.list(
                self.CONFIGURATIONS_COLLECTION, StorageQuery(filters={})
            )
            ...
            metrics_result = await self.backend.list(
                self.METRICS_COLLECTION, StorageQuery(filters={}, limit=1000)
            )
