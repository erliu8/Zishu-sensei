...
class PostgreSQLStorageBackend(IStorageBackend):
    ...
    async def connect(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Connected to PostgreSQL database: {self._pool_config['host']}:{self._pool_config['port']}/{self._pool_config['database']}"
            )
            return True
    ...
    async def _ensure_table_exists(self, collection: str) -> None:
        ...
        async with self._connection_pool.acquire() as conn:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to create default indexes for {table_name}: {e}"
                )
        ...
        for field, value in filters.items():
            if isinstance(value, dict):
                ...
                for op, op_value in value.items():
                    if op == "$eq":
                        conditions.append(
                            f"data->>${param_counter} = ${param_counter + 1}"
                        )
                        ...
                        conditions.append(
                            f"(data->>${param_counter})::numeric > ${param_counter + 1}"
                        )
                        params.extend([field, op_value])
                        param_counter += 2
                    elif op == "$gte":
                        ...
                        placeholders = ",".join(
                            [f"${param_counter + i}" for i in range(len(op_value))]
                        )
                        conditions.append(
                            f"data->>${param_counter} IN ({placeholders})"
                        )
                        params.append(field)
                        ...
                        conditions.append(
                            f"data->>${param_counter} NOT IN ({placeholders})"
                        )
                        params.append(field)
                        params.extend(op_value)
                        param_counter += len(op_value) + 1
    ...
    def _apply_query_options(
        self, base_sql: str, query: StorageQuery, param_counter: int
    ) -> tuple:
        """应用查询选项（排序、分页等）"""
        sql_parts = [base_sql]
        params = []
    ...
    async def read(
        ...
        try:
            ...
            async with self._connection_pool.acquire() as conn:
                ...
                # 处理时间戳
                if "created_at" in record_dict and hasattr(
                    record_dict["created_at"], "isoformat"
                ):
                    record_dict["created_at"] = record_dict["created_at"].isoformat()
                if "updated_at" in record_dict and hasattr(
                    record_dict["updated_at"], "isoformat"
                ):
                    record_dict["updated_at"] = record_dict["updated_at"].isoformat()

                return StorageResult(
                    success=True, data=record_dict, operation_time=datetime.utcnow()
                )
    ...
    async def delete(
        ...
        try:
            ...
            async with self._connection_pool.acquire() as conn:
                ...
                # 删除记录
                await conn.execute(f"DELETE FROM {table_name} WHERE key = $1", key)
    ...
    async def list(
        ...
        try:
            ...
            # 应用查询选项
            final_sql, query_params = self._apply_query_options(
                base_sql, query, param_counter
            )
                        ...
                        result.append(
                            {
                                ...
                                if hasattr(index["created_at"], "isoformat")
                                else index["created_at"],
                            }
                        )
