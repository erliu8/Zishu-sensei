...
class SQLiteStorageBackend(IStorageBackend):
    ...
    async def _execute_with_connection(self, sql: str, params: tuple = (), 
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            tables = [info['name'] for info in db_info if not info['name'].startswith('_')]
            ...
            if isinstance(value, dict):
                ...
                for op, op_value in value.items():
                    if op == '$eq':
                        ...
                        placeholders = ','.join(['?' for _ in op_value])
                        conditions.append(f"JSON_EXTRACT(data, '$.{field}') NOT IN ({placeholders})")
                        params.extend(op_value)
                    elif op == '$regex':
    ...
    def _apply_query_options(self, base_sql: str, query: StorageQuery) -> tuple:
        ...
        if query.sort_by:
            ...
            sql_parts.append(f"ORDER BY JSON_EXTRACT(data, '$.{query.sort_by}') {order}")
    ...
    def _process_record_fields(self, record: Dict[str, Any], query: StorageQuery) -> Dict[str, Any]:
        ...
        if query.include_fields:
            filtered_record = {k: v for k, v in record.items() 
                             if k in query.include_fields or k in ['key', '_created_at', '_updated_at']}
    ...
    async def create(self, collection: str, data: Dict[str, Any], 
        ...
        try:
            ...
            existing = await self._execute_with_connection(
                f"SELECT key FROM {table_name} WHERE key = ?",
                (key,),
                fetch_one=True
            )
    ...
    async def update(self, collection: str, key: str, data: Dict[str, Any],
        ...
        try:
            ...
            try:
                ...
                return StorageResult(
                    success=False,
                    error=f"Failed to parse existing record data: {e}"
                )
    ...
    async def list(self, collection: str, query: Optional[StorageQuery] = None,
        ...
        try:
            ...
            # 执行查询
            records = await self._execute_with_connection(final_sql, all_params, fetch_all=True)
            ...
            try:
                ...
                for key in keys:
                    try:
                        ...
                        if 'data' in record_dict and isinstance(record_dict['data'], str):
                            try:
    ...
    async def create_index(self, collection: str, fields: List[str], 
        ...
        try:
            ...
            await self._execute_with_connection(
                ...
                (collection, index_name, json.dumps(fields), int(unique), datetime.utcnow().isoformat())
            )
