...
class MemoryStorageBackend(IStorageBackend):
    ...
    def _apply_query_filters(self, data: Dict[str, Any], 
    ...
    def _get_data_source(self, collection: str, 
    ...
    def _update_data_source(self, collection: str, key: str, data: Any,
                           transaction_id: Optional[str] = None) -> None:
        """更新数据源"""
        if transaction_id and transaction_id in self._transaction_data:
            if collection not in self._transaction_data[transaction_id]:
                self._transaction_data[transaction_id][collection] = copy.deepcopy(self._data[collection])
            self._transaction_data[transaction_id][collection][key] = data
        else:
            self._data[collection][key] = data
    
    def _delete_from_data_source(self, collection: str, key: str,
                                transaction_id: Optional[str] = None) -> None:
        """从数据源删除"""
        if transaction_id and transaction_id in self._transaction_data:
            if collection not in self._transaction_data[transaction_id]:
        ...
        try:
            with self._lock:
                ...
                if transaction_id in self._transaction_data:
                    for collection, data in self._transaction_data[transaction_id].items():
        ...
        try:
            with self._lock:
                if (collection in self._indexes and 
                    index_name in self._indexes[collection]):
