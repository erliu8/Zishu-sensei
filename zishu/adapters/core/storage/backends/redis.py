...
class RedisStorageBackend(IStorageBackend):
    ...
    async def connect(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Connected to Redis: {redis_config['host']}:{redis_config['port']}/{redis_config['db']}"
            )
            return True
    ...
    async def _update_collection_metadata(
        self, collection: str, operation: str, key: str = None
    ) -> None:
        """更新集合元数据"""
        try:
            ...
            logger.warning(
                f"Failed to update collection metadata for {collection}: {e}"
            )
    ...
    def _apply_field_selection(
        ...
        if query.include_fields:
            return {
                ...
                or k in ["_key", "_created_at", "_updated_at"]
            }
        ...
        try:
            ...
            if self._default_ttl:
                await self._redis_client.setex(
                    redis_key, self._default_ttl, serialized_data
                )
            else:
    ...
    async def update(
        ...
        try:
            ...
            return StorageResult(
                success=True, data=existing_data, operation_time=datetime.utcnow()
            )
        ...
        try:
            ...
            async for redis_key in self._redis_client.scan_iter(match=pattern):
                try:
                    ...
                    if data_str:
                        ...
                        # 应用过滤条件
                        if query.filters and not self._match_filter(
            ...
            if query.sort_by:
                ...
                try:
                    ...
                    # 如果无法比较，按字符串排序
                    records.sort(
                        key=lambda x: str(x.get(query.sort_by, "")), reverse=reverse
                    )
    ...
    async def batch_create(
        ...
        try:
            ...
            for i, (result, record_info) in enumerate(zip(pipe_results[::2], results)):
                if result:  # SETNX返回1表示成功设置
                    ...
                    await self._update_collection_metadata(
                        collection, "create", record_info["key"]
                    )
                else:
                    errors.append(
                        f"Record with key '{record_info['key']}' already exists"
                    )
            ...
            return StorageResult(
                ...
                metadata={
                    "created_count": len(successful_creates),
    ...
    async def begin_transaction(self) -> str:
        ...
        await self._redis_client.set(
            transaction_key, self._serialize_data(transaction_data), ex=3600  # 1小时过期
        )
    ...
    async def commit_transaction(self, transaction_id: str) -> bool:
        ...
        try:
            ...
            if transaction_str:
                ...
                await self._redis_client.set(
                    transaction_key, self._serialize_data(transaction_data)
                )
