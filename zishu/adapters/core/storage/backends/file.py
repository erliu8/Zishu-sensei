...
class FileStorageBackend(IStorageBackend):
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "disk_usage": {
                    "total_size": self._get_directory_size(self._base_path)
                    if self._base_path
                    else 0
                },
            }
    ...
    def _apply_query_filters(
        self, records: List[Dict[str, Any]], query: StorageQuery
    ) -> List[Dict[str, Any]]:
        """应用查询过滤器"""
        results = []
    ...
    def _get_record_path_for_transaction(
        self, collection: str, key: str, transaction_id: Optional[str] = None
    ) -> Path:
        """获取事务中的记录路径"""
        if transaction_id:
            return (
                self._get_transaction_path(transaction_id) / collection / f"{key}.json"
            )
    ...
    async def create(
        ...
        try:
            ...
            record_path = self._get_record_path_for_transaction(
                collection, key, transaction_id
            )
    ...
    async def list(
        ...
        try:
            if transaction_id:
                collection_path = (
                    self._get_transaction_path(transaction_id) / collection
                )
            else:
                ...
                if transaction_path.exists():
                    for collection_dir in transaction_path.iterdir():
                        if collection_dir.is_dir():
                            target_collection_path = self._get_collection_path(
                                collection_dir.name
                            )
                            ...
                            for file_path in collection_dir.iterdir():
                                if file_path.is_file():
                                    target_file_path = (
                                        target_collection_path / file_path.name
                                    )
