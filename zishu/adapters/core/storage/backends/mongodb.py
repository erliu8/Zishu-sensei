...
class MongoDBStorageBackend(IStorageBackend):
    ...
    def __init__(self):
        ...
        self._supported_operators = {
            '$eq', '$ne', '$gt', '$gte', '$lt', '$lte', 
            '$in', '$nin', '$exists', '$regex', '$text',
            '$and', '$or', '$not', '$nor', '$all', '$size',
            '$elemMatch', '$type', '$mod'
        }
    ...
    def _prepare_document(self, data: Dict[str, Any], key: str = None) -> Dict[str, Any]:
        """准备文档数据"""
        doc = data.copy()
    ...
    async def connect(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # 创建客户端
            self._client = motor.motor_asyncio.AsyncIOMotorClient(uri, **connect_options)
    ...
    async def read(self, collection: str, key: str,
        ...
        try:
            ...
            if not doc:
                return StorageResult(
                    success=False,
                    error=f"Record with _id '{key}' not found"
                )
    ...
    async def delete(self, collection: str, key: str,
        ...
        try:
            ...
            if result.deleted_count == 0:
                return StorageResult(
                    success=False,
                    error=f"Failed to delete record with _id '{key}'"
                )
    ...
    async def list(self, collection: str, query: Optional[StorageQuery] = None,
        ...
        try:
            ...
            # 构建查询条件
            mongo_filter = self._build_mongo_filter(query.filters) if query.filters else {}
            ...
            # 排序
            if query.sort_by:
                sort_spec = self._build_mongo_sort(query.sort_by, query.sort_order or "asc")
    ...
    async def batch_create(self, collection: str, data_list: List[Dict[str, Any]],
        ...
        try:
            ...
            return StorageResult(
                ...
                metadata={'created_count': len(successful_inserts), 'error_count': len(errors)},
                operation_time=datetime.utcnow()
            )
        except Exception as e:
    ...
    async def batch_update(self, collection: str, updates: List[Dict[str, Any]],
        ...
        try:
            ...
            if not bulk_ops:
                return StorageResult(
                    success=False,
                    error="No valid update operations"
                )
