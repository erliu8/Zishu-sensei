...
class StorageConfig:
    ...
    def from_env(cls, prefix: str = "ZISHU_STORAGE") -> "StorageConfig":
        ...
        if redis_host:
            config_data["redis"] = {
                ...
                "max_connections": int(
                    os.getenv(f"{prefix}_REDIS_MAX_CONNECTIONS", 100)
                ),
                "decode_responses": os.getenv(
                    f"{prefix}_REDIS_DECODE_RESPONSES", "true"
                ).lower()
                == "true",
            }
    ...
    @classmethod
    def default(
        cls, backend_type: StorageBackendType = StorageBackendType.SQLITE
    ) -> "StorageConfig":
        """创建默认配置"""
    ...
    def get_backend_config(
        self, backend_type: StorageBackendType = None
    ) -> Dict[str, Any]:
        """获取特定后端的配置"""
        ...
        if not backend_config:
            logger.warning(
                f"No configuration found for backend type: {backend_type.value}"
            )
...
class StorageFactory:
    ...
    def register_backend(
        cls, backend_type: StorageBackendType, backend_class: Type[IStorageBackend]
    ):
        """注册新的存储后端类型"""
    ...
    @classmethod
    def create_backend(
        cls, config: StorageConfig, instance_name: str = "default"
    ) -> IStorageBackend:
        """创建存储后端实例"""
        ...
        logger.info(
            f"Created storage backend instance: {backend_type.value} ({instance_name})"
        )
    ...
    async def create_and_connect(
        ...
        if not success:
            raise StorageException(
                f"Failed to connect to storage backend: {config.get_backend_type().value}"
            )

        logger.info(
            f"Successfully connected to storage backend: {config.get_backend_type().value}"
        )
        return backend

    @classmethod
    def get_instance(
        cls, backend_type: StorageBackendType, instance_name: str = "default"
    ...
    async def close_instance(
        ...
        if cache_key in cls._instances:
            ...
            try:
                ...
                logger.error(
                    f"Failed to close storage backend instance {cache_key}: {e}"
                )
                return False
...
class StorageManager:
    ...
    async def migrate_data(
        self, target_config: StorageConfig, collections: List[str] = None
    ) -> bool:
        """数据迁移到另一个存储后端"""
        if not self.backend:
        ...
        try:
            # 创建目标后端
            target_backend = await StorageFactory.create_and_connect(
                target_config, "migration_target"
            )
            ...
            for collection in collections:
                ...
                if not result.success:
                    logger.error(
                        f"Failed to read from source collection {collection}: {result.error}"
                    )
                    continue

                # 批量写入目标后端
                if result.data:
                    batch_result = await target_backend.batch_create(
                        collection, result.data
                    )
                    if not batch_result.success:
                        logger.error(
                            f"Failed to write to target collection {collection}: {batch_result.error}"
                        )
                        continue

                logger.info(
                    f"Migrated {len(result.data)} records from collection {collection}"
                )
    ...
    async def backup_data(
        self, backup_path: str, collections: List[str] = None
    ) -> bool:
        """备份数据"""
        if not self.backend:
        ...
        try:
            ...
            for collection in collections:
                ...
                if result.success:
                    ...
                    logger.error(
                        f"Failed to backup collection {collection}: {result.error}"
                    )
        ...
        try:
            ...
            for collection, data in backup_data.items():
                if data:
                    ...
                    if not result.success:
                        logger.error(
                            f"Failed to restore collection {collection}: {result.error}"
                        )
                    else:
                        logger.info(
                            f"Restored {len(data)} records to collection {collection}"
                        )
