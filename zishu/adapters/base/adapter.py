...
class PerformanceMonitor:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_execution_time = (
                sum(r.execution_time for r in recent_executions) / total_count
            )

            return {
                "total_executions": len(self.execution_history),
                "recent_success_rate": success_count / total_count
                if total_count > 0
                else 0,
                "average_execution_time": avg_execution_time,
                "last_execution": recent_executions[-1].created_at.isoformat()
    ...
    def _calculate_trend(self) -> str:
        ...
        older_times = (
            self.metrics["execution_time"][-20:-10]
            if len(self.metrics["execution_time"]) >= 20
            else []
        )
...
class BaseAdapter(ABC):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.logger.info(
            f"Adapter {self.adapter_id} created with config: {self._safe_config_repr()}"
        )
    ...
    async def initialize(self) -> bool:
        ...
        with self._initialization_lock:
            ...
            try:
                ...
                self.logger.debug(
                    f"Capabilities loaded: {len(self.capabilities)} capabilities"
                )
                ...
                if initialization_success:
                    ...
                    self.logger.info(
                        f"Adapter {self.adapter_id} initialized successfully"
                    )
                    return True
                else:
                ...
                self.status = AdapterStatus.ERROR
                self.logger.error(
                    f"Failed to initialize adapter {self.adapter_id}: {e}"
                )
    ...
    async def process(
        ...
        try:
            ...
            self.logger.debug(
                f"Processing request {exec_context.execution_id} for adapter {self.adapter_id}"
            )
            ...
            self.logger.info(
                f"Successfully processed request {exec_context.execution_id} in {execution_time:.3f}s"
            )
            ...
            self.performance_monitor.record_execution(execution_result)
            self.logger.error(
                f"Request {exec_context.execution_id} timed out after {execution_time:.3f}s"
            )
            ...
            self.performance_monitor.record_execution(execution_result)
            self.logger.error(
                f"Request {exec_context.execution_id} failed: {error_msg}"
            )
    ...
    async def _process_with_timeout(
        ...
        if context.timeout:
            return await asyncio.wait_for(
                self._process_impl(input_data, context), timeout=context.timeout
            )
        else:
    ...
    async def health_check(self) -> HealthCheckResult:
        ...
        try:
            ...
            base_checks = {
                ...
                "status_healthy": self.status
                not in [AdapterStatus.ERROR, AdapterStatus.DEPRECATED],
            }
