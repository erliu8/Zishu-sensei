...
try:
    ...
    raise ImportError(
        "pydantic is required for metadata management. Install with: pip install pydantic"
    )
...
class AdapterVersion(BaseModel):
    ...
    def validate_version_format(cls, v):
        ...
        if len(parts) < 2 or len(parts) > 4:
            raise ValueError(
                'Version format should be like "1.0", "1.0.0", or "1.0.0.1"'
            )
        for part in parts:
            try:
                int(part)
                ...
                if "-" not in part:
                    raise ValueError(
                        "Version parts must be numeric or contain pre-release identifier"
                    )
        return v
...
class AdapterMetadata(BaseModel):
    ...
    # 技术规格
    dependencies: List[AdapterDependency] = Field(
        default_factory=list, description="依赖列表"
    )
    configuration: List[AdapterConfiguration] = Field(
        default_factory=list, description="配置项"
    )
    permissions: AdapterPermissions = Field(
        default_factory=AdapterPermissions, description="权限要求"
    )
    ...
    # 性能和统计
    performance_metrics: AdapterPerformanceMetrics = Field(
        default_factory=AdapterPerformanceMetrics, description="性能指标"
    )
    usage_statistics: AdapterUsageStatistics = Field(
        default_factory=AdapterUsageStatistics, description="使用统计"
    )
...
class MetadataStorage(ABC):
    ...
    @abstractmethod
    async def search(
        self, query: MetadataQuery, options: MetadataSearchOptions
...
class FileSystemMetadataStorage(MetadataStorage):
    ...
    async def save(self, metadata: AdapterMetadata) -> bool:
        ...
        async with self._lock:
            try:
                ...
                self.logger.error(
                    f"Failed to save metadata for {metadata.adapter_id}: {e}"
                )
                raise
    ...
    async def load(self, adapter_id: str) -> Optional[AdapterMetadata]:
        ...
        try:
            ...
            self.logger.warning(
                f"Corrupted metadata file for {adapter_id}: {e}. Removing corrupted file."
            )
            # 删除损坏的文件
            try:
                file_path.unlink()
                ...
                self.logger.error(
                    f"Failed to remove corrupted file {file_path}: {unlink_error}"
                )
            return None
        except Exception as e:
    ...
    def _apply_filters(
        self,
        metadata_list: List[AdapterMetadata],
        query: MetadataQuery,
        ...
        for metadata in metadata_list:
            # 检查是否包含已废弃的适配器
            if (
                not options.include_deprecated
                and metadata.status == AdapterStatus.DEPRECATED
            ):
                continue
    ...
    def _matches_query(self, metadata: AdapterMetadata, query: MetadataQuery) -> bool:
        ...
        # 安全级别过滤
        if (
            query.security_levels
            and metadata.permissions.security_level not in query.security_levels
        ):
            return False
        ...
        if query.capability_categories:
            ...
            if not any(
                cat in capability_categories for cat in query.capability_categories
            ):
                return False
        ...
        if (
            ...
            and metadata.performance_metrics.success_rate < query.min_success_rate
        ):
            return False
        if (
            query.min_executions
            and metadata.usage_statistics.total_executions < query.min_executions
        ):
            return False
    ...
    def _apply_sorting(
        self, metadata_list: List[AdapterMetadata], options: MetadataSearchOptions
    ) -> List[AdapterMetadata]:
        """应用排序"""
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to sort by {sort_field}, using default order: {e}"
            )
    ...
    async def backup(self, backup_path: str) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Backup completed: {backup_info['adapter_count']} adapters backed up to {backup_path}"
            )
            return True
    ...
    async def restore(self, backup_path: str) -> bool:
        ...
        try:
            ...
            if info_file.exists():
                ...
                self.logger.info(
                    f"Restore completed: {restored_count} adapters restored from {backup_path}"
                )
                return True
            else:
                ...
                self.logger.info(
                    f"Restore completed: {restored_count} adapters restored"
                )
    ...
    def get(self, adapter_id: str) -> Optional[AdapterMetadata]:
        ...
        with self._lock:
            ...
            try:
                ...
                self.logger.warning(
                    f"Failed to parse cached metadata for {adapter_id}: {e}"
                )
                self._remove_entry(adapter_id)
                return None
...
class MetadataManager:
    ...
    def remove_event_listener(
        self, listener: Callable[[str, str, AdapterMetadata], None]
    ):
        """移除事件监听器"""
    ...
    async def save_metadata(
        self, metadata: AdapterMetadata, notify: bool = True
    ) -> bool:
        """
        保存适配器元数据
        ...
        """
        # 检查是否是更新操作
        existing_metadata = await self.get_metadata(
            metadata.adapter_id, from_cache=False
        )
        ...
        if success:
            ...
            self.logger.info(
                f"{'Updated' if is_update else 'Created'} metadata for adapter {metadata.adapter_id}"
            )

        return success

    async def get_metadata(
        self, adapter_id: str, from_cache: bool = True
    ...
    ) -> Union[List[str], List[AdapterMetadata]]:
        """
        列出所有适配器
        ...
        if response_time_ms is not None:
            ...
            if total_executions == 1:
                ...
                current_metrics.avg_response_time_ms = (
                    prev_avg * (total_executions - 1) + response_time_ms
                ) / total_executions

                # 更新最大最小值
                current_metrics.max_response_time_ms = max(
                    current_metrics.max_response_time_ms, response_time_ms
                )
                current_metrics.min_response_time_ms = min(
                    current_metrics.min_response_time_ms, response_time_ms
                )

            # 更新成功率和错误率
            current_metrics.success_rate = (
                metadata.usage_statistics.successful_executions / total_executions
            )
            current_metrics.error_rate = (
                metadata.usage_statistics.failed_executions / total_executions
            )
    ...
    async def get_adapter_statistics(self, adapter_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "uptime_days": (datetime.now(timezone.utc) - metadata.created_at).days
    ...
    async def validate_metadata(
        ...
        for cfg in required_configs:
            if cfg.default_value is None:
                errors.append(
                    f"Required configuration '{cfg.name}' must have a default value"
                )
    ...
    async def cleanup(self):
        ...
        if (
            self.enable_cache
            and self.cache
            and hasattr(self.cache, "_cleanup_task")
            and self.cache._cleanup_task
        ):
...
def create_metadata_manager(
    ...
    **kwargs,
    ...
    # 创建管理器
    return MetadataManager(storage=storage, enable_cache=enable_cache, **kwargs)
...
async def create_adapter_metadata(
    ...
    **kwargs,
    ...
    version_info = AdapterVersion(
        version=version, release_date=datetime.now(timezone.utc)
    )
