...
class LoraManager(AdvanceQuantizationManager):
    ...
    def __init__(self,
        ...
        self.target_modules = self.lora_config.get("target_modules",["q_proj","v_proj","k_proj","o_proj"]) #LoRA适配器模块
        
        #适配器路径
        self.adapter_path = Path(self.config.get("model",{}).get("adapters_path","./models/adapters"))
    ...
    def create_lora_config(self,task_type:TaskType=TaskType.CAUSAL_LM)->LoraConfig:
        """创建LoRA配置
    ...
    def generate(self,prompt:str,adapter_name:Optional[str]=None,**kwargs)->str:
        """
        生成文本
        ...
        temperature = kwargs.get("temperature",
                                 self.config.get("inference",{}).get("temperature",0.7))
        top_p = kwargs.get("top_p",
                           self.config.get("inference",{}).get("top_p",0.9))
        top_k = kwargs.get("top_k",
                           self.config.get("inference",{}).get("top_k",40))
