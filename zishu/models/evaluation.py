...
class EvaluationManager(ModelManager):
    ...
    def evaluate_anime_terminology(self,dataset:Dataset)->Dict[str,float]:
        ...
        if not all(field in dataset.column_names for field in required_fields):
            ...
            return {"anime_terminology":0.0,"error":f"数据集缺少必要的字段:{missing}","passed":False}
        ...
        try:
            ...
            threshold = self.eval_config.get("threshold",{}).get("anime_terminology",0.7)
    ...
    def evaluate_anime_expression(self,dataset:Dataset)->Dict[str,float]:
        ...
        if not all(field in dataset.column_names for field in required_fields):
            missing = [f for f in required_fields if f not in dataset.column_names]
            self.logger.error(f"数据集缺少必要的字段:{missing}")
            return {"anime_expression":0.0,"error":f"数据集缺少必要的字段:{missing}","passed":False}
        ...
        try:
            for sample in tqdm(dataset,desc="评估二次元表达"):
                ...
                try:
                    ...
                    # 评估表达方式的符合程度
                    style_score = self.evaluate_expression_style(response,reference_style,category)
    ...
    def evaluate_expression_style(self,response:str,reference_style:str,category:str)->float:
        """
        评估表达方式的符合程度
    ...
    def evaluate_anime_cultural_background(self,dataset:Dataset)->Dict[str,float]:
        ...
        if not all(field in dataset.column_names for field in required_fields):
            missing = [f for f in required_fields if f not in dataset.column_names]
            self.logger.error(f"数据集缺少必要的字段:{missing}")
            return {"anime_cultural_background":0.0,"error":f"数据集缺少必要的字段:{missing}","passed":False}
        ...
        try:
            for sample in tqdm(dataset,desc="评估二次元文化背景"):
                ...
                try:
                    ...
                    # 计算关键点覆盖率
                    covered_points = sum(1 for point in key_points if point.lower() in response.lower())
            ...
            threshold = self.eval_config["threshold"].get("anime_cultural_background",0.7)
